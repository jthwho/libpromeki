/**
 * @file      mediaio_overrides.cpp
 * @copyright Howard Logic. All rights reserved.
 *
 * See LICENSE file in the project root folder for license information.
 *
 * Tests for MediaIO::applyOutputOverrides — the shared helper every
 * transform backend (CSC, FrameSync, SRC, VideoEncoder, VideoDecoder)
 * and the planner use to derive the produced MediaDesc from an input
 * MediaDesc + a MediaConfig.
 */
...
TEST_CASE("MediaIO_applyOutputOverrides_PassThroughOnEmptyConfig") {
        ...
        const MediaDesc out = MediaIO::applyOutputOverrides(input, cfg);
        CHECK(out == input);
}

TEST_CASE("MediaIO_applyOutputOverrides_OutputPixelFormatOverrides") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_InvalidPixelFormatIsPassThrough") {
...
TEST_CASE("MediaIO_applyOutputOverrides_OutputFrameRateOverrides") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_InvalidFrameRateIsPassThrough") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_OutputAudioRateOverrides") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_ZeroAudioRateIsPassThrough") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_OutputAudioChannelsOverrides") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_ZeroAudioChannelsIsPassThrough") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_OutputAudioDataTypeOverrides") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_InvalidAudioDataTypeIsPassThrough") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig     cfg;
...
TEST_CASE("MediaIO_applyOutputOverrides_AppliesToAllVideoLayers") {
...
TEST_CASE("MediaIO_applyOutputOverrides_AppliesToAllAudioLayers") {
        MediaDesc input = makeVideoAudioDesc();
        AudioDesc ad2;
...
TEST_CASE("MediaIO_applyOutputOverrides_CombinedOverrides") {
...
TEST_CASE("MediaIO_applyOutputOverrides_EmptyInputStaysEmpty") {
        ...
        const MediaDesc out = MediaIO::applyOutputOverrides(MediaDesc(), cfg);
        CHECK(out.imageList().isEmpty());
        CHECK(out.audioList().isEmpty());
}

// ---------------------------------------------------------------
// MediaIO::defaultUncompressedPixelFormat
// ---------------------------------------------------------------
//
...
TEST_CASE("MediaIO_defaultUncompressedPixelFormat_InvalidSourceFallsBackToRGBA") {
        // Default-constructed PixelFormat is invalid — isValid() gates
        // the YCbCr check to false, so the helper must return RGBA.
        const PixelFormat out = MediaIO::defaultUncompressedPixelFormat(PixelFormat());
        CHECK(out.id() == PixelFormat::RGBA8_sRGB);
}

TEST_CASE("MediaIO_defaultUncompressedPixelFormat_YUVSourceStaysYUV") {
        ...
        const PixelFormat out =
                MediaIO::defaultUncompressedPixelFormat(PixelFormat(PixelFormat::YUV8_420_SemiPlanar_Rec709));
        CHECK(out.id() == PixelFormat::YUV8_422_Rec709);
}

TEST_CASE("MediaIO_defaultUncompressedPixelFormat_CompressedYUVStaysYUV") {
        ...
        const PixelFormat out = MediaIO::defaultUncompressedPixelFormat(PixelFormat(PixelFormat::H264));
        CHECK(out.id() == PixelFormat::YUV8_422_Rec709);
}

TEST_CASE("MediaIO_defaultUncompressedPixelFormat_RGBSourceFallsBackToRGBA") {
        // RGB source — not YCbCr, so RGBA fallback wins.
        const PixelFormat out = MediaIO::defaultUncompressedPixelFormat(PixelFormat(PixelFormat::RGB8_sRGB));
        CHECK(out.id() == PixelFormat::RGBA8_sRGB);
}

TEST_CASE("MediaIO_defaultUncompressedPixelFormat_RGBACarriesThroughAsRGBA") {
        // Explicit RGBA sRGB source — the idempotent path.
        const PixelFormat out = MediaIO::defaultUncompressedPixelFormat(PixelFormat(PixelFormat::RGBA8_sRGB));
        CHECK(out.id() == PixelFormat::RGBA8_sRGB);
}
