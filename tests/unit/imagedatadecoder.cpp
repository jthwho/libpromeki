...
namespace {
        ...
        UncompressedVideoPayload::Ptr encodeOne(uint32_t width, uint32_t height, PixelFormat::ID id, uint64_t payload,
                                                uint32_t firstLine = 0, uint32_t lineCount = 16) {
                ...
                REQUIRE(enc.encode(*img.modify(), ImageDataEncoder::Item{firstLine, lineCount, payload}).isOk());
                return img;
        }
...
TEST_CASE("ImageDataDecoder rejects mismatched image descriptor") {
        ...
        auto other = UncompressedVideoPayload::allocate(ImageDesc(1280, 32, PixelFormat(PixelFormat::RGBA8_sRGB)));
...
TEST_CASE("ImageDataDecoder rejects band that runs past the bottom") {
        ...
        auto img = UncompressedVideoPayload::allocate(ImageDesc(1920, 32, PixelFormat(PixelFormat::RGBA8_sRGB)));
        std::memset(img.modify()->data()[0].data(), 0, img->plane(0).size());
