...
static HttpTestResponse doRequest(uint16_t port,
                                  const String &method,
                                  const String &path,
        ...
        extraHeaders.forEach([&](const String &name, const String &value) {
                req += name + ": " + value + "\r\n";
        });
        req += "\r\n";
        req += body;
...
TEST_CASE("HttpServer - path parameter") {
        ...
        f.configure([](HttpServer &s) {
                s.route("/items/{id}", HttpMethod::Get,
                        [](const HttpRequest &req, HttpResponse &res) {
                                res.setText("id=" + req.pathParam("id"));
                        });
        });
        f.listenOnAnyPort();
...
TEST_CASE("HttpServer - file serving via HttpFileHandler") {
        ...
        fix.configure([&](HttpServer &s) {
                HttpHandler::Ptr handler = HttpHandler::Ptr::takeOwnership(
                        new HttpFileHandler(scratchDir));
...
static const TestDB::ID kDbWidth  = TestDB::declareID("Width",
        VariantSpec()
...
static const TestDB::ID kDbHeight = TestDB::declareID("Height",
        VariantSpec()
                .setTypes({Variant::TypeS64, Variant::TypeU64})
