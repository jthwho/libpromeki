...
TEST_CASE("HttpRouter") {
        ...
        SUBCASE("exact match invokes the handler") {
                HttpRouter r;
                bool       called = false;
                r.route("/ping", HttpMethod::Get, [&](const HttpRequest &, HttpResponse &res) {
                        called = true;
                        res.setText("pong");
                });
        ...
        SUBCASE("method mismatch -> 405 with Allow") {
                HttpRouter r;
                r.route("/thing", HttpMethod::Get, [](const auto &, auto &res) { res.setText("g"); });
                r.route("/thing", HttpMethod::Post, [](const auto &, auto &res) { res.setText("p"); });
        ...
        SUBCASE("single-segment path parameter") {
                HttpRouter r;
                String     captured;
                r.route("/items/{id}", HttpMethod::Get, [&](const HttpRequest &req, HttpResponse &res) {
        ...
        SUBCASE("greedy tail parameter") {
                HttpRouter r;
                String     captured;
                r.route("/files/{path:*}", HttpMethod::Get, [&](const HttpRequest &req, HttpResponse &res) {
        ...
        SUBCASE("literal match beats parameterized") {
                HttpRouter r;
                String     picked;
                r.route("/api/{thing}", HttpMethod::Get, [&](const auto &, auto &res) {
                        picked = "param";
                        res.setText("p");
                });
                r.route("/api/status", HttpMethod::Get, [&](const auto &, auto &res) {
                        picked = "literal";
                        res.setText("s");
                });
        ...
        SUBCASE("middleware runs before handler in order") {
                ...
                r.route("/x", HttpMethod::Get, [&](const auto &, auto &res) {
                        trace += "H";
                        res.setText("ok");
                });
        ...
        SUBCASE("middleware can short-circuit") {
                ...
                r.route("/x", HttpMethod::Get, [&](const auto &, auto &) { handlerRan = true; });
        ...
        SUBCASE("any() matches every method") {
                ...
                for (const auto &m : {HttpMethod::Get, HttpMethod::Post, HttpMethod::Put, HttpMethod::Delete}) {
