...
TEST_CASE("HttpConnection_FixedLengthBodyStreamStillCompletes") {
        ...
        fix.configure([&](HttpServer &s) {
                s.route("/fixed", HttpMethod::Get,
                        [&](const HttpRequest &, HttpResponse &res) {
                                BufferIODevice *bd = new BufferIODevice(&payload);
                                bd->open(IODevice::ReadOnly);
                                IODevice::Shared dev =
                                        IODevice::Shared::takeOwnership(bd);
                                res.setStatus(HttpStatus::Ok);
                                res.setBodyStream(dev, /*length=*/30,
                                                  "text/plain");
                                res.setHeader("Connection", "close");
                        });
        });
                ...
                if(rawLen > 0 && sock.bytesAvailable() == 0) {
                        ...
                        if(got > 0) {
                                if(rawLen + 1 > raw.allocSize()) {
                                        ...
                                        std::memcpy(grown.data(), raw.data(),
                                                    rawLen);
                                        raw = std::move(grown);
                                }
