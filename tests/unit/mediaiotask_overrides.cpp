...
TEST_CASE("MediaIOTask_applyOutputOverrides_OutputPixelDescOverrides") {
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig cfg;
        ...
        CHECK(out.imageList()[0].pixelDesc().id() == PixelDesc::RGBA8_sRGB);
...
TEST_CASE("MediaIOTask_applyOutputOverrides_InvalidPixelDescIsPassThrough") {
        // An invalid PixelDesc means "inherit" — the helper must not
        ...
        const MediaDesc out = MediaIOTask::applyOutputOverrides(input, cfg);
        CHECK(out.imageList()[0].pixelDesc() == input.imageList()[0].pixelDesc());
}
...
TEST_CASE("MediaIOTask_applyOutputOverrides_OutputAudioRateOverrides") {
        ...
        CHECK(out.audioList()[0].dataType() == input.audioList()[0].dataType());
}
...
TEST_CASE("MediaIOTask_applyOutputOverrides_AppliesToAllVideoLayers") {
        ...
        CHECK(out.imageList()[0].pixelDesc().id() == PixelDesc::RGBA8_sRGB);
        CHECK(out.imageList()[1].pixelDesc().id() == PixelDesc::RGBA8_sRGB);
...
TEST_CASE("MediaIOTask_applyOutputOverrides_AppliesToAllAudioLayers") {
        ...
        ad2.setChannels(1);
        ad2.setDataType(AudioDesc::PCMI_Float32LE);
        ...
        for(size_t i = 0; i < out.audioList().size(); ++i) {
                ...
                CHECK(out.audioList()[i].dataType()   == AudioDesc::PCMI_S16LE);
        }
}
...
TEST_CASE("MediaIOTask_applyOutputOverrides_CombinedOverrides") {
        ...
        const MediaDesc input = makeVideoAudioDesc();
        MediaConfig cfg;
        cfg.set(MediaConfig::OutputPixelDesc, PixelDesc(PixelDesc::RGBA8_sRGB));
        cfg.set(MediaConfig::OutputFrameRate, FrameRate(FrameRate::FPS_24));
        cfg.set(MediaConfig::OutputAudioRate, 96000.0f);
        cfg.set(MediaConfig::OutputAudioChannels, 6);
        ...
        CHECK(out.audioList()[0].sampleRate() == doctest::Approx(96000.0f));
        CHECK(out.audioList()[0].channels()   == 6);
        CHECK(out.audioList()[0].dataType()   == AudioDesc::PCMI_Float32LE);
}
