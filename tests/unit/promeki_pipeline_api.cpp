...
HttpRsp doRequest(uint16_t port,
                  const String &method,
                  const String &path,
        ...
        const String headerBlock = (eol == String::npos)
                ? String() : head.mid(eol + 2);
...
MediaPipelineConfig makeTpgToNullPacingConfig() {
        ...
        src.config.set(MediaConfig::VideoFormat,
                       VideoFormat(VideoFormat::Smpte720p60));
        cfg.addStage(src);
...
MediaPipelineConfig makeTpgToMjpegConfig() {
        ...
        src.config.set(MediaConfig::VideoFormat,
                       VideoFormat(VideoFormat::Smpte720p60));
        src.config.set(MediaConfig::VideoPixelFormat,
...
TEST_CASE("ApiRoutes - GET /api/types/{name}/schema returns spec map") {
        ...
        auto rspMjpeg = doRequest(fix.port, "GET",
                                  "/api/types/MjpegStream/schema");
        ...
        auto rspMissing = doRequest(fix.port, "GET",
                                    "/api/types/DefinitelyNotRegistered/schema");
...
TEST_CASE("ApiRoutes - schema emits presets block for FrameRate-typed specs") {
        ...
        for(int i = 0; i < presets.size(); ++i) {
                ...
                if(preset.getString("label") == "24"
                   && preset.getString("value") == "24/1") {
                        sawTwentyFour = true;
                        break;
                }
        }
...
TEST_CASE("ApiRoutes - defaults and metadata routes") {
        ...
        auto metaRsp = doRequest(fix.port, "GET",
                                 "/api/types/TPG/metadata");
        ...
        auto bad = doRequest(fix.port, "GET",
                             "/api/types/Bogus/defaults");
...
TEST_CASE("ApiRoutes - CRUD lifecycle for a TPG to NullPacing pipeline") {
        ...
        auto created = doRequest(fix.port, "POST", "/api/pipelines",
                                 body.toString(0), "application/json");
        ...
        auto desc = doRequest(fix.port, "GET",
                              String("/api/pipelines/") + id);
        ...
        auto build = doRequest(fix.port, "POST",
                               String("/api/pipelines/") + id + "/build");
        ...
        // Open and start.
        auto opened = doRequest(fix.port, "POST",
                                String("/api/pipelines/") + id + "/open");
        CHECK(opened.status == 200);
        auto started = doRequest(fix.port, "POST",
                                 String("/api/pipelines/") + id + "/start");
        CHECK(started.status == 200);

        // Let the source pump for a few ticks.
        std::this_thread::sleep_for(std::chrono::milliseconds(200));

        auto stopped = doRequest(fix.port, "POST",
                                 String("/api/pipelines/") + id + "/stop");
        CHECK(stopped.status == 200);
        auto closed = doRequest(fix.port, "POST",
                                String("/api/pipelines/") + id + "/close");
        CHECK(closed.status == 200);

        auto del = doRequest(fix.port, "DELETE",
                             String("/api/pipelines/") + id);
        CHECK(del.status == 204);

        auto descMissing = doRequest(fix.port, "GET",
                                     String("/api/pipelines/") + id);
...
TEST_CASE("ApiRoutes - autoplan inserts bridge stage for TPG to MjpegStream") {
        ...
        // Tear down.
        doRequest(fix.port, "POST",
                  String("/api/pipelines/") + id + "/close");
        doRequest(fix.port, "DELETE",
                  String("/api/pipelines/") + id);
}
...
String waitForState(uint16_t port, const String &id, const String &want,
                    int timeoutMs) {
        ElapsedTimer t; t.start();
        String last;
        while(t.elapsed() < timeoutMs) {
                auto rsp = doRequest(port, "GET",
                                     String("/api/pipelines/") + id);
                if(rsp.status == 200) {
                        Error pe;
...
String driveTo(uint16_t port, const String &id, const String &target) {
        const auto post = [&](const String &verb) {
                return doRequest(port, "POST",
                        String("/api/pipelines/") + id + "/" + verb);
        };
...
String createPipelineWithConfig(uint16_t port,
                                const String &name,
                                const MediaPipelineConfig &cfg) {
        JsonObject body;
        body.set("name", name);
        body.set("userConfig", cfg.toJson());
        auto created = doRequest(port, "POST", "/api/pipelines",
                                 body.toString(0), "application/json");
...
TEST_CASE("ApiRoutes - POST /run drives every reachable state to Running") {
        ...
        for(size_t i = 0; i < cases.size(); ++i) {
                ...
                const String id = createPipelineWithConfig(
                        fix.port, name, makeTpgToNullPacingConfig());

                const String reached = driveTo(fix.port, id, target);
                CHECK_MESSAGE(reached == target,
                        ("driveTo(" + target + ") observed " + reached).cstr());

                auto run = doRequest(fix.port, "POST",
                        String("/api/pipelines/") + id + "/run");
                CHECK_MESSAGE(run.status == 200,
                        ("/run from " + target + " returned non-200").cstr());

                // After /run, the pipeline must be Running.  Allow a
                // short window for the strand to publish StateChanged.
                const String settled =
                        waitForState(fix.port, id, "Running", 1500);
                CHECK_MESSAGE(settled == "Running",
                        ("/run from " + target + " ended in " + settled).cstr());
...
TEST_CASE("ApiRoutes - POST /run from Running is a no-op") {
        ApiFixture fix;
        const String id = createPipelineWithConfig(
                fix.port, "run-noop", makeTpgToNullPacingConfig());

        // Drive to Running explicitly.
        REQUIRE(doRequest(fix.port, "POST",
                String("/api/pipelines/") + id + "/build").status == 200);
        REQUIRE(doRequest(fix.port, "POST",
                String("/api/pipelines/") + id + "/open").status == 200);
        REQUIRE(doRequest(fix.port, "POST",
                String("/api/pipelines/") + id + "/start").status == 200);
...
TEST_CASE("ApiRoutes - POST /run on unknown id returns 404") {
        ApiFixture fix;
        auto rsp = doRequest(fix.port, "POST",
                "/api/pipelines/ZZZZZZZZ/run");
...
TEST_CASE("ApiRoutes - PUT settings round-trips and is observable on GET") {
        ...
        auto put = doRequest(fix.port, "PUT",
                             String("/api/pipelines/") + id + "/settings",
                             s.toJson().toString(0), "application/json");
        ...
        auto get = doRequest(fix.port, "GET",
                             String("/api/pipelines/") + id + "/settings");
...
struct WsClient {
        ...
        bool open(uint16_t port, const String &path) {
                ...
                const String req =
                        ...
                        "\r\n";
                if(sock.write(req.cstr(), req.byteCount())
                   != static_cast<int64_t>(req.byteCount())) {
                        return false;
                }
                ...
                while(t.elapsed() < 2000) {
                        ...
                        if(got > 0) {
                                ...
                                if(sep != String::npos) {
                                        ...
                                        std::memcpy(pending.data(), tail.cstr(),
                                                    tail.byteCount());
        ...
        String readTextFrame(int timeoutMs) {
                ...
                while(t.elapsed() < timeoutMs) {
                                ...
                                if(got > 0) {
                                        if(pendingLen + static_cast<size_t>(got)
                                           > pending.allocSize()) {
                                                ...
                                                std::memcpy(grown.data(),
                                                        pending.data(), pendingLen);
                                                pending = std::move(grown);
                                        }
                                        std::memcpy(static_cast<uint8_t *>(pending.data())
                                                + pendingLen, buf, got);
                        ...
                        String text;
                        if(opcode == 0x1) {
                                text = String(reinterpret_cast<const char *>(payload),
                                              plen);
                        }
                        // Shift the consumed bytes off the front.
                        const size_t consumed = headLen + plen;
                        std::memmove(pending.data(),
                                static_cast<uint8_t *>(pending.data()) + consumed,
                                pendingLen - consumed);
                        pendingLen -= consumed;
...
TEST_CASE("ApiRoutes - WS /api/events delivers state-changed events") {
        ...
        WsClient filtered;
        REQUIRE(filtered.open(fix.port,
                              String("/api/events?pipeline=") + id));

        WsClient mismatched;
        REQUIRE(mismatched.open(fix.port,
                                String("/api/events?pipeline=ZZZZZZZZ")));
...
size_t decodeChunkedRaw(const uint8_t *in, size_t inLen,
                        Buffer &out, size_t &outLen) {
        size_t i = 0;
        while(i < inLen) {
                const uint8_t *eol = static_cast<const uint8_t *>(
                        std::memchr(in + i, '\r', inLen - i));
                ...
                const size_t chunkLen = static_cast<size_t>(
                        std::strtoul(hex.cstr(), nullptr, 16));
                ...
                std::memcpy(static_cast<uint8_t *>(out.data()) + outLen,
                            in + i + hdrLen + 2, chunkLen);
                outLen += chunkLen;
                i += needed;
        }
        return i;
...
TEST_CASE("ApiRoutes - preview route streams multipart MJPEG when running") {
        ...
        const HttpRsp preBuildRsp = doRequest(fix.port, "GET",
                String("/api/pipelines/") + id + "/preview/preview");
        ...
        const HttpRsp postBuildRsp = doRequest(fix.port, "GET",
                String("/api/pipelines/") + id + "/preview/preview");
        ...
        const String req =
                String("GET /api/pipelines/") + id + "/preview/preview"
                " HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n";
        ...
        while(t.elapsed() < 3000 && headerEnd == SIZE_MAX) {
                ...
                if(got > 0) {
                        ...
                        std::memcpy(static_cast<uint8_t *>(raw.data()) + rawLen,
                                    buf, got);
        ...
        while(t.elapsed() < 5000 && jpegCount < 3) {
                ...
                const uint8_t *bb =
                        static_cast<const uint8_t *>(raw.data())
                        + bodyStart + bodyConsumed;
                const size_t bbLen = rawLen - bodyStart - bodyConsumed;
                const size_t consumed = decodeChunkedRaw(bb, bbLen,
                                                         decoded, decodedLen);
                bodyConsumed += consumed;
                jpegCount = countJpegs(
                        static_cast<const uint8_t *>(decoded.data()),
                        decodedLen);
        }
        CHECK(jpegCount >= 3);
        sock.close();
...
TEST_CASE("ApiRoutes - preview route 400 when stage is wrong type") {
        ...
        auto bad = doRequest(fix.port, "GET",
                             String("/api/pipelines/") + id + "/preview/tpg1");
        ...
        auto missing = doRequest(fix.port, "GET",
                                 String("/api/pipelines/") + id + "/preview/nope");
