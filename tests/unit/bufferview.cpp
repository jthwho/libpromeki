...
TEST_CASE("BufferView") {
        ...
        SUBCASE("multiple views sharing one buffer") {
                ...
                CHECK(v1.buffer().impl().ptr() == v2.buffer().impl().ptr());
                CHECK(v2.buffer().impl().ptr() == v3.buffer().impl().ptr());
        ...
        SUBCASE("copy semantics") {
                ...
                BufferView v2 = v1;
                CHECK(v2.buffer().impl().ptr() == v1.buffer().impl().ptr());
                CHECK(v2.offset() == 100);
                CHECK(v2.size() == 200);
        }
}
...
TEST_CASE("BufferViewList domain operations") {
        ...
        SUBCASE("ensureExclusive: dedup clones one-per-unique-buffer") {
                ...
                auto buf = Buffer(1024);
                buf.setSize(1024);
                const BufferImpl *originalKey = buf.impl().ptr();
                BufferView    list = {
                ...
                CHECK(list[0].buffer().impl().ptr() != originalKey);
                CHECK(list[1].buffer().impl().ptr() != originalKey);
                CHECK(list[2].buffer().impl().ptr() != originalKey);
                // And all three views now reference the SAME clone —
                // only one CoW happened, not three.
                const BufferImpl *cloneKey = list[0].buffer().impl().ptr();
                CHECK(list[1].buffer().impl().ptr() == cloneKey);
                CHECK(list[2].buffer().impl().ptr() == cloneKey);
                ...
                CHECK(buf.impl().ptr() == originalKey);
        ...
        SUBCASE("ensureExclusive: list-exclusive buffers are left alone") {
                ...
                const BufferImpl *key = buf.impl().ptr();
                buf = Buffer();
                REQUIRE(list.isExclusive());

                list.ensureExclusive();
                // Buffers unchanged — no clone happened.
                CHECK(list[0].buffer().impl().ptr() == key);
                CHECK(list[1].buffer().impl().ptr() == key);
        }

        SUBCASE("ensureExclusive: mixed — one shared, one exclusive") {
                auto shared = Buffer(1024);
                auto unique = Buffer(512);
                shared.setSize(1024);
                unique.setSize(512);
                BufferView list = {
                ...
                const BufferImpl *sharedKey = shared.impl().ptr();
                const BufferImpl *uniqueKey = unique.impl().ptr();
                // Both buffers have an external holder (the local
                // Ptrs).  ensureExclusive must clone both.
                list.ensureExclusive();
                CHECK(list[0].buffer().impl().ptr() != sharedKey);
                CHECK(list[1].buffer().impl().ptr() != uniqueKey);
