...
TEST_CASE("H264Bitstream — Annex-B NAL iteration") {
        ...
        SUBCASE("empty input returns Ok with no invocations") {
                BufferView empty;
                int        calls = 0;
                Error      err = H264Bitstream::forEachAnnexBNal(empty, [&](const H264Bitstream::NalUnit &) {
                        ++calls;
                        return Error::Ok;
                });
                CHECK(err == Error::Ok);
        ...
        SUBCASE("3-byte start codes split payloads correctly") {
                ...
                auto buf = makeBuffer({0x00, 0x00, 0x01, 0xaa, 0xbb, 0x00, 0x00, 0x01, 0xcc, 0xdd, 0xee});
                std::vector<std::vector<uint8_t>> captured;
                Error err = H264Bitstream::forEachAnnexBNal(viewOf(buf), [&](const H264Bitstream::NalUnit &nal) {
                        std::vector<uint8_t> v(nal.view.data(), nal.view.data() + nal.view.size());
        ...
        SUBCASE("4-byte start codes split payloads correctly") {
                ...
                auto buf = makeBuffer({0x00, 0x00, 0x00, 0x01, 0x67, 0x42, 0xc0, 0x00, 0x00, 0x00, 0x01, 0x68, 0xce,
        ...
        SUBCASE("mixed 3- and 4-byte start codes both parsed") {
                ...
                auto buf = makeBuffer(
                        {0x00, 0x00, 0x01, 0xaa, 0x00, 0x00, 0x00, 0x01, 0xbb, 0xcc, 0x00, 0x00, 0x01, 0xdd});
                std::vector<std::vector<uint8_t>> captured;
                Error err = H264Bitstream::forEachAnnexBNal(viewOf(buf), [&](const H264Bitstream::NalUnit &nal) {
                        std::vector<uint8_t> v(nal.view.data(), nal.view.data() + nal.view.size());
                        captured.push_back(std::move(v));
        ...
        SUBCASE("visitor can abort iteration by returning non-Ok") {
                auto  buf = makeBuffer({0x00, 0x00, 0x01, 0xaa, 0x00, 0x00, 0x01, 0xbb, 0x00, 0x00, 0x01, 0xcc});
                int   calls = 0;
                Error err = H264Bitstream::forEachAnnexBNal(viewOf(buf), [&](const H264Bitstream::NalUnit &) {
                        ++calls;
...
TEST_CASE("H264Bitstream — AVCC NAL iteration") {
        ...
        SUBCASE("walks 4-byte length prefix") {
                ...
                auto buf = makeBuffer({0x00, 0x00, 0x00, 0x02, 0xaa, 0xbb, 0x00, 0x00, 0x00, 0x03, 0xcc, 0xdd, 0xee});
                std::vector<std::vector<uint8_t>> captured;
                Error err = H264Bitstream::forEachAvccNal(viewOf(buf), 4, [&](const H264Bitstream::NalUnit &nal) {
                        std::vector<uint8_t> v(nal.view.data(), nal.view.data() + nal.view.size());
                        captured.push_back(std::move(v));
                        return Error::Ok;
                });
        ...
        SUBCASE("walks 2-byte length prefix") {
                ...
                Error err = H264Bitstream::forEachAvccNal(viewOf(buf), 2, [&](const H264Bitstream::NalUnit &nal) {
                        captured.assign(nal.view.data(), nal.view.data() + nal.view.size());
                        return Error::Ok;
                });
                CHECK(err == Error::Ok);
...
TEST_CASE("H264Bitstream — Annex-B ↔ AVCC round trip") {
        ...
        SUBCASE("Annex-B → AVCC with 4-byte length") {
                ...
                CHECK(err == Error::Ok);
                REQUIRE(out);
                std::vector<uint8_t> expected = {0x00, 0x00, 0x00, 0x03, 0x67, 0x42, 0xc0, 0x00, 0x00, 0x00,
                                                 0x02, 0x68, 0xce, 0x00, 0x00, 0x00, 0x02, 0x65, 0x88};
        ...
        SUBCASE("AVCC → Annex-B emits 4-byte start codes") {
                ...
                CHECK(err == Error::Ok);
                REQUIRE(out);
                std::vector<uint8_t> expected = {0x00, 0x00, 0x00, 0x01, 0xaa, 0xbb, 0x00,
...
TEST_CASE("AvcDecoderConfig — extract from Annex-B access unit") {
        ...
        SUBCASE("no SPS in input returns InvalidArgument") {
                std::vector<uint8_t> au = {0x00, 0x00, 0x00, 0x01, 0x68, 0xce, 0x3c, 0x80,
...
TEST_CASE("AvcDecoderConfig — serialize / parse round trip") {
        ...
        SUBCASE("SPS length overrun returns CorruptData") {
                ...
                auto             buf = makeBuffer({
                        0x01, 0x42, 0xe0, 0x1e, 0xff, 0xe1, 0x00, 0x64, // spsLen = 100
...
TEST_CASE("H264Bitstream::annexBToAvccFiltered") {
        ...
        SUBCASE("filter removes SPS and PPS, keeps IDR") {
                ...
                std::vector<uint8_t> expected = {0x00, 0x00, 0x00, 0x03, 0x65, 0x88, 0x11};
                CHECK(bytesOf(out) == expected);
        }
