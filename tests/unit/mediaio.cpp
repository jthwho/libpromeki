...
TEST_CASE("MediaIO_TPG_WriterNotSupported") {
        ...
        MediaIO *io = MediaIO::create(cfg);
        REQUIRE(io != nullptr);
        CHECK(io->open(MediaIO::Sink) == Error::NotSupported);
        delete io;
}
...
TEST_CASE("MediaIO_TPG_NothingEnabledFails") {
        ...
        MediaIO *io = MediaIO::create(cfg);
        REQUIRE(io != nullptr);
        CHECK(io->open(MediaIO::Source) == Error::InvalidArgument);
        delete io;
}
...
TEST_CASE("MediaIO_TPG_DoubleOpenFails") {
        ...
        REQUIRE(io != nullptr);
        REQUIRE(io->open(MediaIO::Source).isOk());
        CHECK(io->open(MediaIO::Source) == Error::AlreadyOpen);
        io->close();
        delete io;
}
...
TEST_CASE("MediaIO_ImageSequence_SidecarAudioPathOverride") {
        ...
        // Read back with the same config override — should auto-detect.
        {
                ...
                REQUIRE(rio != nullptr);
                REQUIRE(rio->open(MediaIO::Source).isOk());
