 ...
 */
inline ::promeki::UncompressedVideoPayload::Ptr decodeCompressedPayload(
        const ::promeki::CompressedVideoPayload::Ptr &src,
 ...
 */
inline ::promeki::CompressedVideoPayload::Ptr encodePayloadToCompressed(
        const ::promeki::UncompressedVideoPayload::Ptr &src,
        const ::promeki::PixelFormat &target,
        ...
        for(PixelFormat::ID s : sources) {
                if(src->desc().pixelFormat().id() == s) { sourceOk = true; break; }
        }
        if(!sourceOk && !sources.isEmpty()) {
                input = src->convert(PixelFormat(sources[0]),
                                     src->desc().metadata(), cfg);
                if(!input.isValid()) { delete enc; return CompressedVideoPayload::Ptr(); }
        }
