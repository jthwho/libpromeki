...
void buildRig(MjpegStreamRig &rig,
              VideoFormat::WellKnownFormat sourceFormat,
              const Rational<int> &maxFps,
              int quality      = 70,
              int ringDepth    = 1) {
        // ---- TPG source ----
...
size_t drainSomeBytes(TcpSocket &sock, Buffer &sink, size_t &sinkLen,
        ...
        while(t.elapsed() < timeoutMs) {
                ...
                if(sinkLen + static_cast<size_t>(got) > sink.allocSize()) {
                        Buffer grown(sink.allocSize() * 2 +
                                     static_cast<size_t>(got));
                        std::memcpy(grown.data(), sink.data(), sinkLen);
                        sink = std::move(grown);
                }
                std::memcpy(static_cast<uint8_t *>(sink.data()) + sinkLen,
...
size_t decodeChunked(const uint8_t *src, size_t len, Buffer &dst,
                     size_t &dstLen) {
        size_t cursor = 0;
        while(cursor < len) {
                ...
                String sizeLine(reinterpret_cast<const char *>(src + cursor),
                                crlf - cursor);
                const size_t semi = sizeLine.find(';');
                const String hex = (semi == String::npos)
                ...
                std::memcpy(static_cast<uint8_t *>(dst.data()) + dstLen,
                            src + bodyStart, chunkSize);
                dstLen += chunkSize;
...
void harvestJpegs(const uint8_t *decoded, size_t decodedLen,
                  size_t &searchStart,
                  promeki::List<Buffer> &out) {
...
TEST_CASE("MediaIOTask_MjpegStream_HttpRouteStreamsMultipart") {
        ...
        fix.configure([&](HttpServer &s) {
                rig.sink->registerHttpRoute(s, "/preview");
        });
        fix.listenOnAnyPort();
        ...
        const String statusLine = (firstEol == String::npos)
                ? head : head.left(firstEol);
        const size_t sp1 = statusLine.find(' ');
        const size_t sp2 = statusLine.find(' ', sp1 + 1);
        const int status = std::atoi(
                statusLine.mid(sp1 + 1, sp2 - sp1 - 1).cstr());
        CHECK(status == 200);

        const String headers = (firstEol == String::npos)
                ? String() : head.mid(firstEol + 2);
        ...
        for(size_t i = 0; i < lines.size(); ++i) {
                ...
                if(name.toLower() == "transfer-encoding"
                   && value.toLower().contains("chunked")) chunked = true;
        }
        ...
        while(timer.elapsed() < 3000 && parts.size() < 3) {
                ...
                const uint8_t *bb = static_cast<const uint8_t *>(raw.data())
                        + bodyStart + bodyConsumed;
                const size_t bbLen = rawLen - bodyStart - bodyConsumed;
                const size_t consumed = decodeChunked(bb, bbLen,
                                                       decoded, decodedLen);
                bodyConsumed += consumed;
                harvestJpegs(static_cast<const uint8_t *>(decoded.data()),
                             decodedLen, scanStart, parts);
        }
...
TEST_CASE("MediaIOTask_MjpegStream_HttpRouteRefusesWhenSinkNotOpen") {
        ...
        ServerFixture fix;
        fix.configure([&](HttpServer &s) {
                task->registerHttpRoute(s, "/preview");
        });
        fix.listenOnAnyPort();
        ...
        const String got(static_cast<const char *>(raw.data()),
                          static_cast<size_t>(std::min<size_t>(rawLen, 64)));
