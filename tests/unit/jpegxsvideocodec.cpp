...
static UncompressedVideoPayload::Ptr makePlanarRGB8(int w, int h) {
        auto img = UncompressedVideoPayload::allocate(ImageDesc(w, h, PixelFormat(PixelFormat::RGB8_Planar_sRGB)));
        if (!img.isValid()) return img;
        const auto &ml = img->desc().pixelFormat().memLayout();
        for (int p = 0; p < 3; p++) {
                uint8_t     *plane = img.modify()->data()[p].data();
                ...
                for (int y = 0; y < h; y++) {
                        ...
                        for (int x = 0; x < w; x++) {
                                if (p == 0)
                                        row[x] = static_cast<uint8_t>(x * 255 / w);
                                else if (p == 1)
                                        row[x] = static_cast<uint8_t>(y * 255 / h);
                                else
                                        row[x] = 128;
                        }
                }
...
static CompressedVideoPayload::Ptr encodeOneJxsFrame(const UncompressedVideoPayload::Ptr &src, const MediaConfig &cfg) {
        MediaConfig sessionCfg = cfg;
        if (!sessionCfg.contains(MediaConfig::OutputPixelFormat)) {
                sessionCfg.set(MediaConfig::OutputPixelFormat, PixelFormat(PixelFormat::JPEG_XS_YUV8_422_Rec709));
        }
...
static UncompressedVideoPayload::Ptr decodeOneJxsFrame(const CompressedVideoPayload::Ptr &pkt, PixelFormat target,
...
TEST_CASE("JpegXsVideoEncoder_Clamping") {
        ...
        CHECK(enc.bpp() == 6);
        setDecomposition(-1);
        CHECK(enc.decomposition() == 0);
        setDecomposition(99);
        CHECK(enc.decomposition() == 5);
        setDecomposition(3);
        CHECK(enc.decomposition() == 3);
}
...
TEST_CASE("JpegXsVideoCodec_DecodeTargetMismatch") {
        ...
        auto dec = decodeOneJxsFrame(pkt, PixelFormat(PixelFormat::YUV8_420_Planar_Rec709), 160, 120);
...
TEST_CASE("JpegXsVideoCodec_ImageConvertToRgba8") {
        ...
        auto rgba = promeki::tests::decodeCompressedPayload(pkt, PixelFormat(PixelFormat::RGBA8_sRGB));
...
TEST_CASE("JpegXsVideoCodec_RGB8_ImageConvertRoundTrip") {
        auto         src = UncompressedVideoPayload::allocate(ImageDesc(128, 96, PixelFormat(PixelFormat::RGB8_sRGB)));
        ...
        auto jxs = promeki::tests::encodePayloadToCompressed(src, PixelFormat(PixelFormat::JPEG_XS_RGB8_sRGB));
        REQUIRE(jxs.isValid());
        CHECK(jxs->desc().pixelFormat().id() == PixelFormat::JPEG_XS_RGB8_sRGB);

        auto decoded = promeki::tests::decodeCompressedPayload(jxs, PixelFormat(PixelFormat::RGB8_sRGB));
