...
TEST_CASE("Url: invalid strings fail to parse") {
        Error err = Error::Ok;
        Url u = Url::fromString(String(), &err);
        CHECK_FALSE(u.isValid());
        CHECK(err.isError());

        err = Error::Ok;
        u = Url::fromString("no-scheme", &err);
        CHECK_FALSE(u.isValid());
        CHECK(err.isError());

        err = Error::Ok;
        u = Url::fromString(":scheme-first", &err);
        CHECK_FALSE(u.isValid());
        CHECK(err.isError());

        err = Error::Ok;
        u = Url::fromString("1bad://x", &err);
        CHECK_FALSE(u.isValid());
        CHECK(err.isError());
}
...
TEST_CASE("Url: query parameters") {
        Url u = Url::fromString("pmfb://bridge?ring=4&sync=true&empty=&flag");
        CHECK(u.host() == "bridge");
...
TEST_CASE("Url: percent decoding in host and query") {
        Url u = Url::fromString("pmfb://hello%20world?k=v%20s&%3Dkey=%26val");
...
TEST_CASE("Url: userinfo and port") {
        Url u = Url::fromString("http://alice:secret@example.com:8080/path");
...
TEST_CASE("Url: IPv6 literal") {
        Url u = Url::fromString("http://[::1]:8080/");
        CHECK(u.host() == "::1");
        CHECK(u.port() == 8080);
        CHECK(u.path() == "/");
}
...
TEST_CASE("Url: opaque form") {
        Url u = Url::fromString("mailto:foo@example.com");
        CHECK(u.isValid());
...
TEST_CASE("Url: fragment") {
        Url u = Url::fromString("http://x/path?q=1#top");
...
TEST_CASE("Url: round-trip with query") {
        ...
        Url a = Url::fromString("pmfb://bridge?ring=4&sync=true");
        Url b = Url::fromString(a.toString());
        CHECK(a == b);
}

TEST_CASE("Url: round-trip preserves percent-encoding need") {
        Url a = Url::fromString("pmfb://name%20with%20spaces");
        Url b = Url::fromString(a.toString());
        CHECK(a == b);
...
TEST_CASE("Url: builder API") {
        ...
        CHECK(u.isValid());
        CHECK(u.hasAuthority());
        Url roundTrip = Url::fromString(u.toString());
        CHECK(roundTrip == u);
...
TEST_CASE("Url: equality") {
        Url a = Url::fromString("pmfb://x?a=1&b=2");
        Url b = Url::fromString("pmfb://x?b=2&a=1");
        CHECK(a == b);

        Url c = Url::fromString("pmfb://x?a=1");
        CHECK(a != c);

        Url d = Url::fromString("pmfb://y?a=1&b=2");
        CHECK(a != d);
}

TEST_CASE("Url: removeQueryValue") {
        Url u = Url::fromString("pmfb://x?a=1&b=2");
