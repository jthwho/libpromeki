...
TEST_CASE("Image_Copy") {
    Image img1(64, 64, PixelFormat::RGBA8_sRGB);
    Image img2 = img1;
...
TEST_CASE("Image_ConstructFromSize2D") {
    ...
    Image img(sz, PixelFormat::RGBA8_sRGB);
    CHECK(img.isValid());
    CHECK(img.width() == 640);
...
TEST_CASE("Image_DescAccessor") {
    Image img(1280, 720, PixelFormat::RGB8_sRGB);
    REQUIRE(img.isValid());
...
TEST_CASE("Image_RGB8_LineStride") {
    Image img(100, 100, PixelFormat::RGB8_sRGB);
    REQUIRE(img.isValid());
...
TEST_CASE("Image_EnsureExclusive_PreservesData") {
    Image img(32, 32, PixelFormat::RGB8_sRGB);
    REQUIRE(img.isValid());
    img.fill(0xAA);
...
TEST_CASE("Image_IsExclusive_ViaPtr") {
    Image::Ptr imgPtr = Image::Ptr::create(64, 64, PixelFormat::RGBA8_sRGB);
...
TEST_CASE("Image_FromCompressedData_Valid") {
    ...
    CHECK(img.width() == 1920);
    CHECK(img.height() == 1080);
    CHECK(img.pixelFormat().id() == PixelFormat::JPEG_RGB8_sRGB);
...
TEST_CASE("Image_FromCompressedData_RGBA8") {
    ...
    Image img = Image::fromCompressedData(fakeData, sizeof(fakeData),
                                          ...
                                          PixelFormat::JPEG_RGBA8_sRGB);
    REQUIRE(img.isValid());
    CHECK(img.pixelFormat().id() == PixelFormat::JPEG_RGBA8_sRGB);
    CHECK(img.isCompressed());
