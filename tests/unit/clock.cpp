...
namespace {
        ...
        class FakeClock : public Clock {
                public:
                        ...
                        void setRaw(int64_t ns) { _rawNs.store(ns, std::memory_order_relaxed); }

                        void setRawError(Error err) { _rawError = err; }
...
TEST_SUITE("Clock") {
        ...
        TEST_CASE("filter is applied before offset") {
                FakeClock c(Duration::fromNanoseconds(10), ClockPauseMode::CannotPause, new BiasFilter(100));
                c.setRaw(1000);
        ...
        TEST_CASE("sleepUntil rejects MediaTimeStamp from another domain") {
                ...
                MediaTimeStamp wrongDomain(ts, ClockDomain(ClockDomain::SystemMonotonic));
