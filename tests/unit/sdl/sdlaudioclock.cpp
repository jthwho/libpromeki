                ...
                public:
                        TestAudioOutput() {
                                ...
                                _domain = ClockDomain(ClockDomain::registerDomain(
                                        "sdl.audio:test", "Test stand-in for SDL audio clock domain",
                                        ClockEpoch::PerStream));
                        }

                        int64_t totalBytesPushed() const override { return _pushed.load(std::memory_order_relaxed); }

                        int queuedBytes() const override { return _queued.load(std::memory_order_relaxed); }
                        ...
                        bool isPaused() const override { return _paused.load(std::memory_order_relaxed); }

                        void setPushed(int64_t v) { _pushed.store(v, std::memory_order_relaxed); }

                        void setQueued(int v) { _queued.store(v, std::memory_order_relaxed); }
...
TEST_SUITE("SDLAudioClock") {
        ...
        TEST_CASE("nowNs advances smoothly between simulated callbacks") {
                // 48 kHz stereo float -> 384 000 bytes/sec.
                const double bytesPerSec = (double)kSampleRate * kChannels * sizeof(float);
                ...
                const int64_t kPrebufferBytes = (int64_t)(bytesPerSec * (kTestDurationMs + 1000) / 1000);
                ...
                MESSAGE("samples=" << sampleCount << " stepCount=" << stepCount << " maxStallNs=" << maxStallNs
                                   << " minStepNs=" << (stepCount ? minStepNs : 0)
                                   << " avgStepNs=" << (stepCount ? sumStepNs / stepCount : 0)
                                   << " regressions=" << regressions << " maxRegressionNs=" << maxRegressionNs
                                   << " wallSecs=" << wallSecs << " clkSecs=" << clkSecs << " rateRatio=" << rateRatio);
                ...
                CHECK(maxStallNs < (int64_t)(callbackPeriodNs / 4.0));
