...
namespace {
        ...
        void openLog(const char *path) {
                ...
                if (!gLogFile) {
                        std::fprintf(stderr,
                                     "WARN: failed to open log file '%s'; "
        ...
        Img generateSource(const Options &opts) {
                ...
                return gen.createPayload(
                        ImageDesc(Size2Du32(opts.width, opts.height), PixelFormat(PixelFormat::RGBA8_sRGB)));
        }
        ...
        EncodeResult runEncode(const char *codecName, const List<Img> &frames, const MediaConfig &cfg, bool verbose) {
                EncodeResult r;
        ...
        void testBasicEncode(const Options &opts, const Img &src) {
                ...
                for (const auto &c : combos) {
                        ...
                        if (r.packets < 1) {
                                fail(c.label, "no packets produced");
                                continue;
                        }
                        if (!r.firstIsKey) {
                                fail(c.label, "first packet not a keyframe");
                                continue;
                        }
                        if (!r.gotEos) {
                                fail(c.label, "no EOS after flush");
                                continue;
                        }
                        pass(c.label);
                        logf("          %d packets, %d keyframes, %.1f KB\n", r.packets, r.keyframes,
                             r.totalBytes / 1024.0);
                }
        }
        ...
        void testForceKeyframe(const Options &opts, const Img &src) {
                ...
                for (const char *codec : codecs) {
                        ...
                        if (!submitOk) {
                                skip(label.cstr(), "submitFrame failed (runtime?)");
                                continue;
                        }

                        if (packets.size() < numFrames) {
                                fail(label.cstr(),
                                     String::sprintf("expected %d packets, got %d", numFrames, (int)packets.size())
                                             .cstr());
                                continue;
                        }
                        ...
                        if (!packets[forceAt]->isKeyframe()) {
                                fail(label.cstr(),
                                     String::sprintf("frame %d not a keyframe after ForceKeyframe", forceAt).cstr());
                                continue;
                        }
        ...
        void testRateControlModes(const Options &opts, const Img &src) {
                ...
                for (const auto &t : tests) {
                        ...
                        if (!r.ok) {
                                skip(t.label, r.errorMsg.cstr());
                                continue;
                        }
                        if (r.packets < 1) {
                                fail(t.label, "no packets");
                                continue;
                        }
                        pass(t.label);
        ...
        void testGopAndIdr(const Options &opts, const Img &src) {
                ...
                for (const auto &t : tests) {
                        ...
                        if (!submitOk) {
                                skip(t.label, "submitFrame failed");
                                continue;
                        }
                        if (keyCount < t.expectedMinKeys) {
                                fail(t.label, String::sprintf("expected >= %d keyframes, got %d in %d packets",
                                                              t.expectedMinKeys, keyCount, pktCount)
                                                      .cstr());
                                continue;
                        }
                        pass(t.label);
        ...
        void testProfileLevel(const Options &opts, const Img &src) {
                ...
                for (const auto &t : tests) {
                        ...
                        List<Img> frames;
                        for (int i = 0; i < std::min(opts.frames, 10); ++i) frames.pushToBack(converted);

                        MediaConfig cfg;
                        cfg.set(MediaConfig::BitrateKbps, int32_t(10000));
                        cfg.set(MediaConfig::GopLength, int32_t(30));
                        cfg.set(MediaConfig::VideoPreset, VideoEncoderPreset::LowLatency);
                        if (std::strlen(t.profile) > 0) cfg.set(MediaConfig::VideoProfile, String(t.profile));
                        if (std::strlen(t.level) > 0) cfg.set(MediaConfig::VideoLevel, String(t.level));
        ...
        void testBFrames(const Options &opts, const Img &src) {
                ...
                for (const auto &t : tests) {
                        ...
                        if (r.packets < numFrames) {
                                fail(t.label,
                                     String::sprintf("expected >= %d packets, got %d", numFrames, r.packets).cstr());
                                continue;
                        }
                        if (!r.firstIsKey) {
                                fail(t.label, "first packet not a keyframe");
                                continue;
                        }
                        if (!r.gotEos) {
                                fail(t.label, "no EOS after flush");
                                continue;
                        }
                        pass(t.label);
                        logf("          %d packets, %.1f KB\n", r.packets, r.totalBytes / 1024.0);
        ...
        void testLookahead(const Options &opts, const Img &src) {
                ...
                LaTest tests[] = {
                        {"H264", 4, "H264 LA=4"}, {"H264", 8, "H264 LA=8"},   {"H264", 16, "H264 LA=16"},
                        {"HEVC", 8, "HEVC LA=8"}, {"HEVC", 16, "HEVC LA=16"},
                };
        ...
        void testAdaptiveQuantization(const Options &opts, const Img &src) {
                ...
                for (const auto &t : tests) {
                        ...
                        if (t.aqStrength > 0) cfg.set(MediaConfig::VideoSpatialAQStrength, t.aqStrength);
        ...
        void testRepeatHeaders(const Options &opts, const Img &src) {
                ...
                for (const char *codec : codecs) {
                        ...
                        if (!r.ok) {
                                skip(label.cstr(), r.errorMsg.cstr());
                                continue;
                        }
                        if (r.packets < numFrames) {
                                fail(label.cstr(),
        ...
        void testHdrMetadata(const Options &opts, const Img &src) {
                ...
                for (const auto &t : tests) {
                        ...
                        if (!r.firstIsKey) {
                                fail(t.label, "first not keyframe");
                                continue;
                        }
                        pass(t.label);
                        logf("          %d packets, %.1f KB\n", r.packets, r.totalBytes / 1024.0);
        ...
        void testColorDescription(const Options &opts, const Img &src) {
                ...
                Img p010 = convertTo(src, PixelFormat::YUV10_420_SemiPlanar_LE_Rec709);
                if (!nv12.isValid()) {
                        skip("ColorDesc", "NV12 CSC failed");
                        return;
                }
                if (!p010.isValid()) {
                        skip("ColorDesc", "P010 CSC failed");
                        return;
                }
                ...
                ColorTest tests[] = {
                        ...
                        {"H264", PixelFormat::YUV8_420_SemiPlanar_Rec709, ColorPrimaries::Auto,
                         TransferCharacteristics::Auto, MatrixCoefficients::Auto, VideoRange::Unknown,
                         "H264 NV12 Auto (→ BT.709 limited)"},
                        {"HEVC", PixelFormat::YUV8_420_SemiPlanar_Rec709, ColorPrimaries::Auto,
                         TransferCharacteristics::Auto, MatrixCoefficients::Auto, VideoRange::Unknown,
                         "HEVC NV12 Auto (→ BT.709 limited)"},
                        {"AV1", PixelFormat::YUV8_420_SemiPlanar_Rec709, ColorPrimaries::Auto,
                         TransferCharacteristics::Auto, MatrixCoefficients::Auto, VideoRange::Unknown,
                         "AV1  NV12 Auto (→ BT.709 limited)"},
                        // Explicit HDR10 override on P010 — SMPTE2084 + BT.2020.
                        {"HEVC", PixelFormat::YUV10_420_SemiPlanar_LE_Rec709, ColorPrimaries::BT2020,
                         TransferCharacteristics::SMPTE2084, MatrixCoefficients::BT2020_NCL, VideoRange::Limited,
                         "HEVC P010 HDR10 (BT.2020 + PQ)"},
                        {"AV1", PixelFormat::YUV10_420_SemiPlanar_LE_Rec709, ColorPrimaries::BT2020,
                         TransferCharacteristics::SMPTE2084, MatrixCoefficients::BT2020_NCL, VideoRange::Limited,
                         "AV1  P010 HDR10 (BT.2020 + PQ)"},
                        // HLG override on P010.
                        {"HEVC", PixelFormat::YUV10_420_SemiPlanar_LE_Rec709, ColorPrimaries::BT2020,
                         TransferCharacteristics::ARIB_STD_B67, MatrixCoefficients::BT2020_NCL, VideoRange::Limited,
                         "HEVC P010 HLG"},
                        // Explicit full-range override (rare but legal).
                        {"H264", PixelFormat::YUV8_420_SemiPlanar_Rec709, ColorPrimaries::BT709,
                         TransferCharacteristics::BT709, MatrixCoefficients::BT709, VideoRange::Full,
                         "H264 NV12 explicit full-range"},
                        // Unspecified — suppress the color description block.
                        {"H264", PixelFormat::YUV8_420_SemiPlanar_Rec709, ColorPrimaries::Unspecified,
                         TransferCharacteristics::Unspecified, MatrixCoefficients::Unspecified, VideoRange::Unknown,
        ...
        void testTimecode(const Options &opts, const Img &src) {
                ...
                for (const auto &t : tests) {
                        ...
                        if (!submitOk) {
                                skip(t.label, "submitFrame failed (runtime?)");
                                continue;
                        }
                        if (packets < 1) {
                                fail(t.label, "no packets");
        ...
        void testInterlaced(const Options &opts, const Img &src) {
                ...
                Img nv12 = convertTo(src, PixelFormat::YUV8_420_SemiPlanar_Rec709);
                if (!nv12.isValid()) {
                        skip("Interlaced", "NV12 CSC failed");
                        return;
                }
        ...
        // build.
        Metadata encodeDecodeRoundTrip(const char *codecName, const List<Img> &inFrames, const MediaConfig &encCfg,
                                       String &errOut) {
                VideoEncoder *enc = createEncoderByName(codecName);
                if (!enc) {
                        errOut = String::sprintf("no encoder for %s", codecName);
                        return {};
                }
                ...
                if (error(decRes).isError()) {
                        errOut = String::sprintf("createDecoder failed: %s", error(decRes).name().cstr());
                        return {};
                }
        ...
        void testEncodeDecodeRoundTrip(const Options &opts, const Img &src) {
                ...
                Img p010 = convertTo(src, PixelFormat::YUV10_420_SemiPlanar_LE_Rec709);
                if (!nv12.isValid()) {
                        skip("RoundTrip", "NV12 CSC failed");
                        return;
                }
                if (!p010.isValid()) {
                        skip("RoundTrip", "P010 CSC failed");
                        return;
                }
                        ...
                        if (meta.isEmpty()) {
                                ...
                                if (!meta.contains(Metadata::ContentLightLevel)) {
                                        ...
                                        ContentLightLevel cll =
                                                meta.getAs<ContentLightLevel>(Metadata::ContentLightLevel);
                                ...
                                if (ok)
                                        ...
                                        fail(label, "metadata mismatch (see above)");
                        }
                }
                                                ...
                                                if (scan.value() != VideoScanMode::InterlacedEvenFirst.value()) {
                                                        fail(label,
                                                             String::sprintf(
                                                                     ...
                                                                     scan.valueName().cstr(), scan.value())
                                                                     .cstr());
                                                } else {
                                                        pass(label);
                                                }
        ...
        void testUnsupportedFormat(const Options &opts) {
                ...
                Img rgb = UncompressedVideoPayload::allocate(
                        ImageDesc(Size2Du32(256, 128), PixelFormat(PixelFormat::RGB8_sRGB)));
                ...
                if (err == Error::PixelFormatNotSupported) {
                        ...
                        fail("RejectRGB",
                             String::sprintf("expected PixelFormatNotSupported, got %s", err.name().cstr()).cstr());
                }
        }
        ...
        void testSupportedInputsList() {
                ...
                VideoEncoder *enc = createEncoderByName("H264");
                if (!enc) {
                        skip("supportedInputs", "no encoder");
                        return;
                }
