...
namespace {
        ...
        void feedSilence(SDLAudioOutput *out, std::atomic<bool> *running) {
                ...
                while (running->load(std::memory_order_relaxed)) {
                        ...
                        BasicThread::sleepUs(kFeedIntervalMicros);
                }
        }
...
int main(int argc, char **argv) {
        ...
        auto          clockNow = [&clock]() -> int64_t {
                auto r = clock.nowNs();
                if (isError(r)) {
                        promekiErr("clock read failed: %s", error(r).name().cstr());
                        return 0;
                }
                return value(r);
        };
        ...
        while (true) {
                BasicThread::sleepUs(kPollIntervalMicros);
        ...
        if (s.regressions > 0) {
                std::fprintf(stderr,
                             ...
                             (long long)s.regressions, (long long)s.maxRegressionNs,
                             (long long)(s.maxRegressionNs / 1000000LL));
                pass = false;
        }
        ...
        if (wallSecs > 0.0 && (ratio < 1.0 - kMaxAllowedRateDrift || ratio > 1.0 + kMaxAllowedRateDrift)) {
                std::fprintf(stderr, "FAIL: clock/wall ratio %.6f outside [%.3f, %.3f]\n", ratio,
                             1.0 - kMaxAllowedRateDrift, 1.0 + kMaxAllowedRateDrift);
                pass = false;
        }
        if (s.stepCount == 0) {
