...
namespace {
        ...
        void usage() {
                std::fprintf(stderr, "Usage: roundtrip-functest [OPTIONS]\n"
                                     "\n"
        ...
        // for us.
        static List<PixelFormat> nativePixelFormatsFor(const String &backendName, const String &extension) {
                ...
                const String probePath = String("/mnt/data/tmp/promeki/roundtrip_probe.") + extension;
        ...
        static List<Case> filterCases(const List<Case> &cases, const StringList &regexes) {
                ...
                for (size_t i = 0; i < cases.size(); ++i) {
                        ...
                        for (size_t j = 0; j < compiled.size(); ++j) {
                                if (compiled[j].search(cases[i].label)) {
                                        keep = true;
                                        break;
                                }
                        }
        ...
        static MediaPipelineConfig::Stage makeTpgStage(const Options &opts, uint32_t streamId,
        ...
        static MediaPipelineConfig::Stage makeEncoderStage(const VideoCodec &codec, const PixelFormat &variant) {
                MediaPipelineConfig::Stage s;
                s.name = String("enc");
        ...
        static MediaPipelineConfig buildWriteConfig(const Case &c, const String &path, const Options &opts,
                                                    uint32_t streamId) {
        ...
        static PhaseOutcome runPhase(MediaPipeline &pipe, const MediaPipelineConfig &cfg, EventLoop *loop,
        ...
        static RunResult runCase(const Case &c, const Options &opts, uint32_t streamId) {
                RunResult  r;
                ...
                // ---- Write phase ----
                {
                        ...
                        if (p.timedOut) {
                                ...
                                r.message = String("write phase deadlocked past ") +
                                            String::number(opts.phaseTimeoutMs) + String(" ms");
                                return r;
                        }
                        if (p.sawError) {
                                r.status = RunResult::Skip;
                                r.message = String("write pipeline error: ") + p.errorDetail;
                                return r;
                        }
...

                {
                        ...
                        if (!p.built) {
                                r.status = isPlannerGap(p.buildError) ? RunResult::Skip : RunResult::Fail;
                                r.message = String("read build failed: ") + p.buildError.desc();
                                return r;
                        }
                        if (!p.opened) {
                                r.status = isPlannerGap(p.openError) ? RunResult::Skip : RunResult::Fail;
                                r.message = String("read open failed: ") + p.openError.desc();
                                return r;
                        }
                        if (!p.started) {
                                r.status = isPlannerGap(p.startError) ? RunResult::Skip : RunResult::Fail;
                        ...
                        if (p.sawError) {
                                r.status = RunResult::Fail;
                                r.message = String("read pipeline error: ") + p.errorDetail;
                                return r;
                        }
                }
                ...
                if (c.isSequence) {
                        ...
                        if (diskCount > 0 && r.framesProcessed != diskCount) {
                                r.status = RunResult::Fail;
                                r.message = String("inspector read ") + String::number(r.framesProcessed) +
                                            String(" of ") + String::number(diskCount) + String(" files on disk");
                                return r;
                        }
                }
        ...
        static void printCaseResult(const Case &c, const RunResult &r) {
                std::printf("[%4s] %-32s  written=%lld  processed=%lld  pic=%lld  ltc=%lld  disc=%lld",
                            statusWord(r.status), c.label.cstr(), (long long)r.framesWritten,
                            (long long)r.framesProcessed, (long long)r.framesWithPictureData,
                            (long long)r.framesWithLtc, (long long)r.totalDiscontinuities);
...
int main(int argc, char **argv) {
        ...
        if (cases.isEmpty()) {
                std::fprintf(stderr, "Error: no cases matched (registry has %zu candidates; %zu regex filter%s)\n",
                             (size_t)buildCases().size(), (size_t)opts.regexes.size(),
        ...
        std::printf("roundtrip-functest: %zu case%s, %d frame%s each, path=%s\n", (size_t)cases.size(),
                    cases.size() == 1 ? "" : "s", opts.frames, opts.frames == 1 ? "" : "s", opts.path.cstr());
        ...
        for (size_t i = 0; i < cases.size(); ++i) {
                ...
                promekiInfo("=== BEGIN case %zu / %zu : %s ===", i + 1, (size_t)cases.size(), c.label.cstr());
                RunResult r = runCase(c, opts, streamId++);
                promekiInfo("=== END   case %zu / %zu : %s -> %s%s%s ===", i + 1, (size_t)cases.size(), c.label.cstr(),
                            statusWord(r.status), r.message.isEmpty() ? "" : " : ", r.message.cstr());
                printCaseResult(c, r);
                switch (r.status) {
        ...
        std::printf("\nSummary: %d passed, %d failed, %d skipped, %d timed out of %zu case%s\n", passed, failed,
                    skipped, timedOut, (size_t)cases.size(), cases.size() == 1 ? "" : "s");
