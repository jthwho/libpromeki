...
TEST_CASE("TuiPainter: drawChar") {
        ...
        CHECK(cell.ch == Char(U'X'));
        CHECK(cell.style.foreground() == Color::Red);
}
...
TEST_CASE("TuiPainter: drawText") {
        ...
        CHECK(screen.cell(0, 0).ch == Char(U'H'));
        CHECK(screen.cell(1, 0).ch == Char(U'e'));
        CHECK(screen.cell(2, 0).ch == Char(U'l'));
        CHECK(screen.cell(3, 0).ch == Char(U'l'));
        CHECK(screen.cell(4, 0).ch == Char(U'o'));
}
...
TEST_CASE("TuiPainter: fillRect") {
        ...
        CHECK(screen.cell(2, 2).ch == Char(U'#'));
        CHECK(screen.cell(4, 4).ch == Char(U'#'));
        CHECK(screen.cell(2, 2).style.foreground() == Color::Green);
        CHECK(screen.cell(2, 2).style.background() == Color::Blue);
}
...
TEST_CASE("TuiPainter: clipping") {
        ...
        CHECK(screen.cell(2, 2).ch == Char(U'A'));
...
TEST_CASE("TuiPainter: style flags") {
        ...
        CHECK(cell.style.attrs() == (TuiStyle::Bold | TuiStyle::Underline));
}
