...
void printBuildWarnings() {
        ...
        if(!libOk) {
                if(libType.isEmpty()) {
                        std::printf(
                                "  !! libpromeki build type is unset — results will likely not\n"
                                "  !! reflect release performance.  Reconfigure with\n"
                                "  !! -DCMAKE_BUILD_TYPE=Release before benchmarking.\n");
                } else {
                        std::printf(
                                "  !! libpromeki was built as '%s' — results will not reflect\n"
        ...
        if(!benchOk) {
                std::printf(
                        "  !! promeki-bench was compiled without NDEBUG — the driver itself\n"
...
int main(int argc, char **argv) {
        ...
        parser.registerOptions({
                ...
                {'l', "list",
                 "List every registered benchmark case and exit",
                 CmdLineParser::OptionCallback([&]() { listBenches = true; return 0; })},
                {'f', "filter",
                 "Regex (ECMAScript) matched against 'suite.name'",
                 CmdLineParser::OptionStringCallback(
                        [&](const String &s) { filter = s; return 0; })},
                {'o', "output",
                 "Write full JSON results to this file",
                 CmdLineParser::OptionStringCallback(
                        [&](const String &s) { outputPath = s; return 0; })},
                {'b', "baseline",
                 "Compare results against a previously-saved JSON baseline",
                 CmdLineParser::OptionStringCallback(
                        [&](const String &s) { baselinePath = s; return 0; })},
                {'t', "min-time",
                 "Target measurement window per case in ms (default: 500)",
                 CmdLineParser::OptionIntCallback([&](int v) {
                        if(v > 0) minTimeMs = static_cast<unsigned int>(v);
                        return 0;
                 })},
                {'W', "warmup",
                 "Warmup window per case in ms (default: 100)",
                 CmdLineParser::OptionIntCallback([&](int v) {
                        if(v > 0) warmupMs = static_cast<unsigned int>(v);
                        return 0;
                 })},
                {'r', "repeats",
                 "Number of measurement runs per case (default: 1)",
                 CmdLineParser::OptionIntCallback([&](int v) {
                ...
                {'p', "param",
                 "Set a case parameter (key=value, key+=value, or bare key)",
                 CmdLineParser::OptionStringCallback([&](const String &s) {
                        Error e = benchParams().parseArg(s);
                        if(e.isError()) {
                                std::fprintf(stderr,
                                        "promeki-bench: invalid -p argument '%s': %s\n",
                                        s.cstr(), e.name().cstr());
                                return 1;
                        }
                        return 0;
        ...
        String estimate     = Duration::fromMilliseconds(
                                  runner.estimatedDurationMs()).toString();

        const BuildInfo *bi = getBuildInfo();
        std::printf("promeki-bench\n");
        if(bi && bi->repoident) std::printf("  build:      %s\n", bi->repoident);
        {
                String libType = (bi && bi->type && bi->type[0])
                        ? String(bi->type)
                        : String("(unset)");
        ...
        if(runErr.isError()) {
                std::fprintf(stderr,
                        "promeki-bench: runAll returned %s\n",
                        runErr.name().cstr());
        }
        ...
        if(!baselinePath.isEmpty()) {
                ...
                List<BenchmarkResult> base =
                        BenchmarkRunner::loadBaseline(baselinePath, &baseErr);
                if(baseErr.isError()) {
                        std::fprintf(stderr,
                                "promeki-bench: could not load baseline '%s': %s\n",
                                baselinePath.cstr(), baseErr.name().cstr());
                } else {
        ...
        if(!outputPath.isEmpty()) {
                ...
                if(wErr.isError()) {
                        std::fprintf(stderr,
                                "promeki-bench: failed to write '%s': %s\n",
                                outputPath.cstr(), wErr.name().cstr());
                        return 1;
                }
