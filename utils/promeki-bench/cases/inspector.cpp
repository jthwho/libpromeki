...
namespace benchutil {
        ...
        namespace {
                ...
                PixelFormat::ID resolveInspectorFormat(const String &name) {
                        ...
                        std::fprintf(stderr, "promeki-bench: inspector: unknown PixelFormat '%s'\n", name.cstr());
 ...
 */
                BenchmarkCase::Function buildPipelineCase(PipelineSpec spec) {
                        return [spec](BenchmarkState &state) {
                                ...
                                const size_t bytesPerIter = static_cast<size_t>(spec.width) *
                                                            static_cast<size_t>(spec.height) *
                                                            4; // assume 4 bytes/pixel for the upper bound
        ...
        void registerInspectorCases() {
                ...
                for (const auto &spec : resolveSpecs()) {
                        String name = String("pipeline_") + pipelineSizeLabel(spec) + "_" + PixelFormat(spec.pd).name();
                        ...
                        BenchmarkRunner::registerCase(BenchmarkCase(suite, name, desc, buildPipelineCase(spec)));
                }
        }

        String inspectorParamHelp() {
                return String("inspector suite parameters:\n"
...
namespace benchutil {
        ...
        String inspectorParamHelp() {
                return String("inspector suite parameters: (disabled — built without PROMEKI_ENABLE_CSC)\n");
        }
