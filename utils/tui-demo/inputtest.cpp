                ...
                if(ret > 0 && (pfd.revents & POLLIN)) {
                        ...
                        if(n > 0) {
                                ...
                                for(ssize_t i = 0; i < n; i++) {
                                        pos += snprintf(out + pos, sizeof(out) - pos,
                                                "0x%02X ", (unsigned char)buf[i]);
                                }
                                // Also show printable interpretation
                                pos += snprintf(out + pos, sizeof(out) - pos, " [");
                                for(ssize_t i = 0; i < n; i++) {
                                        char c = buf[i];
                                        if(c == 0x09) pos += snprintf(out + pos, sizeof(out) - pos, "TAB");
                                        else if(c == 0x0D) pos += snprintf(out + pos, sizeof(out) - pos, "CR");
                                        else if(c == 0x0A) pos += snprintf(out + pos, sizeof(out) - pos, "LF");
                                        else if(c == 0x1B) pos += snprintf(out + pos, sizeof(out) - pos, "ESC");
                                        else if(c >= 1 && c <= 26) pos += snprintf(out + pos, sizeof(out) - pos, "^%c", c + 'A' - 1);
                                        else if(c >= 32 && c < 127) pos += snprintf(out + pos, sizeof(out) - pos, "%c", c);
                                        else pos += snprintf(out + pos, sizeof(out) - pos, "?");
