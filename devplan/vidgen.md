...
1. **MediaPort** — connection point data object
2. **MediaNode** — base processing node (including error reporting and statistics — see below)
3. **MediaLink** — buffered port-to-port connection
4. **MediaGraph** — DAG of connected nodes, topology validation
5. **MediaPipeline** — owns graph + thread pool, orchestrates processing

### MediaNode Error Reporting
...
- [ ] `struct NodeMessage` — structured message from a node:
  - [ ] `Severity severity`
  - [ ] `String message`
  - [ ] `uint64_t frameNumber` — which frame the error relates to (0 if not frame-specific)
  - [ ] `TimeStamp timestamp`
- [ ] `PROMEKI_SIGNAL(messageEmitted, NodeMessage)` — nodes emit messages via this signal. Pipeline connects to it for centralized logging/handling.
- [ ] Protected: `void emitMessage(Severity severity, const String &message)` — convenience for subclasses to emit messages with auto-populated frame number and timestamp
- [ ] Protected: `void emitWarning(const String &message)` — shorthand for Warning severity
- [ ] Protected: `void emitError(const String &message)` — shorthand for Error severity. Also transitions node to Error state and emits `errorOccurred`.
- [ ] Fatal messages propagate to MediaPipeline, which stops the pipeline.
...
### MediaNode Statistics

Base-level statistics common to all nodes. Avoids ad-hoc per-node counters.

**Additions to MediaNode:**
- [ ] `struct NodeStats` — snapshot of node performance:
  - [ ] `uint64_t processCount` — total process() invocations
  - [ ] `uint64_t starvationCount` — total starvation() invocations
  - [ ] `double lastProcessDuration` — wall-clock time of last process() call (seconds)
  - [ ] `double avgProcessDuration` — exponential moving average of process() duration
  - [ ] `double peakProcessDuration` — peak process() duration
  - [ ] `int currentQueueDepth` — current input queue depth
  - [ ] `int peakQueueDepth` — peak input queue depth observed
- [ ] `NodeStats stats() const` — return current statistics
- [ ] `void resetStats()` — reset all counters
- [ ] Pipeline automatically wraps `process()` calls with timing instrumentation
- [ ] Individual nodes can extend with node-specific stats via `virtual Map<String, Variant> extendedStats() const` — returns additional stats as key-value pairs (e.g., RtpVideoSinkNode adds "packetsSent", "bytesSent")
...
**Implementation checklist (uncomment and implement in Impl + PaintEngine facade):**
- [ ] `drawRect(const Pixel &pixel, const Rect2Di32 &rect)` — outline rectangle
- [ ] `fillRect(const Pixel &pixel, const Rect2Di32 &rect)` — filled rectangle (critical for color bars)
- [ ] `drawCircle(const Pixel &pixel, const Point2Di32 &center, int radius)` — outline circle
- [ ] `fillCircle(const Pixel &pixel, const Point2Di32 &center, int radius)` — filled circle
- [ ] `drawEllipse(const Pixel &pixel, const Point2Di32 &center, const Size2Du32 &size)` — outline ellipse
- [ ] `fillEllipse(const Pixel &pixel, const Point2Di32 &center, const Size2Du32 &size)` — filled ellipse
- [ ] Implement in each existing pixel-format-specific Impl subclass (RGBA8, RGB8, JPEG, etc.)
- [ ] Update existing doctest with tests for new methods

---

### 3B. TimecodeGenerator
...
- [ ] `src/timecodegenerator.cpp`
- [ ] `tests/timecodegenerator.cpp`

**Implementation checklist:**
- [ ] Header guard, includes, namespace
- [ ] Simple data object (no `PROMEKI_SHARED_FINAL` — cheap to copy, no shared state)
- [ ] `enum RunMode { Still, Forward, Reverse }`
- [ ] Constructor: `TimecodeGenerator()` — default (mode derived later from frame rate)
- [ ] Constructor: `TimecodeGenerator(const Rational &frameRate, bool dropFrame = false)` — sets mode from rate

**Frame rate / mode configuration:**
- [ ] `void setFrameRate(const Rational &frameRate)` — sets TC mode automatically from rate + dropFrame flag
- [ ] `Rational frameRate() const`
- [ ] `void setDropFrame(bool df)` — only takes effect when frame rate is 30000/1001. At all other rates, dropFrame is forced to false. Recalculates mode.
- [ ] `bool dropFrame() const`
- [ ] `Timecode::Mode timecodeMode() const` — the resolved mode (read-only, derived from frameRate + dropFrame)
- [ ] Mode derivation rules (default is always non-drop-frame):
  - [ ] 23.976/24 fps → NDF24
  - [ ] 25 fps → NDF25
  - [ ] 29.97 fps (30000/1001) → NDF30 (default), DF30 only if `setDropFrame(true)` was called
  - [ ] 30 fps → NDF30
  - [ ] Other rates → custom mode via libvtc `vtc_format_find_or_create()`

**Run control:**
- [ ] `void setRunMode(RunMode mode)` — Still, Forward, or Reverse. Default: Forward.
- [ ] `RunMode runMode() const`

**Timecode state:**
- [ ] `void setTimecode(const Timecode &tc)` — set current timecode (initial value or jam)
- [ ] `Timecode timecode() const` — current timecode value
- [ ] `void jam(const Timecode &tc)` — jam to a new value (equivalent to setTimecode, but semantically distinct: use for mid-run resync)
- [ ] `Timecode advance()` — advance one frame according to runMode and return the **previous** value (the value for the current frame). Forward increments, Reverse decrements, Still returns the same value repeatedly.
- [ ] `uint64_t frameCount() const` — total frames advanced (counts advance() calls)
- [ ] `void reset()` — reset frameCount to 0 and timecode to the value set by setTimecode

**Doctest:**
- [ ] Default construction, verify invalid/default state
- [ ] Set frame rate → verify correct mode derivation for each standard rate
- [ ] Forward mode: advance N frames, verify TC increments correctly (including DF skip at 29.97)
- [ ] Reverse mode: advance N frames, verify TC decrements
- [ ] Still mode: advance N frames, verify TC stays the same
- [ ] Jam: run forward, jam to new value, verify TC continues from jammed value
- [ ] Reset: verify frameCount resets and TC returns to start
- [ ] Drop frame at 29.97: verify frames 0 and 1 are skipped at minute boundaries (except 10-minute marks)
- [ ] Non-29.97 with dropFrame=true: verify dropFrame is forced to false (mode stays NDF)
- [ ] Default at 29.97 without setDropFrame: verify NDF30 (not DF30)
...
- [ ] `src/ltcencoder.cpp`
- [ ] `tests/ltcencoder.cpp`

**Implementation checklist:**
- [ ] Header guard, includes, namespace
- [ ] Simple utility class (not ObjectBase, not shareable — owns encoder state, not copyable, movable)

**Configuration:**
- [ ] Constructor: `LtcEncoder(int sampleRate, float level = 0.5f)`
- [ ] `int sampleRate() const`
- [ ] `void setLevel(float level)` — output amplitude 0.0–1.0
- [ ] `float level() const`

**Encoding:**
- [ ] `Audio encode(const Timecode &tc)` — encode one timecode frame, return Audio containing the LTC samples for that frame. Sample count varies slightly per frame (e.g., ~2002 samples at 48kHz/24fps). Output is mono.
- [ ] `Audio encode(const Timecode &tc, const AudioDesc &desc)` — encode and convert to target AudioDesc (sample format, optionally duplicate to multiple channels)
- [ ] `size_t frameSizeApprox() const` — approximate samples per LTC frame (for buffer pre-allocation)

**Internal:**
- [ ] Wraps `VtcLTCEncoder` struct
- [ ] `vtc_ltc_encoder_init()` called in constructor
- [ ] `vtc_ltc_audio_encode()` called in encode()
- [ ] Converts libvtc's `int8_t` output to the target sample format (float32, int16, etc.)

**Doctest:**
- [ ] Encode a known timecode, verify output sample count is reasonable for the sample rate
- [ ] Encode at different sample rates (44100, 48000, 96000), verify proportional sample counts
- [ ] Verify output level: peak sample values should match configured level
- [ ] Encode sequential timecodes, feed output to LtcDecoder, verify round-trip
...
- [ ] `src/ltcdecoder.cpp`
- [ ] `tests/ltcdecoder.cpp`

**Implementation checklist:**
- [ ] Header guard, includes, namespace
- [ ] Simple utility class (not ObjectBase — but uses a callback mechanism for decoded TC delivery). Not copyable, movable.

**Configuration:**
- [ ] Constructor: `LtcDecoder(int sampleRate)`
- [ ] `int sampleRate() const`
- [ ] `void setThresholds(int8_t lower, int8_t upper)` — hysteresis thresholds for edge detection (default: ±3)
- [ ] `void setFuzz(int fuzz)` — timing tolerance in samples (default: 3)

**Decoding:**
- [ ] `struct DecodedTimecode` — result struct:
  - [ ] `Timecode timecode` — the decoded value
  - [ ] `int64_t sampleStart` — sample position where this frame began
  - [ ] `int64_t sampleLength` — number of samples in this frame
  - [ ] `bool forward` — true if LTC was running forward, false if reverse
- [ ] `using DecodedList = List<DecodedTimecode>`
- [ ] `DecodedList decode(const Audio &audio)` — feed audio samples, return any timecodes decoded from this chunk. May return 0, 1, or multiple results depending on how much audio is provided.
- [ ] `DecodedList decode(const int8_t *samples, size_t count)` — low-level: feed raw int8_t samples directly
- [ ] `void reset()` — clear decoder state (e.g., after seeking or switching sources)

**Internal:**
- [ ] Wraps `VtcLTCDecoder` struct
- [ ] Uses `vtc_ltc_decoder_init()` with internal static callback
- [ ] Callback accumulates results into a thread-local or member `DecodedList` during `decode()` calls
- [ ] Converts input Audio samples to `int8_t` range before feeding to libvtc

**Doctest:**
- [ ] Encode→Decode round-trip: encode a sequence of timecodes, decode the audio, verify all values match
- [ ] Verify forward/reverse detection
- [ ] Verify sampleStart/sampleLength are consistent
- [ ] Feed audio in small chunks, verify decoder still finds timecodes across chunk boundaries
- [ ] Reset: verify decoder state is cleared
...
- [ ] `src/testpatternnode.cpp`
- [ ] `tests/testpatternnode.cpp`

**Implementation checklist:**
- [ ] Derive from `MediaNode`, use `PROMEKI_OBJECT`
- [ ] Constructor: creates one Frame output port (carries image + audio + metadata)

**Video pattern configuration:**
- [ ] `enum Pattern { ColorBars, ColorBars75, Ramp, Grid, Crosshatch, Checkerboard, SolidColor, White, Black, Noise, ZonePlate }`
- [ ] `void setPattern(Pattern p)`
- [ ] `Pattern pattern() const`
- [ ] `void setVideoDesc(const VideoDesc &desc)` — frame rate, resolution, pixel format
- [ ] `void setSolidColor(uint16_t r, uint16_t g, uint16_t b)` — for SolidColor pattern

**Motion configuration:**
- [ ] `void setMotion(double speed)` — motion speed and direction. 0.0 = static (default), positive = forward, negative = reverse. Magnitude controls speed (1.0 = one pattern-width per second).
- [ ] `double motion() const`
- [ ] Motion behavior per pattern (direction is pattern-specific):
  - [ ] **ColorBars / ColorBars75** — horizontal scroll (positive = left, negative = right)
  - [ ] **Ramp** — horizontal scroll
  - [ ] **Grid / Crosshatch** — diagonal scroll
  - [ ] **Checkerboard** — diagonal scroll
  - [ ] **ZonePlate** — phase rotation (positive = clockwise, negative = counter-clockwise)
  - [ ] **Noise** — always animated (new random values each frame regardless of motion value)
  - [ ] **SolidColor / White / Black** — motion has no effect (static by nature)
- [ ] Internal: `_motionOffset` accumulator (double, subpixel precision), advanced by `motion * pixelsPerFrame` each process() cycle. Wraps at pattern period for seamless looping.

**Timecode configuration (delegates to internal TimecodeGenerator):**
- [ ] `TimecodeGenerator &timecodeGenerator()` — direct access to the internal generator for full control
- [ ] `const TimecodeGenerator &timecodeGenerator() const`
- [ ] `void setStartTimecode(const Timecode &tc)` — convenience, calls `timecodeGenerator().setTimecode(tc)`. Default: 01:00:00:00.
- [ ] `void setDropFrame(bool df)` — convenience, calls `timecodeGenerator().setDropFrame(df)`. Default: false (NDF). Only takes effect at 30000/1001 frame rate.
- [ ] `Timecode currentTimecode() const` — convenience, calls `timecodeGenerator().timecode()`
- [ ] `uint64_t frameCount() const` — total frames generated
- [ ] TimecodeGenerator's frame rate is set automatically from VideoDesc during `configure()`

**Audio configuration:**
- [ ] `void setAudioDesc(const AudioDesc &desc)` — sample rate, channels, format
- [ ] `void setAudioEnabled(bool enable)` — default: true
- [ ] `bool audioEnabled() const`
- [ ] `enum AudioMode { Tone, Silence, LTC }`
- [ ] `void setAudioMode(AudioMode mode)` — default: Tone
- [ ] `AudioMode audioMode() const`
- [ ] `void setChannelConfig(size_t chan, AudioGen::Config config)` — per-channel signal config (Tone mode)
- [ ] `void setToneFrequency(double hz)` — convenience: all channels to sine at given freq
- [ ] `void setToneAmplitude(double amplitude)` — convenience: all channels
- [ ] `void setLtcLevel(float level)` — LTC output amplitude 0.0–1.0 (default: 0.5)
- [ ] `void setLtcChannel(size_t chan)` — which channel carries LTC (default: 0). Other channels are silent. Set to -1 for all channels.
- [ ] `void setSamplesPerFrame(size_t samples)` — samples per process cycle (default: computed from video frame rate and audio sample rate, e.g., 48000/29.97 = 1601.6 → alternating 1601/1602). **TODO:** Investigate behavior when overridden in LTC mode — LtcEncoder determines its own sample count per frame. Override may cause sample count mismatch. Needs investigation before implementation.

**Lifecycle overrides:**
- [ ] Override `configure()`:
  - [ ] Validate VideoDesc and AudioDesc
  - [ ] Set output port VideoDesc and AudioDesc
  - [ ] Set TimecodeGenerator frame rate from VideoDesc
  - [ ] Create internal `AudioGen` (Tone/Silence modes) or `LtcEncoder` (LTC mode) based on audioMode
  - [ ] Pre-render initial pattern into template Image (for static patterns)
  - [ ] Initialize motion offset to 0
  - [ ] Compute audio samples-per-frame from video rate and audio rate (Tone/Silence modes)
- [ ] Override `process()`:
  - [ ] **Timecode:** Call `TimecodeGenerator::advance()` to get current frame's TC
  - [ ] **Video:** Render current frame (from template + motion offset, or generate fresh for Noise/ZonePlate)
  - [ ] **Audio:** Based on audioMode:
    - [ ] Tone/Silence: Call `AudioGen::generate()` for this frame's sample count
    - [ ] LTC: Call `LtcEncoder::encode()` with current TC, place on configured channel(s)
  - [ ] **Metadata:** Stamp timecode, frame number, generate timestamp. Metadata is set on both the Image and Audio objects within the Frame, ensuring downstream consumers have access regardless of how the Frame is later split.
  - [ ] Create `Frame::Ptr` with Image + Audio + Metadata
  - [ ] Push Frame to output port
  - [ ] Advance motion offset
  - [ ] Source node pacing: gated by downstream back-pressure. Does NOT enforce real-time timing — that's the responsibility of the sink node (e.g., RtpVideoSinkNode).
- [ ] Override `stop()`: reset TimecodeGenerator and motion state

**Extended stats (via `extendedStats()`):**
- [ ] `"framesGenerated"` — total frames produced
- [ ] `"currentTimecode"` — current TC as string

**Internal pattern generators (private methods):**
- [ ] `renderPattern(double motionOffset)` — dispatches to specific generator with current offset
- [ ] `renderColorBars(double offset)` — SMPTE RP 219-2002 HD color bars (100% or 75%), offset scrolls horizontally
- [ ] `renderRamp(double offset)` — luminance gradient, offset shifts ramp start position
- [ ] `renderGrid(double offset)` — white grid lines on black, offset shifts grid position
- [ ] `renderCrosshatch(double offset)` — diagonal crosshatch lines, offset shifts
- [ ] `renderCheckerboard(double offset)` — alternating squares, offset shifts
- [ ] `renderZonePlate(double phase)` — circular zone plate (Siemens star variant), phase rotates the pattern
- [ ] `renderNoise()` — random pixel values (ignores offset, always fresh)
- [ ] `renderSolid()` — solid fill (ignores offset)
- [ ] All generators use `PaintEngine` for drawing operations

**Doctest:**
- [ ] Generate one frame of each pattern, verify image dimensions and pixel format
- [ ] Generate multiple frames with motion != 0, verify images differ between frames
- [ ] Verify timecode increments correctly across frames
- [ ] Verify audio output sample count matches expected samples-per-frame (Tone mode)
- [ ] Verify LTC mode: decode output audio with LtcDecoder, verify TC values match TimecodeGenerator sequence
- [ ] Verify frame count tracks correctly
- [ ] Verify static patterns (Solid/White/Black) are identical frame-to-frame regardless of motion setting
...
- [ ] `src/framedemuxnode.cpp`
- [ ] `tests/framedemuxnode.cpp`

**Implementation checklist:**
- [ ] Derive from `MediaNode`, use `PROMEKI_OBJECT`
- [ ] Constructor: one Frame input port, one Image output port, one Audio output port
- [ ] Override `configure()`:
  - [ ] Extract ImageDesc from input Frame port's VideoDesc, set on Image output port
  - [ ] Extract AudioDesc from input Frame port, set on Audio output port
- [ ] Override `process()`:
  - [ ] Pull Frame from input
  - [ ] Extract Image from Frame, copy Frame's Metadata to Image's Metadata (timecode, frame number, timestamp)
  - [ ] Extract Audio from Frame, copy Frame's Metadata to Audio's Metadata
  - [ ] Push Image to image output port
  - [ ] Push Audio to audio output port
  - [ ] If Frame has no audio (audio disabled), only push to image output
- [ ] Doctest: split a Frame, verify Image and Audio have correct data and metadata
...
| `Queue` | MediaLink | Thread-safe buffering |
