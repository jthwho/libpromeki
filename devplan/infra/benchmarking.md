...
- `MediaIO::ParamBenchmarkReport` and `MediaIO::ParamBenchmarkReset` — base-class param ids; call `sendParams(MediaIOParams().get(ParamBenchmarkReport))` (resp. `.get(ParamBenchmarkReset)`) before strand submit (no queuing behind real work). `BenchmarkReport` fills the action's value with the reporter's `summaryReport()` string; `BenchmarkReset` clears the reporter's accumulators. Both resolve `Error::NotSupported` when no reporter is attached.
