...
The driver lives under `utils/promeki-bench/` and is the single binary that drives every microbench suite. Currently registered: **CSC suite only** (programmatically generated from `PixelDesc::registeredIDs()`). Every other suite from the original plan is still pending — each lands as a new `cases/<suite>.cpp` with a `registerCases()` hook called from `main.cpp::registerAllSuites()`.
...
- [ ] `MediaIOTask_Converter` — `ConvertBegin` / `CscDone` / `CodecDone` / `ConvertEnd` to isolate CSC cost from codec cost inside a single converter stage.
- [ ] `MediaIOTask_Rtp` writer — `PacketizeBegin` / `PacketizeEnd` / `SendBegin` / `SendEnd` to isolate packetization cost from socket send cost.
- [ ] `MediaIOTask_QuickTime` reader — `DemuxBegin` / `DemuxEnd` / `DecodeBegin` / `DecodeEnd` to isolate demux from codec decode.
...
**Blocked on `MediaPipeline::build()`** (`devplan/proav_pipeline.md`). Listed here so the plan is complete.
...
2. **`promeki-bench` binary + CSC case migration** — COMPLETE (CSC suite registered via programmatic `PixelDesc::registeredIDs()` walk rather than a hand-curated pair list)
3. **Initial non-MediaIO case suites** — `network.cpp` is the highest-leverage next item (covers the `proav_optimization.md` wishlist). Then `concurrency.cpp`, `container.cpp`, `histogram.cpp`, `variantdatabase.cpp`, `codec.cpp`. Incremental; each suite is its own small commit.
4. **`MediaConfig::EnableBenchmark` + base-class stamp hooks (Part C)** — COMPLETE
5. **MediaIO identifiers (`localId` + `Name` + `UUID`)** — COMPLETE (absorbed `devplan/ideas.md#1`; the thread-naming half of that idea — backends spawning threads outside the shared pool naming them via `name()` — is still pending in individual backends)
