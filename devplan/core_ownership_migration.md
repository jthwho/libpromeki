...
**Problem:** `AudioFile::Impl` and `QuickTime::Impl` (plus its `QuickTimeReader` / `QuickTimeWriter` subclasses) are held inside their owning classes via `SharedPtr<Impl>` with copy-on-write enabled.  `PROMEKI_SHARED` emits a `_promeki_clone()` method whose default body is `return new Impl(*this);` — so the Impl's copy constructor must be callable at template instantiation time.  Adding a `UniquePtr<IODevice>` / `UniquePtr<File>` member deletes the copy constructor and breaks the clone.
...
- **(a) Disable CoW on the holder.**  Change `SharedPtr<Impl> d;` to `SharedPtr<Impl, /*CopyOnWrite=*/false> d;` in `AudioFile` and `QuickTime`.  With CoW disabled, `_promeki_clone()` is never called from the SharedPtr path, and the copy constructor no longer needs to compile.  (It still needs to be present in the class body because `PROMEKI_SHARED` always defines `_promeki_clone`, but the subclass copy constructor can be `= delete` since it's never invoked.)  Effort: low.  Risk: need to verify no caller relies on CoW detach (every `d.modify()->foo()` call site becomes a plain "modify in place" — which is likely the intended behavior anyway, since opening an audio file is state-ful).
