...
**Phase 4A mostly complete** — MediaSink, MediaSource, MediaNodeConfig, MediaPipelineConfig, MediaNode, MediaPipeline, EncodedDesc, Image::ensureExclusive()/isExclusive(), Audio::convertTo() all done. Remaining: Audio::ensureExclusive()/isExclusive(), MemSpace::Stats, MemSpacePool. **Phase 4B in progress** — MediaIO generic media I/O framework (abstract base, config-driven factory including `defaultConfig()`, backend registry via PROMEKI_REGISTER_MEDIAIO) and MediaIO_TPG (test pattern generator backend: video/audio/timecode, static pattern caching, motion, image-level TC metadata, solid color via single `Color` config key) complete; TestPatternNode refactored to delegate entirely to MediaIO_TPG (solid color now configured via `SolidColor` Color key); VideoTestPattern `setSolidColor()` API refactored to accept a single `Color` object (replacing three uint16_t args); 18 MediaIO test cases; vidgen nodes (TestPatternNode, TimecodeOverlayNode, JpegEncoderNode, FrameDemuxNode, RtpVideoSinkNode, RtpAudioSinkNode) complete; image file I/O backends complete (DPX read+write with embedded audio+metadata+DIO, Cineon read-only, TGA read+write, SGI read+write with RLE, PNM read+write); ImageFile refactored to carry Frame (image+audio+metadata); file I/O nodes, mixer, gain, color space, frame sync remaining. **Phase 4C (DSP) not started.** **Phase 4D partially complete** — Font rendering system (Font/FastFont/BasicFont) complete; codec system (ImageCodec/AudioCodec/JpegImageCodec encode+decode for RGB, RGBA, YUYV, UYVY, planar 4:2:2/4:2:0, NV12; VideoTestPattern, AudioTestPattern) complete; PixelFormat/PixelDesc expanded with full YCbCr format matrix (UYVY/YUYV interleaved, planar 4:2:2/4:2:0, NV12, v210, chroma siting) and RawYUV file I/O; CSC pack/unpack bugs fixed (ARGB8 double-remapping, DPX 10-bit BE paths); userspace packet pacing done; automatic node processing and kernel-level pacing (`sendmmsg`/`SO_MAX_PACING_RATE`/`SO_TXTIME`) remaining. See `proav_optimization.md`.
