...
/**
 * @brief MediaIO backend that performs color space conversion.
 * @ingroup proav
 *
 ...
 * rejected — use @ref VideoEncoderMediaIO /
 * @ref VideoDecoderMediaIO for codec transitions.
 *
 * @par Mode support
 *
 ...
 */
class CscMediaIO : public SharedThreadMediaIO {
                PROMEKI_OBJECT(CscMediaIO, SharedThreadMediaIO)
        public:
                ...
                CscMediaIO(ObjectBase *parent = nullptr);
                ~CscMediaIO() override;
        ...
        private:
                ...
                Error convertFrame(const Frame &input, Frame &output);
...
/**
 * @brief @ref MediaIOFactory for the CSC (color space converter) backend.
 * @ingroup proav
 */
class CscFactory : public MediaIOFactory {
        public:
                CscFactory() = default;

                String name() const override { return String("CSC"); }
                String displayName() const override { return String("Color Space Converter"); }
                String description() const override {
                        return String("Color space converter (uncompressed pixel format conversion)");
                }
