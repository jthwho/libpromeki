 ...
 * | @c MediaConfig::AudioMode            | Enum @ref AudioPattern | AvSync | Audio pattern selector. |
 ...
 * | @c MediaConfig::AudioLtcChannel      | int    | 0      | LTC channel (-1 = all). |
 *
 ...
 */
class TpgMediaIO : public SharedThreadMediaIO {
                PROMEKI_OBJECT(TpgMediaIO, SharedThreadMediaIO)
        public:
                ...
                TpgMediaIO(ObjectBase *parent = nullptr);
...
/**
 * @brief @ref MediaIOFactory for the TPG generator backend.
 * @ingroup proav
 */
class TpgFactory : public MediaIOFactory {
        public:
                TpgFactory() = default;

                String name() const override { return String("TPG"); }
                String displayName() const override { return String("Test Pattern Generator"); }
                String description() const override {
                        return String("Video/audio/timecode test pattern generator");
                }
