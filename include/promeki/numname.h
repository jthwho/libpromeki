...
class NumName {
        public:
                 ...
                 */
                String name(int val) const { return px + String::number(val, 10, pad ? dl : 0, '0') + sx; }
                 ...
                 */
                String hashmask() const { return px + String(pad ? dl : 1, '#') + sx; }

                /** @brief Returns true if both NumNames have identical components. */
                bool operator==(const NumName &other) const {
                        return dl == other.dl && pad == other.pad && px == other.px && sx == other.sx;
                }

                /** @brief Returns true if the NumNames differ in any component. */
                bool operator!=(const NumName &other) const { return !(*this == other); }
