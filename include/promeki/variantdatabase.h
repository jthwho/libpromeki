 ...
 * @endcode
 */
template <CompiledString Name> class VariantDatabase {
        public:
                 ...
                 */
                static const VariantSpec *spec(ID id) { return specRegistry().find(id.id()); }
                 ...
                 */
                static Map<uint64_t, VariantSpec> registeredSpecs() { return specRegistry().all(); }
                ...
                static VariantDatabase fromJson(const JsonObject &json) {
                        VariantDatabase db;
                        json.forEach([&db](const String &key, const Variant &val) { db.setFromJson(ID(key), val); });
                        return db;
                }
                 ...
                 * @return     The substituted output.
                 */
                String format(const String &tmpl, Error *err = nullptr) const { return format(tmpl, nullptr, err); }
                ...
                void merge(const VariantDatabase &other) {
                        other.forEach([this](ID id, const Variant &val) { set(id, val); });
                }
 ...
 */
template <CompiledString Name> DataStream &operator<<(DataStream &stream, const VariantDatabase<Name> &db) {
        db.writeTo(stream);
        return stream;
}
 ...
 * @return A reference to the stream.
 */
template <CompiledString Name> DataStream &operator>>(DataStream &stream, VariantDatabase<Name> &db) {
        db.readFrom(stream);
        return stream;
}
 ...
 * @return A reference to the stream.
 */
template <CompiledString Name> TextStream &operator<<(TextStream &stream, const VariantDatabase<Name> &db) {
        db.writeTo(stream);
        return stream;
}
