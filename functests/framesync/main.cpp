...
void usage() {
        std::fprintf(stderr,
                "Usage: framesync-functest [OPTIONS]\n"
                "\n"
...
int main(int argc, char **argv) {
        ...
        if(source == nullptr) {
                std::fprintf(stderr,
                        "Error: failed to create MediaIO backend '%s'\n",
                        opts.backend.cstr());
                return 1;
        }
        ...
        if(mdesc.imageList().isEmpty()) {
                std::fprintf(stderr,
                        "Error: source has no video — FrameSync smoke test "
        ...
        std::thread consumer([&]() {
                ...
                while(running.load(std::memory_order_relaxed)) {
                        ...
                        if(e.isError()) {
                                ...
                                if(e == Error::Cancelled ||
                                   e == Error::Interrupt) break;
                                pullErrors.fetch_add(1,
                                        std::memory_order_relaxed);
                                std::fprintf(stderr,
                                        "Sync pull error: %s\n",
                                        e.name().cstr());
        ...
        if(framesIn.load() == 0) {
                std::fprintf(stderr,
                        "FAIL: no frames read from source\n");
                pass = false;
        }
        if(framesOut.load() == 0) {
                std::fprintf(stderr,
                        "FAIL: FrameSync produced no output frames\n");
                pass = false;
        }
        if(readErrors.load() > 0) {
                std::fprintf(stderr,
                        "FAIL: %llu source read errors\n",
                        (unsigned long long)readErrors.load());
                pass = false;
        }
        if(pushErrors.load() > 0) {
                std::fprintf(stderr,
                        "FAIL: %llu FrameSync push errors\n",
                        (unsigned long long)pushErrors.load());
                pass = false;
        }
        if(pullErrors.load() > 0) {
                std::fprintf(stderr,
                        "FAIL: %llu FrameSync pull errors\n",
                        (unsigned long long)pullErrors.load());
                pass = false;
        }
        ...
        if(expectedOut > 0.0) {
                const double ratio =
                        (double)framesOut.load() / expectedOut;
