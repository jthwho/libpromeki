...
TuiSplitter::TuiSplitter(Orientation orientation, ObjectBase *parent) : TuiWidget(parent), _orientation(orientation) {
...
void TuiSplitter::updateChildGeometry() {
        if (_orientation == Horizontal) {
                ...
                if (_second) _second->setGeometry(Rect2Di32(splitPos + 1, 0, width() - splitPos - 1, height()));
        } else {
                int splitPos = static_cast<int>(height() * _splitRatio);
                if (_first) _first->setGeometry(Rect2Di32(0, 0, width(), splitPos));
                if (_second) _second->setGeometry(Rect2Di32(0, splitPos + 1, width(), height() - splitPos - 1));
        }
}
...
void TuiSplitter::paintEvent(PaintEvent *) {
        ...
        TuiStyle s = pal.style(fgRole, false, isEnabled()).merged(pal.style(TuiPalette::Window, false, isEnabled()));
        painter.setStyle(s);
