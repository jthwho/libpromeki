...
void TuiSubsystem::setupEventSources() {
        ...
        _stdinSourceHandle =
                _eventLoop->addIoSource(STDIN_FILENO, EventLoop::IoRead, [this](int, uint32_t) { processInput(); });
...
void TuiSubsystem::focusNext(bool reverse) {
        ...
        if (reverse) {
                ...
                next = (current < 0 || current >= static_cast<int>(focusable.size()) - 1) ? 0 : current + 1;
        }
...
void TuiSubsystem::dispatchKeyEvent(const TuiInputParser::ParsedEvent &ev) {
        ...
        if (ev.key == static_cast<KeyEvent::Key>('q') && (ev.modifiers & KeyEvent::CtrlModifier)) {
                Application::quit(0);
                return;
        }
...
void TuiSubsystem::dispatchMouseEvent(const TuiInputParser::ParsedEvent &ev) {
        ...
        if (action == MouseEvent::Press) {
                auto now = Clock::now();
                auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - _lastClickTime).count();
                if (elapsed <= DoubleClickIntervalMs && ev.mouseButton == _lastClickButton &&
...
void TuiSubsystem::handleResize() {
        ...
        if (_rootWidget) {
                ...
                if (_rootWidget->layout()) {
                        _rootWidget->layout()->calculateLayout(Rect2Di32(0, 0, cols, rows));
                }
        }
        markNeedsRepaint();
}
...
void TuiSubsystem::paintWidget(TuiWidget *widget) {
        ...
        Point2Di32 screenPos = widget->mapToGlobal(Point2Di32(0, 0));
        Rect2Di32  clipRect(screenPos.x(), screenPos.y(), widget->width(), widget->height());
