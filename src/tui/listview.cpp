...
void TuiListView::insertItem(int index, const String &item) {
        ...
        if(_currentIndex < 0) _currentIndex = 0;
        else if(_currentIndex >= idx) _currentIndex++;
...
void TuiListView::keyEvent(KeyEvent *e) {
        switch(e->key()) {
                ...
                case KeyEvent::Key_Down:
                        if(_currentIndex < static_cast<int>(_items.size()) - 1)
                ...
                case KeyEvent::Key_PageDown:
                        setCurrentIndex(std::min(static_cast<int>(_items.size()) - 1,
                                                  _currentIndex + height()));
                        e->accept();
                        break;
