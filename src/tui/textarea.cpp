...
TuiTextArea::TuiTextArea(ObjectBase *parent)
        : TuiWidget(parent) {
...
void TuiTextArea::paintEvent(TuiPaintEvent *) {
        ...
        for(int row = 0; row < height(); ++row) {
                ...
                String visible = line.substr(_scrollCol,
                        std::min(static_cast<size_t>(width()), line.length()));
...
void TuiTextArea::keyEvent(KeyEvent *e) {
        ...
        switch(e->key()) {
                ...
                case KeyEvent::Key_Right:
                        if(static_cast<size_t>(_cursorCol) < _lines[_cursorRow].length()) _cursorCol++;
