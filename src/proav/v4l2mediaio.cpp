...
PROMEKI_REGISTER_MEDIAIO_FACTORY(V4l2Factory)
PROMEKI_DEBUG(V4l2MediaIO)

// Clock domains for V4L2 video and ALSA audio timestamps.
static const ClockDomain::ID _v4l2KernelClockID = ClockDomain::registerDomain(
        "v4l2.kernel", "V4L2 CLOCK_MONOTONIC kernel video capture timestamp", ClockEpoch::Correlated);
static const ClockDomain::ID _alsaClockID =
        ClockDomain::registerDomain("alsa.hardware", "ALSA PCM hardware audio capture clock", ClockEpoch::PerStream);
...
PixelFormat::ID V4l2MediaIO::v4l2ToPixelFormat(uint32_t v4l2fmt) {
...
uint32_t V4l2MediaIO::pixelFormatToV4l2(PixelFormat::ID pd) {
        for (int i = 0; i < FormatMapCount; i++) {
...
static List<RuntimeControl> enumerateAndRegisterControls(int fd) {
        ...
        while (xioctl(fd, VIDIOC_QUERYCTRL, &qc) == 0) {
                ...
                if (!id.isValid()) {
                        String      keyName = sanitizeControlName(reinterpret_cast<const char *>(qc.name));
                        VariantSpec vs;
                        ...
                        if (qc.type == V4L2_CTRL_TYPE_MENU) {
                                ...
                                for (qm.index = static_cast<uint32_t>(qc.minimum);
                                     static_cast<int>(qm.index) <= qc.maximum; qm.index++) {
                                        if (xioctl(fd, VIDIOC_QUERYMENU, &qm) == 0) {
                                                values.pushToBack(
                                                        Enum::Value(String(reinterpret_cast<const char *>(qm.name)),
                        ...
                        vs.setDescription(String::sprintf("%s (empty/unset = device default).", qc.name));
...
static void applyV4l2Controls(int fd, const MediaIO::Config &cfg) {
        ...
        for (const auto &rc : controls) {
                ...
                if (xioctl(fd, VIDIOC_S_CTRL, &ctrl) < 0) {
                        promekiWarn("V4l2MediaIO: failed to set %s=%d: %s", rc.v4l2Name.cstr(), val,
                                    std::strerror(errno));
                }
        }
}
...
static String findPairedAlsaDevice(const String &v4l2DevPath) {
        ...
        for (const auto &cardEntry : cards) {
                ...
                promekiInfo("V4l2MediaIO: auto-detected paired ALSA "
...
static bool v4l2CanHandlePath(const String &path) {
        ...
        struct v4l2_capability cap;
        std::memset(&cap, 0, sizeof(cap));
        bool ok = (xioctl(fd, VIDIOC_QUERYCAP, &cap) == 0) && (cap.capabilities & V4L2_CAP_VIDEO_CAPTURE);
        if (ok) {
...
static List<MediaDesc> v4l2QueryDevice(const MediaIO::Config &config) {
        ...
        if (devPath.isEmpty()) devPath = config.getAs<String>(MediaConfig::Filename, String());
        ...
        while (xioctl(fd, VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
                PixelFormat::ID pdId = V4l2MediaIO::v4l2ToPixelFormat(fmtdesc.pixelformat);
                ...
                while (xioctl(fd, VIDIOC_ENUM_FRAMESIZES, &frmsize) == 0) {
                        ...
                        while (xioctl(fd, VIDIOC_ENUM_FRAMEINTERVALS, &frmival) == 0) {
                                ...
                                if (frmival.discrete.numerator > 0) {
                                        FrameRate fps(FrameRate::RationalType(frmival.discrete.denominator,
                                                                              frmival.discrete.numerator));
                                        MediaDesc md;
                                        md.setFrameRate(fps);
                                        md.imageList().pushToBack(ImageDesc(w, h, pdId));
...
static void v4l2PrintControls(const String &devPath) {
        ...
        fprintf(stdout, "\nControls:\n");
        for (const auto &rc : controls) {
                fprintf(stdout, "  %-34s min=%-6d max=%-6d default=%-6d  [--sc %s:N]\n", rc.v4l2Name.cstr(), rc.minimum,
                        rc.maximum, rc.defaultValue, rc.configId.name().cstr());
        }
}
...
bool V4l2Factory::canHandlePath(const String &path) const {
        return v4l2CanHandlePath(path);
}

StringList V4l2Factory::enumerate() const {
        return enumerateV4l2Devices();
}

V4l2Factory::Config::SpecMap V4l2Factory::configSpecs() const {
        ...
        // Camera controls — for the integer-typed controls the
        // convention is "-1 = don't touch".  The two Enum-typed controls
        // (V4l2AutoExposure, V4l2PowerLineFreq) keep the global spec's
        // Enum() default (which means "use device default") rather than
        // taking int32_t(-1) — overriding with an int would violate the
...
List<MediaDesc> V4l2Factory::queryDevice(const Config &config) const {
        return v4l2QueryDevice(config);
}

void V4l2Factory::printDeviceInfo(const Config &config) const {
        String p = config.getAs<String>(MediaConfig::V4l2DevicePath, String());
        if (p.isEmpty()) p = config.getAs<String>(MediaConfig::Filename, String());
        if (!p.isEmpty()) v4l2PrintControls(p);
}

MediaIO *V4l2Factory::create(const Config &config, ObjectBase *parent) const {
        auto *io = new V4l2MediaIO(parent);
        io->setConfig(config);
        return io;
}

// ============================================================================
// V4l2MediaIO lifecycle
// ============================================================================

V4l2MediaIO::V4l2MediaIO(ObjectBase *parent) : DedicatedThreadMediaIO(parent) {}

V4l2MediaIO::~V4l2MediaIO() {
...
Error V4l2MediaIO::openVideo(const MediaIO::Config &cfg) {
        ...
        if (devPath.isEmpty()) {
                promekiErr("V4l2MediaIO: V4l2DevicePath is empty");
        ...
        if (_fd < 0) {
                promekiErr("V4l2MediaIO: cannot open %s: %s", devPath.cstr(), std::strerror(errno));
        ...
        if (xioctl(_fd, VIDIOC_QUERYCAP, &cap) < 0) {
                promekiErr("V4l2MediaIO: VIDIOC_QUERYCAP failed on %s", devPath.cstr());
                return Error::DeviceError;
        }
        if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
                promekiErr("V4l2MediaIO: %s does not support video capture", devPath.cstr());
                return Error::NotSupported;
        }
        if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
                promekiErr("V4l2MediaIO: %s does not support streaming I/O", devPath.cstr());
                return Error::NotSupported;
        }

        promekiDebug("V4l2MediaIO: device %s  driver=%s  card=%s  bus=%s  caps=0x%08x", devPath.cstr(),
                     reinterpret_cast<const char *>(cap.driver), reinterpret_cast<const char *>(cap.card),
                     reinterpret_cast<const char *>(cap.bus_info), cap.capabilities);
        ...
        PixelFormat reqPd =
                cfg.getAs<PixelFormat>(MediaConfig::VideoPixelFormat, PixelFormat(PixelFormat::YUV8_422_Rec709));
        ...
        auto               tryFormat = [&](uint32_t pixfmt) -> bool {
                ...
                return fmt.fmt.pix.width == reqSize.width() && fmt.fmt.pix.height == reqSize.height();
        };
        ...
        if (!sizeOk) {
                ...
                tryFormat(v4l2fmt);
                promekiWarn("V4l2MediaIO: device cannot provide %ux%u; "
                            "using %ux%u",
                            reqSize.width(), reqSize.height(), fmt.fmt.pix.width, fmt.fmt.pix.height);
        }
        ...
        if (actualPd == PixelFormat::Invalid) {
                promekiErr("V4l2MediaIO: driver selected unsupported pixel format 0x%08x",
                           fmt.fmt.pix.pixelformat);
                return Error::NotSupported;
        }
        if (fmt.fmt.pix.pixelformat != v4l2fmt) {
                PixelFormat selectedPd(actualPd);
                promekiInfo("V4l2MediaIO: pixel format changed from %s to %s "
                            "to achieve %ux%u",
                            reqPd.name().cstr(), selectedPd.name().cstr(), fmt.fmt.pix.width, fmt.fmt.pix.height);
        }
        _imageDesc = ImageDesc(fmt.fmt.pix.width, fmt.fmt.pix.height, actualPd);
        if (!_imageDesc.isValid()) {
                promekiErr("V4l2MediaIO: resulting ImageDesc is invalid");
        ...
        if (xioctl(_fd, VIDIOC_S_PARM, &parm) < 0) {
                promekiWarn("V4l2MediaIO: VIDIOC_S_PARM failed: %s (using device default)", std::strerror(errno));
        }

        // Read back actual frame rate
        std::memset(&parm, 0, sizeof(parm));
        parm.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
        if (xioctl(_fd, VIDIOC_G_PARM, &parm) == 0 && parm.parm.capture.timeperframe.denominator > 0 &&
            parm.parm.capture.timeperframe.numerator > 0) {
                // V4L2 timeperframe is duration (num/den), so fps = den/num.
                _frameRate = FrameRate(FrameRate::RationalType(parm.parm.capture.timeperframe.denominator,
                                                               parm.parm.capture.timeperframe.numerator));
        } else {
                _frameRate = reqFps;
        }
        if (_frameRate != reqFps) {
                promekiInfo("V4l2MediaIO: frame rate changed from %s to %s "
                            "(device does not support requested rate)",
                            reqFps.toString().cstr(), _frameRate.toString().cstr());
        }

        promekiDebug("V4l2MediaIO: negotiated %ux%u %s @ %s "
                     "(requested %ux%u %s @ %s)",
                     _imageDesc.size().width(), _imageDesc.size().height(), PixelFormat(actualPd).name().cstr(),
                     _frameRate.toString().cstr(), reqSize.width(), reqSize.height(), reqPd.name().cstr(),
        ...
        if (xioctl(_fd, VIDIOC_REQBUFS, &req) < 0) {
                promekiErr("V4l2MediaIO: VIDIOC_REQBUFS failed: %s", std::strerror(errno));
                return Error::DeviceError;
        }
        if (req.count < 2) {
                promekiErr("V4l2MediaIO: insufficient buffer memory (got %u)", req.count);
                return Error::NoMem;
        }
...
Error V4l2MediaIO::startStreaming() {
        // Queue all buffers
        for (size_t i = 0; i < _buffers.size(); i++) {
                ...
                if (xioctl(_fd, VIDIOC_QBUF, &buf) < 0) {
                        promekiErr("V4l2MediaIO: VIDIOC_QBUF failed for buffer %zu: %s", i, std::strerror(errno));
                        return Error::DeviceError;
                }
        }
        ...
        if (xioctl(_fd, VIDIOC_STREAMON, &type) < 0) {
                promekiErr("V4l2MediaIO: VIDIOC_STREAMON failed: %s", std::strerror(errno));
                return Error::DeviceError;
        }
        _streaming = true;
...
Error V4l2MediaIO::openAudio(const MediaIO::Config &cfg) {
        ...
        if (alsaDev.isEmpty() || alsaDev == "none") {
                promekiDebug("V4l2MediaIO: audio disabled (V4l2AudioDevice=%s)",
        ...
        if (alsaDev == "auto") {
                ...
                if (v4l2Path.isEmpty()) v4l2Path = cfg.getAs<String>(MediaConfig::Filename, String());
                alsaDev = findPairedAlsaDevice(v4l2Path);
                if (alsaDev.isEmpty()) {
                        promekiDebug("V4l2MediaIO: no paired audio device found, "
                                     "audio disabled");
                        _audioEnabled = false;
                        return Error::Ok;
                }
        ...
        promekiDebug("V4l2MediaIO: opening ALSA device %s", alsaDev.cstr());
        ...
        if (err < 0) {
                promekiErr("V4l2MediaIO: snd_pcm_open(%s) failed: %s", alsaDev.cstr(), snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        if (err < 0) {
                promekiErr("V4l2MediaIO: cannot set interleaved access: %s", snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        if (err < 0) {
                ...
                if (err < 0) {
                        promekiErr("V4l2MediaIO: cannot set PCM format: %s", snd_strerror(err));
                        return Error::DeviceError;
                }
                _audioDesc = AudioDesc(AudioFormat::PCMI_S16LE, sampleRate, static_cast<unsigned int>(channels));
        } else {
                _audioDesc = AudioDesc(AudioFormat::PCMI_Float32LE, sampleRate, static_cast<unsigned int>(channels));
        }

        unsigned int rate = static_cast<unsigned int>(sampleRate);
        err = snd_pcm_hw_params_set_rate_near(_pcm, hwparams, &rate, nullptr);
        if (err < 0) {
                promekiErr("V4l2MediaIO: cannot set sample rate %u: %s", rate, snd_strerror(err));
                return Error::DeviceError;
        }

        err = snd_pcm_hw_params_set_channels(_pcm, hwparams, static_cast<unsigned int>(channels));
        if (err < 0) {
                promekiErr("V4l2MediaIO: cannot set %d channels: %s", channels, snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        if (err < 0) {
                promekiErr("V4l2MediaIO: snd_pcm_hw_params failed: %s", snd_strerror(err));
                return Error::DeviceError;
        }

        // Update the audio desc with the actual rate the driver selected
        if (rate != static_cast<unsigned int>(sampleRate)) {
                promekiWarn("V4l2MediaIO: ALSA sample rate adjusted to %u", rate);
                _audioDesc = AudioDesc(_audioDesc.format().id(), static_cast<float>(rate),
                                       static_cast<unsigned int>(channels));
        }

        err = snd_pcm_prepare(_pcm);
        if (err < 0) {
                promekiErr("V4l2MediaIO: snd_pcm_prepare failed: %s", snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        AudioDesc ringDesc(AudioFormat::NativeFloat, static_cast<float>(rate), static_cast<unsigned int>(channels));
        _audioRing = AudioBuffer(ringDesc, ringCapacity);
        _audioRing.setInputFormat(_audioDesc);

        _audioEnabled = true;
        promekiDebug("V4l2MediaIO: ALSA opened %s  format=%s  rate=%u  "
                     "channels=%d  bufferSize=%lu  ringCapacity=%zu",
                     alsaDev.cstr(), _audioDesc.format().name().cstr(), rate, channels,
                     static_cast<unsigned long>(bufferSize), ringCapacity);
        return Error::Ok;
}

void V4l2MediaIO::closeAudio() {
        if (_pcm != nullptr) {
...
void V4l2MediaIO::videoCaptureLoop() {
        ...
        while (!_stopFlag.load(std::memory_order_acquire)) {
                ...
                if (!_seqInitialized) {
                        ...
                        uint32_t    src = vbuf.flags & V4L2_BUF_FLAG_TSTAMP_SRC_MASK;
                        const char *srcName = (src == V4L2_BUF_FLAG_TSTAMP_SRC_SOE)   ? "SOE"
                                              : (src == V4L2_BUF_FLAG_TSTAMP_SRC_EOF) ? "EOF"
                                                                                      : "unknown";
                        uint32_t    monoFlag = vbuf.flags & V4L2_BUF_FLAG_TIMESTAMP_MASK;
                        const char *monoName = (monoFlag == V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC) ? "MONOTONIC"
                                               : (monoFlag == V4L2_BUF_FLAG_TIMESTAMP_COPY)    ? "COPY"
                                                                                               : "unknown";
                        promekiDebug("V4l2MediaIO: first DQBUF  "
                ...
                if (_frameRate.toDouble() > 0.0) {
                        maxLagUs = static_cast<int64_t>(3.0e6 / _frameRate.toDouble());
                }
                if (rawLagUs > maxLagUs || rawLagUs < -maxLagUs) {
                        promekiDebug("V4l2MediaIO: frame seq=%u has "
                ...
                if (pd.isCompressed()) {
                        ...
                        auto cvp = CompressedVideoPayload::Ptr::create(capDesc, imgBuf);
...
Error V4l2MediaIO::executeCmd(MediaIOCommandOpen &cmd) {
        ...
        _debugReport = PeriodicCallback(1.0, [this] {
                ...
                if (_firstCaptureFrame >= 0 && _frameCount.value() > _firstCaptureFrame + 1) {
                        double totalSec = (_lastCaptureTime - _firstCaptureTime).toSecondsDouble();
                        if (totalSec > 0.0) {
                                avgFps = (double)(_frameCount.value() - _firstCaptureFrame) / totalSec;
                        }
                }
                ...
                if (_frameDeltaCount > 0) {
                        ...
                        double variance = (_frameDeltaSqSum / (double)_frameDeltaCount) - (meanDelta * meanDelta);
                ...
                if (_audioEnabled) {
                        ...
                        if (_ringAccumFrames > 0) {
                                ...
                                if (!_ringBaselineSet) {
                                        ...
                                        double elapsedSec = (_lastCaptureTime - _ringBaselineTime).toSecondsDouble();
                                        if (elapsedSec > 0.0) {
                        ...
                        promekiDebug("f=%lld cap=%lld q=%zu/%d "
                                     ...
                                     static_cast<long long>(_frameCount.value()), static_cast<long long>(captured),
                                     qDepth, VideoQueueDepth, avgFps, periodFps, fpsChange, jitterUs, ringAvail,
                                     ringCap, static_cast<long long>(overruns), ringAvg, ringDrift, lastSeq,
                                     static_cast<long long>(kDrop), static_cast<long long>(tsSub), loopAvgMs, loopMaxMs,
                                     lagAvgMs, lagMaxMs);
                } else {
                        promekiDebug("f=%lld cap=%lld q=%zu/%d "
                                     "fps=%.3f pfps=%.3f fpsd=%+.3f jit=%.0fus "
                                     ...
                                     "loop=%.2f/%.2fms lag=%.2f/%.2fms",
                                     static_cast<long long>(_frameCount.value()), static_cast<long long>(captured),
                                     qDepth, VideoQueueDepth, avgFps, periodFps, fpsChange, jitterUs, lastSeq,
                                     static_cast<long long>(kDrop), static_cast<long long>(tsSub), loopAvgMs, loopMaxMs,
                                     lagAvgMs, lagMaxMs);
                }
        });
        ...
        // V4L2 is a capture device: source-only, single port group.
        MediaIOPortGroup *group = addPortGroup("v4l2");
...
Error V4l2MediaIO::executeCmd(MediaIOCommandClose &cmd) {
        (void)cmd;
        stopThreads();
        closeAudio();
        closeVideo();
