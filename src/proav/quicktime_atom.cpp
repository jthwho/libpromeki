...
namespace quicktime_atom {
        ...
        uint64_t ReadStream::readU64() {
                ...
                return (static_cast<uint64_t>(b[0]) << 56) | (static_cast<uint64_t>(b[1]) << 48) |
        ...
        FourCC ReadStream::readFourCC() {
                ...
                return FourCC(static_cast<char>(b[0]), static_cast<char>(b[1]), static_cast<char>(b[2]),
                ...
                if (size32 == 1) {
                        ...
                        if (large < 16) {
                                promekiWarn("QuickTime: 64-bit box '%c%c%c%c' largesize %llu < 16",
                                            (type.value() >> 24) & 0xff, (type.value() >> 16) & 0xff,
                                            (type.value() >> 8) & 0xff, (type.value() >> 0) & 0xff,
                        ...
                        if (size32 < 8) {
                                promekiWarn("QuickTime: 32-bit box '%c%c%c%c' size %u < 8", (type.value() >> 24) & 0xff,
                                            (type.value() >> 16) & 0xff, (type.value() >> 8) & 0xff,
                                            (type.value() >> 0) & 0xff, size32);
                                return Error::CorruptData;
                        }
                        payloadSize = static_cast<int64_t>(size32) - 8;
                }

                if (enforceEnd > 0 && payloadSize >= 0 && payloadOffset + payloadSize > enforceEnd) {
        ...
        Error findTopLevelBox(ReadStream &stream, FourCC type, int64_t startOffset, int64_t endOffset, Box &out) {
