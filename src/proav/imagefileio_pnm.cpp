...
Error ImageFileIO_PNM::load(ImageFile &imageFile, const MediaConfig &config) const {
        ...
        if (!p) {
                promekiErr("PNM load '%s': missing width", filename.cstr());
                return Error::Invalid;
        }
        p = parseUInt(p, end, height);
        if (!p) {
                promekiErr("PNM load '%s': missing height", filename.cstr());
                return Error::Invalid;
        }
        ...
        if (type != PBM_ASCII) {
                ...
                if (!p) {
                        promekiErr("PNM load '%s': missing maxval", filename.cstr());
                        return Error::Invalid;
                }
        }
        ...
        promekiDebug("PNM load '%s': %ux%u %s (%s)", filename.cstr(), width, height, PixelFormat(pdId).name().cstr(),
                     magic);
        return Error::Ok;
}
...
Error ImageFileIO_PNM::save(ImageFile &imageFile, const MediaConfig &config) const {
        ...
        switch (pdId) {
                ...
                case PixelFormat::Mono16_BE_sRGB:
                        ...
                        break;
                default:
                        promekiErr("PNM save '%s': unsupported pixel format '%s'", filename.cstr(),
        ...
        if (written != static_cast<int64_t>(imageBytes)) {
                file.close();
                return Error::IOError;
        }

        file.close();
        promekiDebug("PNM save '%s': %zux%zu %s", filename.cstr(), width, height, desc.pixelFormat().name().cstr());
        return Error::Ok;
}
