 ...
 */
        ...
        uint64_t rescaleRound(uint64_t value, uint32_t toScale, uint32_t fromScale) {
                ...
                return (value * static_cast<uint64_t>(toScale) + fromScale / 2) / static_cast<uint64_t>(fromScale);
        }
        ...
        template <typename TrackList> uint32_t lcmTimescale(const TrackList &tracks) {
                uint64_t ts = 1;
                bool     any = false;
        ...
        uint16_t packLanguage(const String &lang) {
                ...
                return static_cast<uint16_t>(((c[0] - 0x60) & 0x1f) << 10 | ((c[1] - 0x60) & 0x1f) << 5 |
                                             ((c[2] - 0x60) & 0x1f));
        }
...
Error QuickTimeWriter::open() {
        ...
        if (err.isError()) {
                promekiWarn("QuickTime writer: open '%s': %s", _filename.cstr(), err.name().cstr());
                return err;
        }
...
Error QuickTimeWriter::addVideoTrack(const PixelFormat &codec, const Size2Du32 &size, const FrameRate &frameRate,
                                     uint32_t *outTrackId) {
...
Error QuickTimeWriter::addTimecodeTrack(const Timecode &startTimecode, const FrameRate &frameRate,
                                        uint32_t *outTrackId) {
        if (!_isOpen) return Error::NotOpen;
        ...
        t.tcFrameCount =
                static_cast<uint8_t>((frameRate.rational().numerator() + frameRate.rational().denominator() - 1) /
...
Error QuickTimeWriter::writeSample(uint32_t trackId, const QuickTime::Sample &sample) {
        ...
        if (isH264 || isHEVC) {
                ...
                Error convErr = H264Bitstream::annexBToAvccFiltered(
                        srcView, 4, isH264 ? keepNonH264ParameterSetNal : keepNonHevcParameterSetNal, convertedPayload);
                if (convErr.isError()) {
        ...
        if (_layout == QuickTime::LayoutFragmented) {
                ...
                if (t.pcmBytesPerSample != 0 && t.type == QuickTime::Audio) {
                        if ((payloadSize % t.pcmBytesPerSample) != 0) {
                                promekiWarn("QuickTime: audio write payload %lld not a multiple "
                                            ...
                                            static_cast<long long>(payloadSize), t.pcmBytesPerSample);
                ...
                std::memcpy(t.fragPayload.data() + oldSize, bytes, static_cast<size_t>(payloadSize));
...
Error QuickTimeWriter::flush() {
        ...
        if (_flushSync && _file != nullptr) {
                ...
                if (syncErr.isError()) {
                        promekiWarn("QuickTime writer: flushSync failed: %s", syncErr.name().cstr());
                        return syncErr;
                }
        }
        return Error::Ok;
...
void QuickTimeWriter::appendTrak(AtomWriter &w, const QuickTimeWriterTrack &t, uint32_t movieTimescale) {
        ...
        if (t.type == QuickTime::Video) {
                ...
                if (t.type == QuickTime::TimecodeTrack) {
                        ...
                        w.writeU16(0xffff);
                        w.writeU16(0xffff);
                        w.writeU16(0xffff); // text_color
                        w.writeU16(0);
                        w.writeU16(0);
                        w.writeU16(0); // background
        ...
        if (t.type == QuickTime::Audio) {
                ...
                for (int64_t off : t.audioChunkOffsets) {
                        if (off > static_cast<int64_t>(UINT32_MAX)) {
                                need64 = true;
                                break;
                        }
                }
...
void QuickTimeWriter::appendInitTrak(AtomWriter &w, const QuickTimeWriterTrack &t, uint32_t movieTimescale) {
        ...
        {
                auto b = w.beginFullBox(kStts, 0, 0);
                w.writeU32(0);
                w.endBox(b);
        }
        {
                auto b = w.beginFullBox(kStsc, 0, 0);
                w.writeU32(0);
                w.endBox(b);
        }
        {
                auto b = w.beginFullBox(kStsz, 0, 0);
                w.writeU32(0);
                w.writeU32(0);
                w.endBox(b);
        }
        {
                auto b = w.beginFullBox(kStco, 0, 0);
                w.writeU32(0);
                w.endBox(b);
        }
...
Error QuickTimeWriter::writeFragment() {
        ...
        for (const FragTrack &ft : fragTracks) {
                ...
                bool hasCtts = false;
                for (int32_t v : t.fragSampleCtsOffsets) {
                        if (v != 0) {
                                hasCtts = true;
                                break;
                        }
                }
                uint32_t uniformSampleFlags = (!t.fragSampleKeyframes.isEmpty() && t.fragSampleKeyframes[0])
                                                      ? kSampleFlagsSync
                                                      : kSampleFlagsNonSync;
                ...
                for (size_t i = 0; i < sampleCount; ++i) {
                        ...
                        if (writePerSampleFlags) {
                                uint32_t sflags = t.fragSampleKeyframes[i] ? kSampleFlagsSync : kSampleFlagsNonSync;
        ...
        for (const TrunPatch &p : patches) {
                int64_t dataOffset =
                        static_cast<int64_t>(moofSize) + mdatHeaderSize + static_cast<int64_t>(p.mdatOffset);
        ...
        if (mdatTotalSize > UINT32_MAX) {
                promekiWarn("QuickTime: fragment mdat size %lld exceeds 32-bit", static_cast<long long>(mdatTotalSize));
                return Error::OutOfRange;
        }
        AtomWriter mdatHdr;
