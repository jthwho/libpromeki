...
bool isJpegEncoderInputFormat(const PixelFormat &pd) {
        ...
        const List<PixelFormat> supported =
                VideoCodec(VideoCodec::JPEG).encoderSupportedInputs();
...
Buffer::Ptr copyPayloadToBuffer(const CompressedVideoPayload &cvp) {
        ...
        for(size_t i = 0; i < view.count(); ++i) {
                ...
                std::memcpy(static_cast<uint8_t *>(raw->data()) + cursor,
...
MediaIO::FormatDesc MediaIOTask_MjpegStream::formatDesc() {
        return {
                ...
                []() -> MediaIOTask * {
                        return new MediaIOTask_MjpegStream();
                },
...
bool MediaIOTask_MjpegStream::latestRingEntry(Buffer::Ptr *out,
                                              TimeStamp *outTs) const {
        Mutex::Locker lk(_stateMutex);
...
Error MediaIOTask_MjpegStream::executeCmd(MediaIOCommandOpen &cmd) {
        ...
        Rational<int> configuredFps =
                cfg.getAs<Rational<int>>(MediaConfig::MjpegMaxFps,
                                         Rational<int>(15, 1));
        if(configuredFps.numerator() <= 0 || configuredFps.denominator() <= 0) {
                ...
                FrameRate::RationalType r(
                        static_cast<unsigned int>(configuredFps.numerator()),
        ...
        encCfg.set(MediaConfig::OutputPixelFormat,
        ...
        if(error(encResult).isError()) {
                promekiErr("MediaIOTask_MjpegStream: createEncoder failed: %s",
        ...
        promekiInfo("MediaIOTask_MjpegStream: opened maxFps=%.4f quality=%d "
                    "ringDepth=%d",
                    _maxRate.isValid() ? _maxRate.toDouble() : 0.0,
                    _quality, _ringDepth);
...
Error MediaIOTask_MjpegStream::executeCmd(MediaIOCommandClose &cmd) {
        ...
        const double avgUs = snap.encodeSamples > 0
                ? static_cast<double>(snap.totalEncodeUs)
                  / static_cast<double>(snap.encodeSamples)
                : 0.0;
        promekiInfo("MediaIOTask_MjpegStream: closed — encoded %lld, "
                    ...
                    static_cast<long long>(snap.framesEncoded),
                    static_cast<long long>(snap.framesRateLimited),
                    static_cast<long long>(snap.framesEncodeError),
                    avgUs,
                    static_cast<long long>(snap.peakEncodeUs),
...
Error MediaIOTask_MjpegStream::executeCmd(MediaIOCommandWrite &cmd) {
        ...
        const int64_t   bytes   = jpeg.isValid()
                ? static_cast<int64_t>(jpeg->size())
                : 0;
...
Error MediaIOTask_MjpegStream::executeCmd(MediaIOCommandStats &cmd) {
        ...
        cmd.stats.set(MediaIOStats::PeakLatencyMs,
                      static_cast<double>(snap.peakEncodeUs) / 1000.0);
        return Error::Ok;
}
...
Error MediaIOTask_MjpegStream::proposeInput(const MediaDesc &offered,
...
Error MediaIOTask_MjpegStream::encodeFrame(const Frame &frame, Buffer::Ptr *out,
                                           TimeStamp *ts) {
        ...
        for(const auto &vp : videos) {
                ...
                UncompressedVideoPayload::Ptr cand =
                        sharedPointerCast<UncompressedVideoPayload>(vp);
                if(cand.isValid()) { uvp = cand; break; }
        }
...
void MediaIOTask_MjpegStream::publishEncoded(const Buffer::Ptr &jpeg,
                                             const TimeStamp &ts) {
...
class MjpegMultipartAdapter : public ObjectBase,
                              public MjpegStreamSubscriber {
        public:
                MjpegMultipartAdapter(MediaIOTask_MjpegStream *sink,
                ...
                void onFrame(const Buffer::Ptr &jpeg,
                        ...
                        const String header = String("--")
                                + MediaIOTask_MjpegStream::HttpBoundary
                                + "\r\nContent-Type: "
                                + MediaIOTask_MjpegStream::LatestJpegContentType
                                + "\r\nContent-Length: "
                                + String::number(static_cast<int64_t>(n))
                                + "\r\n\r\n";
                        const String trailer("\r\n");

                        Buffer::Ptr hdr =
                                Buffer::Ptr::create(header.byteCount());
                        std::memcpy(hdr.modify()->data(), header.cstr(),
                                    header.byteCount());
                        hdr.modify()->setSize(header.byteCount());

                        Buffer::Ptr tl =
                                Buffer::Ptr::create(trailer.byteCount());
                        std::memcpy(tl.modify()->data(), trailer.cstr(),
                                    trailer.byteCount());
...
HttpHandlerFunc MediaIOTask_MjpegStream::buildMultipartHandler(
        MediaIOTask_MjpegStream *sink) {
        ...
        return [sink](const HttpRequest &, HttpResponse &res) {
                ...
                IODevice::Shared queueShared =
                        IODevice::Shared::takeOwnership(rawQueue);

                MjpegMultipartAdapter *adapter =
                        new MjpegMultipartAdapter(sink, rawQueue);
                ...
                res.setBodyStream(queueShared, /*length=*/-1,
                                  contentType);
        };
}

void MediaIOTask_MjpegStream::registerHttpRoute(HttpServer &server,
                                                const String &path) {
