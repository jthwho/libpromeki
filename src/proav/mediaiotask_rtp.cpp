...
// ----------------------------------------------------------------------------
class MediaIOTask_Rtp::SendThread : public Thread {
        public:
...
// Content probe for SDP files.  RFC 4566 mandates that every SDP
// session description starts with a v= version line, which is
// conventionally "v=0" followed by a newline.  We accept any leading
// whitespace so that SDP files produced by tools that prefix a BOM
// or blank lines still parse.  The probe only inspects the first
// handful of bytes — the rest of the file is parsed by
// SdpSession::fromString() when the task actually opens.
...
MediaIO::FormatDesc MediaIOTask_Rtp::formatDesc() {
        return {
                ...
                {"sdp"}, // Extension match for readers: an SDP file
                         ...
                         // never open via a filesystem path, but the
...
Error MediaIOTask_Rtp::executeCmd(MediaIOCommandOpen &cmd) {
        ...
        // the first entry in the resolved media descriptor.
        // Without this, downstream consumers that check
...
Error MediaIOTask_Rtp::executeCmd(MediaIOCommandParams &cmd) {
        if(cmd.name == ParamGetSdp.name()) {
                ...
                // SdpSession — MediaIOTask doesn't currently
                // expose a typed variant-returning params command.
                cmd.result.set(ParamSdp, _sdpSession.toString());
                return Error::Ok;
        }
        return Error::NotSupported;
}
