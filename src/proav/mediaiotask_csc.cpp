...
namespace {
        ...
        bool cscBridge(const MediaDesc &from, const MediaDesc &to, MediaIO::Config *outConfig, int *outCost) {
...
Error MediaIOTask_CSC::convertPayload(const UncompressedVideoPayload &input,
        ...
        if (!output.isValid()) {
                promekiErr("MediaIOTask_CSC: convert %s -> %s failed", srcPd.name().cstr(),
...
Error MediaIOTask_CSC::proposeInput(const MediaDesc &offered, MediaDesc *preferred) const {
...
Error MediaIOTask_CSC::proposeOutput(const MediaDesc &requested, MediaDesc *achievable) const {
