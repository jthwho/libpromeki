        ...
        while(src < srcEnd) {
                ...
                if(src > runStart) {
                        ...
                        *dst++ = *runStart;
                        ...
                        while(src < srcEnd && (src + 1 >= srcEnd || *src != *(src + 1)) &&
                              (src - litStart) < 126) ++src;
...
static void deplanarize8(uint8_t *dst, const uint8_t *planar,
                         size_t w, size_t h, uint16_t zSize) {
...
static void deplanarize16(uint16_t *dst, const uint16_t *planar,
                          size_t w, size_t h, uint16_t zSize) {
        size_t planeSize = w * h;
        for(size_t y = 0; y < h; ++y) {
                size_t sgiRow = h - 1 - y;
...
static void planarize8(uint8_t *planar, const uint8_t *src,
                       size_t w, size_t h, uint16_t zSize) {
        size_t planeSize = w * h;
        for(size_t y = 0; y < h; ++y) {
                size_t sgiRow = h - 1 - y;
...
static void planarize16(uint16_t *planar, const uint16_t *src,
                        size_t w, size_t h, uint16_t zSize) {
        size_t planeSize = w * h;
        for(size_t y = 0; y < h; ++y) {
                size_t sgiRow = h - 1 - y;
...
Error ImageFileIO_SGI::load(ImageFile &imageFile, const MediaConfig &config) const {
        ...
        if(hdr.storage == 1) {
                ...
                file.close();
                if(n != dataSize) { return Error::IOError; }
                ...
                if(hdr.bpc == 1) {
                        for(size_t chan = 0; chan < z; ++chan) {
                                for(size_t row = 0; row < h; ++row) {
                                        ...
                                        uint8_t *dst = static_cast<uint8_t *>(planarBuf.data())
                                                       + (chan * h * w) + (row * w);
                        ...
                        for(size_t chan = 0; chan < z; ++chan) {
                                for(size_t row = 0; row < h; ++row) {
                                        ...
                                        uint16_t *dst = reinterpret_cast<uint16_t *>(
                                                static_cast<uint8_t *>(planarBuf.data())
                                                + (chan * h * w + row * w) * 2);
...
Error ImageFileIO_SGI::save(ImageFile &imageFile, const MediaConfig &config) const {
        ...
        if(written != sizeof(writeHdr)) { file.close(); return Error::IOError; }

        written = file.write(planarBuf.data(), static_cast<int64_t>(planarBytes));
        if(written != static_cast<int64_t>(planarBytes)) { file.close(); return Error::IOError; }
