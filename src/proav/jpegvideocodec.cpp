...
namespace {
        ...
        static YCbCrInfo classifyYCbCr(PixelFormat::ID id) {
                switch (id) {
                        ...
                        case PixelFormat::YUV8_422_Rec601_Full: return {LayoutInterleavedYUYV, false};
                        // 4:2:2 interleaved UYVY (Rec.601 / Rec.709 limited variants only).
                        case PixelFormat::YUV8_422_UYVY_Rec709:
                        case PixelFormat::YUV8_422_UYVY_Rec601: return {LayoutInterleavedUYVY, false};
                        // 4:2:2 planar (Rec.709 limited only; no other planar
                        // variants exist as independent IDs).
                        case PixelFormat::YUV8_422_Planar_Rec709: return {LayoutPlanar422, false};
                        ...
                        case PixelFormat::YUV8_420_Planar_Rec601_Full: return {LayoutPlanar420, true};
                        // 4:2:0 semi-planar (NV12, Rec.709 / Rec.601 limited only).
                        case PixelFormat::YUV8_420_SemiPlanar_Rec709:
                        case PixelFormat::YUV8_420_SemiPlanar_Rec601: return {LayoutSemiPlanar420, true};
                        default: return {LayoutNone, false};
                }
        }
        ...
        static PixelFormat::ID jpegPixelFormatFor(PixelFormat::ID srcDesc) {
                switch (srcDesc) {
                        ...
                        case PixelFormat::YUV8_420_Planar_Rec709:
                        case PixelFormat::YUV8_420_SemiPlanar_Rec709: return PixelFormat::JPEG_YUV8_420_Rec709;

                        // 4:2:2 Rec.601 limited.
                        case PixelFormat::YUV8_422_Rec601:
                        case PixelFormat::YUV8_422_UYVY_Rec601: return PixelFormat::JPEG_YUV8_422_Rec601;

                        // 4:2:0 Rec.601 limited.
                        case PixelFormat::YUV8_420_Planar_Rec601:
                        case PixelFormat::YUV8_420_SemiPlanar_Rec601: return PixelFormat::JPEG_YUV8_420_Rec601;

                        // 4:2:2 Rec.709 full.
                        case PixelFormat::YUV8_422_Rec709_Full: return PixelFormat::JPEG_YUV8_422_Rec709_Full;

                        // 4:2:0 Rec.709 full.
                        case PixelFormat::YUV8_420_Planar_Rec709_Full: return PixelFormat::JPEG_YUV8_420_Rec709_Full;

                        // 4:2:2 Rec.601 full (strict JFIF).
                        case PixelFormat::YUV8_422_Rec601_Full: return PixelFormat::JPEG_YUV8_422_Rec601_Full;

                        // 4:2:0 Rec.601 full (strict JFIF).
                        case PixelFormat::YUV8_420_Planar_Rec601_Full: return PixelFormat::JPEG_YUV8_420_Rec601_Full;
                ...
                while (cinfo.next_scanline < cinfo.image_height) {
                        ...
                        for (int r = 0; r < mcuRows; r++) {
                                ...
                                switch (info.layout) {
                                        ...
                                        case LayoutInterleavedUYVY:
                                        case LayoutInterleavedYUYV:
                                                deinterleave(srcY + line * strideInterleaved, yRowBufs[r].data(),
                        ...
                        if (info.layout != LayoutInterleavedUYVY && info.layout != LayoutInterleavedYUYV) {
                                for (int r = 0; r < chromaMcuRows; r++) {
                                        ...
                                        switch (info.layout) {
                                                case LayoutPlanar422:
                                                case LayoutPlanar420:
                                                        std::memcpy(cbRowBufs[r].data(), srcCb + chromaLine * strideCb,
                                                                    chromaWidth);
                                                        std::memcpy(crRowBufs[r].data(), srcCr + chromaLine * strideCr,
                                                                    chromaWidth);
                                                        break;
                                                case LayoutSemiPlanar420:
                                                        deinterleaveNV12(srcCbCr + chromaLine * strideCbCr,
                                                                         cbRowBufs[r].data(), crRowBufs[r].data(),
                                                                         chromaWidth);
                                                        break;
                                                default: break;
                                        }
                ...
                while (dinfo.output_scanline < dinfo.output_height) {
                        ...
                        for (int r = 0; r < chromaMcuRows; r++) {
                                ...
                                switch (info.layout) {
                                        case LayoutPlanar422:
                                        case LayoutPlanar420:
                                                std::memcpy(dstCb + chromaLine * strideCb, cbRowBufs[r].data(),
                                                            chromaWidth);
                                                std::memcpy(dstCr + chromaLine * strideCr, crRowBufs[r].data(),
                                                            chromaWidth);
                                                break;
                                        case LayoutSemiPlanar420:
                                                interleaveNV12(dstCbCr + chromaLine * strideCbCr, cbRowBufs[r].data(),
                                                               crRowBufs[r].data(), chromaWidth);
                                                break;
                                        default: break;
                                }
                        }
                        ...
                        if (info.layout == LayoutInterleavedUYVY || info.layout == LayoutInterleavedYUYV) {
                                for (int r = 0; r < mcuRows; r++) {
                                        int line = lumaStart + r;
                                        if (line >= height) break;
                                        int chromaRow = (mcuRows > 0) ? (r * chromaMcuRows / mcuRows) : 0;
                                        if (chromaRow >= chromaMcuRows) chromaRow = chromaMcuRows - 1;
                                        interleaveFn(dstY + line * strideOut, yRowBufs[r].data(),
                                                     cbRowBufs[chromaRow].data(), crRowBufs[chromaRow].data(), width);
                                }
                        }
                }
...
Error JpegVideoEncoder::submitPayload(const UncompressedVideoPayload::Ptr &payload) {
        ...
        if (static_cast<int>(_queue.size()) >= _capacity && !_capacityWarned) {
                promekiWarn("JpegVideoEncoder: output queue exceeded capacity (%d)", _capacity);
...
Error JpegVideoDecoder::submitPayload(const CompressedVideoPayload::Ptr &payload) {
        ...
        if (static_cast<int>(_queue.size()) >= _capacity && !_capacityWarned) {
                promekiWarn("JpegVideoDecoder: output queue exceeded capacity (%d)", _capacity);
                _capacityWarned = true;
        }
        ...
        struct JpegVideoCodecRegistrar {
                        JpegVideoCodecRegistrar() {
                                ...
                                VideoEncoder::registerBackend({
                                        ...
                                        .factory = []() -> VideoEncoder * { return new JpegVideoEncoder(); },
                                });
                                VideoDecoder::registerBackend({
                                        ...
                                        .factory = []() -> VideoDecoder * { return new JpegVideoDecoder(); },
                                });
                        }
        };
