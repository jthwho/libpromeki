...
MediaIO::FormatDesc MediaIOTask_RawBitstream::formatDesc() {
        return {"RawBitstream",
                ...
                []() -> MediaIOTask * { return new MediaIOTask_RawBitstream(); },
...
Error MediaIOTask_RawBitstream::executeCmd(MediaIOCommandOpen &cmd) {
        ...
        if (err.isError()) {
                promekiErr("MediaIOTask_RawBitstream: open '%s' for write failed: %s", _filename.cstr(),
                           err.name().cstr());
                return err;
        }
        ...
        for (const VideoPayload::Ptr &vp : frame.videoPayloads()) {
                ...
                for (size_t pi = 0; pi < cvp->planeCount(); ++pi) {
                        ...
                        if (n < 0 || static_cast<size_t>(n) != view.size()) {
                                promekiErr("MediaIOTask_RawBitstream: short write (%lld / %zu)", (long long)n,
                                           view.size());
                                stampWorkEnd();
...
Error MediaIOTask_RawBitstream::proposeInput(const MediaDesc &offered, MediaDesc *preferred) const {
