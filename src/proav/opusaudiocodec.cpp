...
bool isSupportedRate(float sr) {
        return sr == 8000.0f  || sr == 12000.0f || sr == 16000.0f ||
               sr == 24000.0f || sr == 48000.0f;
}
...
bool isSupportedFrameSizeMs(float ms) {
        return ms == 2.5f || ms == 5.0f  || ms == 10.0f ||
               ms == 20.0f || ms == 40.0f || ms == 60.0f;
}
        ...
        public:
                ...
                Error flush() override {
                        ...
                        if(_enc != nullptr) {
                                ...
                                if(_useFloat) {
                                        if(_pendingFloat.size() > 0 &&
                                           _pendingFloat.size() < needed) {
                                                _pendingFloat.resize(needed, 0.0f);
                                                flushOneFrameFloat();
                                        }
                                } else {
                                        if(_pendingS16.size() > 0 &&
                                           _pendingS16.size() < needed) {
        ...
        private:
                bool ensureEncoder(const AudioDesc &desc) {
                        ...
                        if(!isSupportedRate(desc.sampleRate())) {
                                setError(Error::Invalid,
                                         "Opus does not support this sample rate "
                        ...
                        if(ch != 1 && ch != 2) {
                                setError(Error::Invalid,
                                         "Opus encoder supports 1 or 2 channels only");
                                return false;
                        }
                        ...
                        _enc = opus_encoder_create(static_cast<opus_int32>(_sampleRate),
                                                   ...
                                                   applicationFromEnum(_application),
                                                   &err);
                        if(err != OPUS_OK || _enc == nullptr) {
                                setError(Error::LibraryFailure,
                                         String::sprintf("opus_encoder_create failed: %s",
                                                         opus_strerror(err)));
                                _enc = nullptr;
                                return false;
                        }
                ...
                MediaTimeStamp ptsForCurrentFrame() {
                        ...
                        int64_t ns = static_cast<int64_t>(
                                static_cast<double>(_samplesEmitted) *
                                1.0e9 / static_cast<double>(_sampleRate) + 0.5);
                ...
                void flushOneFrameS16() {
                        ...
                        if(wrote < 0) {
                                setError(Error::EncodeFailed,
                                         String::sprintf("opus_encode failed: %s",
                                                         opus_strerror(wrote)));
                                _pendingS16.erase(_pendingS16.begin(),
                                                  _pendingS16.begin() + consumed);
                                return;
                        }
                ...
                void flushOneFrameFloat() {
                        size_t n = framePcmSamples();
                        auto buf = Buffer::Ptr::create(kMaxPacketBytes);
                        opus_int32 wrote = opus_encode_float(
                                _enc,
                                _pendingFloat.data(),
                                static_cast<int>(n),
                                static_cast<unsigned char *>(buf.modify()->data()),
                        ...
                        if(wrote < 0) {
                                setError(Error::EncodeFailed,
                                         String::sprintf("opus_encode_float failed: %s",
                                                         opus_strerror(wrote)));
                                _pendingFloat.erase(_pendingFloat.begin(),
                                                    _pendingFloat.begin() + consumed);
                                return;
                        }
                        buf.modify()->setSize(static_cast<size_t>(wrote));
                        emitPacket(buf, n);
                ...
                Error submitPacket(const AudioPacket::Ptr &packet) override {
                        ...
                        if(decoded < 0) {
                                setError(Error::DecodeFailed,
                                         String::sprintf("opus_decode failed: %s",
        ...
        private:
                bool ensureDecoder() {
                        ...
                        if(_outChannels != 1 && _outChannels != 2) {
                                setError(Error::Invalid,
                                         "Opus decoder supports 1 or 2 channels only");
                                return false;
                        }
                        ...
                        _dec = opus_decoder_create(static_cast<opus_int32>(_outRate),
                                                   static_cast<int>(_outChannels),
                                                   &err);
                        if(err != OPUS_OK || _dec == nullptr) {
                                setError(Error::LibraryFailure,
                                         String::sprintf("opus_decoder_create failed: %s",
                                                         opus_strerror(err)));
                                _dec = nullptr;
                                return false;
                        }
