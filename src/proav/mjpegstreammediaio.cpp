...
PROMEKI_DEBUG(MjpegStreamMediaIO)

PROMEKI_REGISTER_MEDIAIO_FACTORY(MjpegStreamFactory)

const String MjpegStreamMediaIO::HttpBoundary("promeki-mjpeg");
const String MjpegStreamMediaIO::LatestJpegContentType("image/jpeg");
...
MjpegStreamFactory::Config::SpecMap MjpegStreamFactory::configSpecs() const {
...
MediaIO *MjpegStreamFactory::create(const Config &config, ObjectBase *parent) const {
        auto *io = new MjpegStreamMediaIO(parent);
        io->setConfig(config);
        return io;
}
...
MjpegStreamMediaIO::MjpegStreamMediaIO(ObjectBase *parent) : DedicatedThreadMediaIO(parent) {}

MjpegStreamMediaIO::~MjpegStreamMediaIO() {
        if (isOpen()) (void)close().wait();
        // Belt-and-braces: notify any stragglers and clear in case the
        // user destructed without a prior close() (or close() failed).
...
void MjpegStreamMediaIO::detachSubscriber(int id) {
...
MjpegStreamSnapshot MjpegStreamMediaIO::snapshot() const {
        Mutex::Locker lk(_stateMutex);
        return _stats;
}

bool MjpegStreamMediaIO::isStreaming() const {
        Mutex::Locker lk(_stateMutex);
        return _isStreaming;
}
...
Error MjpegStreamMediaIO::executeCmd(MediaIOCommandOpen &cmd) {
        ...
        if (error(encResult).isError()) {
                promekiErr("MjpegStreamMediaIO: createEncoder failed: %s", error(encResult).name().cstr());
        ...
        promekiInfo("MjpegStreamMediaIO: opened maxFps=%.4f quality=%d "
                    "ringDepth=%d",
...
Error MjpegStreamMediaIO::executeCmd(MediaIOCommandClose &cmd) {
        (void)cmd;
        ...
        const double avgUs = snap.encodeSamples > 0
                                     ...
                                     : 0.0;
        promekiInfo("MjpegStreamMediaIO: closed — encoded %lld, "
...
Error MjpegStreamMediaIO::executeCmd(MediaIOCommandStats &cmd) {
        MjpegStreamSnapshot snap;
        {
                Mutex::Locker lk(_stateMutex);
                snap = _stats;
...
Error MjpegStreamMediaIO::proposeInput(const MediaDesc &offered, MediaDesc *preferred) const {
...
namespace {
        ...
        class MjpegMultipartAdapter : public ObjectBase, public MjpegStreamSubscriber {
                public:
                        MjpegMultipartAdapter(MjpegStreamMediaIO *sink, AsyncBufferQueue *queue)
                        ...
                        void onFrame(const Buffer::Ptr &jpeg, const TimeStamp &) override {
                                ...
                                const String header =
                                        String("--") + MjpegStreamMediaIO::HttpBoundary +
                                        "\r\nContent-Type: " + MjpegStreamMediaIO::LatestJpegContentType +
                ...
                private:
                        MjpegStreamMediaIO *_sink = nullptr;
...
HttpHandlerFunc MjpegStreamMediaIO::buildMultipartHandler(MjpegStreamMediaIO *sink) {
        ...
        return [sink](const HttpRequest &, HttpResponse &res) {
                ...
                const String contentType =
                        String("multipart/x-mixed-replace; boundary=") + MjpegStreamMediaIO::HttpBoundary;
...
void MjpegStreamMediaIO::registerHttpRoute(HttpServer &server, const String &path) {
