...
static MediaIO::Config::SpecMap imageFileConfigSpecs() {
        ...
        s(MediaConfig::SequenceHead, int32_t(MediaIOTask_ImageFile::DefaultSequenceHead));
...
static MediaIO::FormatDesc buildFormatDesc(const String &name, const String &displayName, const String &description,
                                           StringList extensions, bool canBeSource, bool canBeSink) {
...
MediaIO::FormatDesc MediaIOTask_ImageFile::buildFormatDescFor(const ImageFileIO *io) {
        ...
        // ("DPX (.dpx)" etc).
        return buildFormatDesc(io->mediaIoName(), io->description(), io->description(), std::move(exts),
...
MediaIO::FormatDesc MediaIOTask_ImageFile::formatDesc() {
        ...
        return buildFormatDesc(String("ImageFile"), String("Image File"),
...
Error MediaIOTask_ImageFile::openSingle(MediaIOCommandOpen &cmd, MediaDesc &mediaDesc, const String &frSource) {
        const MediaIO::Config &cfg = cmd.config;
        ...
        if (_imageFileID == ImageFile::Invalid) {
                promekiErr("MediaIOTask_ImageFile: cannot determine image format for '%s'", _filename.cstr());
        ...
        if (cmd.mode == MediaIO::Source && !io->canLoad()) {
                promekiErr("MediaIOTask_ImageFile: backend '%s' does not support loading", io->name().cstr());
                return Error::NotSupported;
        }
        if (cmd.mode == MediaIO::Sink && !io->canSave()) {
                promekiErr("MediaIOTask_ImageFile: backend '%s' does not support saving", io->name().cstr());
                return Error::NotSupported;
        }
        ...
        if (cmd.mode == MediaIO::Source) {
                ...
                if (err.isError()) {
                        promekiErr("MediaIOTask_ImageFile: load '%s' failed: %s", _filename.cstr(), err.name().cstr());
                        return err;
                }
...
Error MediaIOTask_ImageFile::openSequence(MediaIOCommandOpen &cmd, MediaDesc &mediaDesc, const String &frSourceIn) {
        const MediaIO::Config &cfg = cmd.config;
        ...
        if (filenameIsImgSeqSidecar(_filename)) {
                ...
                if (sErr.isError() || !seq.isValid()) {
                        promekiErr("MediaIOTask_ImageFile: invalid .imgseq sidecar '%s'", _filename.cstr());
        ...
        if (!pattern.isValid()) {
                promekiErr("MediaIOTask_ImageFile: cannot parse sequence mask from '%s'", _filename.cstr());
                return Error::Invalid;
        }
        ...
        if (cmd.mode == MediaIO::Source) {
                ...
                Error err = imgFile.load(_ioConfig);
                if (err.isError()) {
                        promekiErr("MediaIOTask_ImageFile: failed to load head frame '%s': %s", firstPath.cstr(),
                                   err.name().cstr());
                        return err;
                }
                ...
                if (_sidecarAudioEnabled) {
                        ...
                        if (FilePath(audioPath).exists()) {
                                ...
                                if (!_sidecarAudio.isValid()) {
                                        promekiErr(
                                                "MediaIOTask_ImageFile: failed to create sidecar audio reader for '%s'",
                                                audioPath.cstr());
                                        return Error::NotSupported;
                                }
                ...
                if (_sidecarAudioEnabled) {
                        ...
                        if (audioDesc.isValid()) {
                                ...
                                if (!_sidecarAudio.isValid()) {
                                        promekiErr(
                                                "MediaIOTask_ImageFile: failed to create sidecar audio writer for '%s'",
                                                audioPath.cstr());
                                        return Error::NotSupported;
                                }
                                ...
                                if (audioErr.isError()) {
                                        promekiErr(
                                                "MediaIOTask_ImageFile: sidecar audio open '%s' for write failed: %s",
                                                audioPath.cstr(), audioErr.name().cstr());
                                        return audioErr;
                                }
                                _sidecarAudioDesc = audioDesc;
...
Error MediaIOTask_ImageFile::writeImgSeqSidecar() {
        ...
        if (err.isError()) {
                ...
                promekiInfo("MediaIOTask_ImageFile: wrote .imgseq sidecar '%s'", sidecarPath.toString().cstr());
        }
        return err;
}
...
Error MediaIOTask_ImageFile::readSequence(MediaIOCommandRead &cmd) {
        ...
        Error err = imgFile.load(_ioConfig);
        if (err.isError()) {
                promekiErr("MediaIOTask_ImageFile: failed to load sequence frame '%s': %s", fn.cstr(),
                           err.name().cstr());
                return err;
        }
        ...
        if (_sidecarAudioOpen) {
                ...
                if (audioErr.isError()) {
                        promekiErr("MediaIOTask_ImageFile: sidecar audio read failed: %s", audioErr.name().cstr());
                        return audioErr;
                }
...
Error MediaIOTask_ImageFile::writeSingle(MediaIOCommandWrite &cmd) {
        ...
        if (err.isError()) {
                promekiErr("MediaIOTask_ImageFile: save '%s' failed: %s", _filename.cstr(), err.name().cstr());
                return err;
        }
        ++_writeCount;
...
Error MediaIOTask_ImageFile::writeSequence(MediaIOCommandWrite &cmd) {
        ...
        Error err = imgFile.save(_ioConfig);
        if (err.isError()) {
                promekiErr("MediaIOTask_ImageFile: save sequence frame '%s' failed: %s", fn.cstr(), err.name().cstr());
                return err;
        }
...
Error MediaIOTask_ImageFile::executeCmd(MediaIOCommandSeek &cmd) {
        ...
        if (_sidecarAudioOpen) {
                ...
                if (audioErr.isError()) {
                        promekiErr("MediaIOTask_ImageFile: sidecar audio seek to sample %zu failed: %s", targetSample,
                                   audioErr.name().cstr());
                        return audioErr;
                }
        }
...
PixelFormat MediaIOTask_ImageFile::preferredWriterPixelFormat(const String &filename, const PixelFormat &source) const {
...
Error MediaIOTask_ImageFile::proposeInput(const MediaDesc &offered, MediaDesc *preferred) const {
...
namespace {
        ...
        int registerImageFileUmbrella() {
                ...
                MediaIO::registerFormat(
                        buildFormatDesc(String("ImageFile"), String("Image File"),
                                        String("Single-image files and image sequences (DPX, Cineon, TGA, SGI, "
                                               "PNM, PNG, JPEG, JPEG XS, RawYUV, .imgseq)"),
                                        buildExtensions(), true, true));
                ...
                MediaIO::registerFormat(
                        buildFormatDesc(String("ImgSeq"), String("Image Sequence (.imgseq)"),
