...
namespace promeki {
        namespace csc {
                namespace HWY_NAMESPACE {
                        ...
                        void MatrixMultiply3x3Impl(float *buf0, float *buf1, float *buf2, size_t width,
                                                   const float matrix[3][3], const float preOffset[3],
                                ...
                                for (; x < width; x++) {
                                        ...
                                        buf0[x] = matrix[0][0] * s0 + matrix[0][1] * s1 + matrix[0][2] * s2 +
                                                  postOffset[0];
                                        buf1[x] = matrix[1][0] * s0 + matrix[1][1] * s1 + matrix[1][2] * s2 +
                                                  postOffset[1];
                                        buf2[x] = matrix[2][0] * s0 + matrix[2][1] * s1 + matrix[2][2] * s2 +
                                                  postOffset[2];
                                }
                                return;
                        }
