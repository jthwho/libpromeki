                ...
                namespace HWY_NAMESPACE {
                        ...
                        static void packI4x8(const float *const *buffers, uint8_t *dst, size_t width) {
                                ...
                                for (; x + Nf <= width; x += Nf) {
                                        ...
                                        auto rf = hn::Min(hn::Max(hn::Add(hn::LoadU(df, buffers[0] + x), vhalf), vzero),
                                                          v255);
                                        auto gf = hn::Min(hn::Max(hn::Add(hn::LoadU(df, buffers[1] + x), vhalf), vzero),
                                                          v255);
                                        auto bf = hn::Min(hn::Max(hn::Add(hn::LoadU(df, buffers[2] + x), vhalf), vzero),
                                                          v255);
                                        auto af = hn::Min(hn::Max(hn::Add(hn::LoadU(df, buffers[3] + x), vhalf), vzero),
                                                          v255);
                        ...
                        void PackInterleavedImpl(const float *const *buffers, void *dst, size_t width, int compCount,
                                                 int bitsPerComp, int bytesPerBlock, int pixelsPerBlock,
                                                 const int *compByteOffset, const int *compBits, bool hasAlpha,
                                                 int alphaCompIndex, bool useSimd) {
                                ...
                                if (useSimd && pixelsPerBlock == 1 && compCount == 4 && bitsPerComp == 8 &&
                                    bytesPerBlock == 4 && compByteOffset[0] == 0 && compByteOffset[1] == 1 &&
                                ...
                                if (useSimd && pixelsPerBlock == 1 && compCount == 3 && bitsPerComp == 8 &&
                                    bytesPerBlock == 3 && compByteOffset[0] == 0 && compByteOffset[1] == 1 &&
                                    compByteOffset[2] == 2) {
                                ...
                                if (pixelsPerBlock == 1 && compCount == 3 && bitsPerComp == 10 && bytesPerBlock == 4) {
                                        bool methodB =
                                                (compByteOffset[0] != 0 && compByteOffset[0] == compByteOffset[1] &&
                                        ...
                                        for (size_t x = 0; x < width; x++) {
                                                ...
                                                uint32_t word = methodB ? (c0 | (c1 << 10) | (c2 << 20))
                                                                        : ((c0 << 22) | (c1 << 12) | (c2 << 2));
                                ...
                                if (pixelsPerBlock == 1) {
                                        for (size_t x = 0; x < width; x++) {
                                                ...
                                                for (int c = 0; c < compCount; c++) {
                                                        packSample(pixel, compByteOffset[c], compBits[c],
                                                                   buffers[c][x]);
                                                }
                                        }
                                } else {
                        ...
                        void PackPlanarImpl(const float *const *buffers, void *const *planes, const size_t *strides,
                                            size_t width, int compCount, int bitsPerComp, const int *compPlane,
                                            const int *planeHSub, const int *planeBytesPerSample, bool hasAlpha,
                                            int alphaCompIndex, bool useSimd) {
                                for (int c = 0; c < compCount; c++) {
                        ...
                        void PackSemiPlanarImpl(const float *const *buffers, void *const *planes, const size_t *strides,
