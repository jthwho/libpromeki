...
namespace promeki {
        namespace csc {
                ...
                void packInterleaved(const float *const *buffers, void *dst, size_t width, int compCount,
                                     ...
                                     const int *compBits, bool hasAlpha, int alphaCompIndex, bool useSimd) {
                        HWY_DYNAMIC_DISPATCH(PackInterleavedImpl)(buffers, dst, width, compCount, bitsPerComp,
                                                                  bytesPerBlock, pixelsPerBlock, compByteOffset,
                                                                  compBits, hasAlpha, alphaCompIndex, useSimd);
                        return;
                }
                ...
                void packPlanar(const float *const *buffers, void *const *planes, const size_t *strides, size_t width,
                                ...
                                const int *planeBytesPerSample, bool hasAlpha, int alphaCompIndex, bool useSimd) {
                        HWY_DYNAMIC_DISPATCH(PackPlanarImpl)(buffers, planes, strides, width, compCount, bitsPerComp,
                                                             compPlane, planeHSub, planeBytesPerSample, hasAlpha,
                                                             alphaCompIndex, useSimd);
                        return;
                }
                ...
                void packSemiPlanar(const float *const *buffers, void *const *planes, const size_t *strides,
                                    ...
                                    bool hasAlpha, int alphaCompIndex, const int *compByteOffset, bool useSimd) {
                        static const int defaultOffsets[3] = {0, 0, 1};
                        const int       *offsets = compByteOffset ? compByteOffset : defaultOffsets;
                        HWY_DYNAMIC_DISPATCH(PackSemiPlanarImpl)(buffers, planes, strides, width, bitsPerComp,
                                                                 planeHSub, planeBytesPerSample, hasAlpha,
                                                                 alphaCompIndex, offsets, useSimd);
                        return;
                }
