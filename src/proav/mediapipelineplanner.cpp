...
Error topologicallySort(const MediaPipelineConfig &config,
...
bool findSingleBridge(const MediaDesc &from,
                      const MediaDesc &to,
                      const MediaPipelinePlanner::Policy &policy,
                      BridgeStep *out,
                      promeki::List<BridgeTraceEntry> *trace = nullptr) {
        ...
        for(const auto &fd : formats) {
                ...
                const int cost = MediaPipelinePlanner::adjustCostForQuality(
                        rawCost, policy.quality);
                entry.adjustedCost = cost;

                // ZeroCopyOnly: hard reject anything beyond the
                // metadata / lossless-conversion bands.
                if(policy.quality == MediaPipelinePlanner::Quality::ZeroCopyOnly
                   && rawCost > 100) {
...
bool findCodecTransitive(const MediaDesc &from,
                         const MediaDesc &to,
                         const MediaPipelinePlanner::Policy &policy,
...
void appendBridgeTrace(String *diagnostic,
                       const promeki::List<BridgeTraceEntry> &trace) {
        if(diagnostic == nullptr) return;
        if(trace.isEmpty()) {
...
Error resolveRoute(const MediaPipelineConfig::Route &route,
                   MediaIO *fromStage,
                   MediaIO *toStage,
        ...
        if(toStage == nullptr) {
                appendDiagnostic(diagnostic,
                        String("MediaPipelinePlanner: stage '") + route.to
                                + "' is missing.");
        ...
        MediaDesc target = (pe == Error::Ok && preferred.isValid())
                ? preferred
                : producedDesc;
        ...
        bool single = findSingleBridge(producedDesc, target, policy,
                                       &singleAttempt, &trace);
        if(single) {
                chain.pushToBack(singleAttempt);
        } else if(policy.maxBridgeDepth >= 2 &&
                  findCodecTransitive(producedDesc, target, policy, &chain)) {
                // Two-hop succeeded after the single-hop search filled
                // the trace; fall through to splice.
        } else {
                appendDiagnostic(diagnostic,
                        String("MediaPipelinePlanner: route '") + route.from
                                + "' -> '" + route.to
                                + "' has a format gap and no bridge chain fits.");
                appendDiagnostic(diagnostic,
                        String("    upstream produced: ") + shortMediaDescLine(producedDesc));
                appendDiagnostic(diagnostic,
                        String("    sink preferred:    ") + shortMediaDescLine(target));
                appendBridgeTrace(diagnostic, trace);
                if(producedDesc.imageList().isEmpty()
                   || target.imageList().isEmpty()) {
                        appendDiagnostic(diagnostic,
                        ...
                        // to invent one.
                        const bool rateDiffers  = producedDesc.frameRate()
                                != target.frameRate();
                        const bool pixelDiffers = fromPd != toPd;
                        if(rateDiffers && pixelDiffers
                           && !fromPd.isCompressed() && !toPd.isCompressed()) {
                                appendDiagnostic(diagnostic,
        ...
        if(static_cast<int>(chain.size()) > policy.maxBridgeDepth) {
                appendDiagnostic(diagnostic,
                        String("MediaPipelinePlanner: bridge chain for '")
                                + route.from + "' -> '" + route.to
                                + "' exceeds maxBridgeDepth (" + String::number(chain.size())
...
// instantiate.
bool buildStageMap(const MediaPipelineConfig &config,
                   const MediaPipelinePlanner::InjectedStages &injected,
...
void destroyOwnedStages(promeki::Map<String, MediaIO *> &stages,
                        const promeki::Set<String> &ownedNames) {
        for(auto it = stages.begin(); it != stages.end(); ++it) {
...
bool MediaPipelinePlanner::isResolved(const MediaPipelineConfig &config,
                                      String *diagnostic) {
        ...
        promeki::Set<String> ownedNames;
        String failedName;
        if(!buildStageMap(config, InjectedStages(), &stages, &ownedNames,
                          &failedName)) {
                destroyOwnedStages(stages, ownedNames);
                if(diagnostic != nullptr) {
                        *diagnostic = String("MediaPipelinePlanner: cannot instantiate '")
                                + failedName + "'.";
                }
                return false;
        }
        ...
        for(const auto &r : config.routes()) {
                ...
                const MediaDesc fromDesc = producedBy.contains(r.from)
                        ? producedBy[r.from] : MediaDesc();
                MediaDesc preferred;
                Error pe = to->proposeInput(fromDesc, &preferred);
                if(pe.isError() || !preferred.formatEquals(fromDesc)) {
                        if(diagnostic != nullptr) {
                                *diagnostic = String("MediaPipelinePlanner: route '")
                                        + r.from + "' -> '" + r.to
...
Error MediaPipelinePlanner::plan(const MediaPipelineConfig &in,
                                 MediaPipelineConfig       *out,
                                 const Policy              &policy,
                                 String                    *diagnostic) {
...
Error MediaPipelinePlanner::plan(const MediaPipelineConfig &in,
        ...
        *out = MediaPipelineConfig();
        ...
        if(vErr.isError()) {
                if(diagnostic != nullptr) {
                        *diagnostic = String("MediaPipelinePlanner: input config invalid (")
                                + vErr.name() + ").";
                }
                return vErr;
        }
                ...
                if(!src.isValid()) {
                        if(diagnostic != nullptr) {
                                *diagnostic = String("MediaPipelinePlanner: cannot determine "
                                        "produced MediaDesc for source '") + name
                                        + "' (no describe(), expectedDesc(), or open()-able shape).";
                        }
                        destroyOwnedStages(stages, ownedNames);
                        return Error::NotSupported;
                }
        ...
        for(size_t i = 0; i < order.size(); ++i) {
                ...
                for(size_t j = 0; j < routes.size(); ++j) {
                        ...
                        const MediaDesc producedDesc = producedBy.contains(fromName)
                                ? producedBy[fromName] : MediaDesc();
                        MediaDesc postDesc;
                        Error rerr = resolveRoute(r, stages[r.from], stages[r.to],
                                                  producedDesc, policy, out,
                                                  &bridgeCounter, &postDesc,
                                                  diagnostic);
                        if(rerr.isError()) {
                                destroyOwnedStages(stages, ownedNames);
                                *out = MediaPipelineConfig();
