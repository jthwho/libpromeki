...
struct FrameBridge::Impl {
        ...
        // Runtime state
        // ====================================================================
        enum Role { RoleNone, RoleOutput, RoleInput };
        ...
        // -------------- Path helpers --------------
        static String makeShmName(const String &logical) {
                return String("/promeki-fb-") + logical;
        }
        static String makeSocketPath(const String &logical) {
                return Dir::ipc().path()
                        .join(String("promeki-fb-") + logical + String(".sock"))
                        .toString();
        }
        ...
        size_t audioBytesTotal() const {
                return audioDesc.bufferSize(audioCapacitySamples);
        }

        void computeSlotLayout() {
                slotOff.metadataOff = roundUp(48, SlotAlign);
                slotOff.imagesOff   = roundUp(
                        slotOff.metadataOff + metadataReserveBytes, SlotAlign);
                slotOff.audioOff    = roundUp(
                        slotOff.imagesOff + imageBytesTotal, SlotAlign);
        ...
        uint64_t computeConfigHash() const {
                ...
                ws << mediaDesc << audioDesc
                   << static_cast<uint32_t>(ringDepth)
        ...
        uint8_t *slotBase(uint64_t index) {
                ...
                const BridgeHeader *hdr =
                        reinterpret_cast<const BridgeHeader *>(base);
                return base + hdr->slotsOffset +
                       static_cast<size_t>(index) *
                       static_cast<size_t>(hdr->slotStride);
        }
        const uint8_t *slotBase(uint64_t index) const {
                const uint8_t *base =
                        static_cast<const uint8_t *>(shm.data());
                if(base == nullptr) return nullptr;
                const BridgeHeader *hdr =
                        reinterpret_cast<const BridgeHeader *>(base);
                return base + hdr->slotsOffset +
        ...
        Error handshakeWithClient(LocalSocket &c, bool &syncModeOut) {
                ...
                uint8_t  peerSync = 1;
                {
                        ...
                        dev.open(IODevice::ReadOnly);
                        DataStream rs = DataStream::createReader(&dev);
                        rs >> peerMajor >> peerMinor >> peerBuild
                           >> wantHash >> peerSync;
                ...
                if(peerMajor != WireMajor) {
                        sendReject(w, 2, String::sprintf(
                                "wire major mismatch: peer=%u.%u, us=%u.%u",
                                peerMajor, peerMinor,
                                WireMajor, WireMinor));
                        return Error::NotSupported;
                }
                ...
                Buffer acptBuf(512);
                {
                        ...
                        dev.open(IODevice::ReadWrite);
                        DataStream ws = DataStream::createWriter(&dev);
                        ws << static_cast<uint32_t>(WireMajor)
                           << static_cast<uint32_t>(WireMinor)
                           << buildInfoString()
                           << shmName
                           << static_cast<uint64_t>(shm.size())
                           << configHash
                           << uuid
                           << static_cast<uint32_t>(ringDepth)
                           << static_cast<uint64_t>(slotStride)
                           << mediaDesc << audioDesc;
                        if(ws.status() != DataStream::Ok) {
                                return ws.toError();
                        }
                        size_t n = static_cast<size_t>(dev.pos());
                        err = w.writeFrame(KeyACPT, acptBuf.data(),
        ...
        Error handshakeAsInput(const String &logicalName) {
                ...
                Buffer buf(256);
                {
                        ...
                        ws << static_cast<uint32_t>(WireMajor)
                           << static_cast<uint32_t>(WireMinor)
                           << buildInfoString()
                           << static_cast<uint64_t>(0) // wantHash=0 → accept any
                           << static_cast<uint8_t>(inputSyncMode ? 1 : 0);
                        size_t n = static_cast<size_t>(dev.pos());
                        Error err = w.writeFrame(KeyHELO, buf.data(),
                                                 static_cast<uint32_t>(n));
                        if(err.isError()) return err;
                }
                ...
                if(resp.key == KeyRJCT) {
                        ...
                        DataStream rs = DataStream::createReader(&dev);
                        rs >> code >> reason;
                        promekiWarn("FrameBridge: rejected: code=%u, %s",
                                    code, reason.cstr());
                        return Error::NotSupported;
                }
                ...
                UUID peerUuid;
                {
                        ...
                        DataStream rs = DataStream::createReader(&dev);
                        rs >> peerMajor >> peerMinor >> peerBuild
                           >> shmNameIn >> shmSize >> hash >> peerUuid
                           >> depth >> stride
                           >> mediaDesc >> audioDesc;
                ...
                const BridgeHeader *hdr =
                        reinterpret_cast<const BridgeHeader *>(shm.data());
        ...
        Error writeSlot(uint64_t index, const Frame &frame) {
                ...
                uint64_t seq = std::atomic_ref<uint64_t>(
                        *reinterpret_cast<uint64_t *>(base + slotOff.seqOff))
                ...
                size_t metaBytes = 0;
                {
                        ...
                        std::memcpy(base + slotOff.metadataOff, tmp.data(),
                                    metaBytes);
                }
                put32(slotOff.metadataSizeOff,
        ...
        Frame::Ptr readSlot(uint64_t index, Error *errOut) {
                ...
                for(int attempt = 0; attempt < 4; ++attempt) {
                        ...
                        std::memcpy(&audioSamples,
                                    base + slotOff.audioSampleCtOff, 8);
                        if(metaSize > metadataReserveBytes ||
                           audioSamples > audioCapacitySamples) {
                                continue;
                        }
        ...
        // -------------- TICK emission --------------
        void emitTick(LocalSocket &c, uint32_t slot, uint64_t seq,
                      uint64_t frameNumber, int64_t timestampNs) {
        ...
        void waitForAcks(uint64_t seq) {
                for(auto it = clients.begin(); it != clients.end(); ) {
                        ...
                        const int maxAttempts = std::max(
                                1, static_cast<int>(SyncAckTimeoutMs / pollMs));
                        for(int tries = 0;
                            tries < maxAttempts && !acked && !dead; ++tries) {
...
Error FrameBridge::openOutput(const String &name, const Config &config) {
        ...
        _d->audioCapacitySamples = _d->worstCaseAudioSamples(
                config.audioHeadroomFraction);

        Error err = _d->computeGeometry();
        if(err.isError()) { _d->role = Impl::RoleNone; return err; }
        ...
        size_t totalBytes = sizeof(Impl::BridgeHeader)
                + configBlobReserve
                + static_cast<size_t>(_d->ringDepth) * _d->slotStride;
        totalBytes = roundUp(totalBytes, SlotAlign);

        err = _d->shm.create(_d->shmName, totalBytes,
                             config.accessMode, config.groupName);
        if(err.isError()) {
                _d->role = Impl::RoleNone;
                return err;
        }
        ...
        hdr->metadataReserveBytes =
                static_cast<uint32_t>(_d->metadataReserveBytes);
        ...
        size_t used = 0;
        err = _d->writeConfigBlob(
                static_cast<uint8_t *>(_d->shm.data()) + hdr->configBlobOffset,
                configBlobReserve, used);
        if(err.isError()) {
                close();
                return err;
        }
        ...
        Dir::ipc().mkpath();
        err = _d->server.listen(_d->socketPath, config.accessMode,
                                config.groupName);
        if(err.isError()) {
                close();
                return err;
...
const UUID &FrameBridge::uuid() const { return _d->uuid; }
const String &FrameBridge::name() const { return _d->name; }
const MediaDesc &FrameBridge::mediaDesc() const { return _d->mediaDesc; }
const AudioDesc &FrameBridge::audioDesc() const { return _d->audioDesc; }
int FrameBridge::ringDepth() const { return _d->ringDepth; }
...
Error FrameBridge::writeFrame(const Frame::Ptr &frame) {
        ...
        if(_d->clients.empty()) {
                if(_d->waitForConsumer) {
                        ...
                        // another thread.
                        while(_d->role == Impl::RoleOutput &&
                              !_d->abortFlag.load(std::memory_order_acquire) &&
                              _d->clients.empty()) {
        ...
        for(auto it = _d->clients.begin(); it != _d->clients.end(); ) {
                if(it->sock && it->sock->isConnected()) {
                        _d->emitTick(*it->sock, static_cast<uint32_t>(idx),
                                     _d->currentSeq, _d->nextFrameNumber,
                                     tsNs);
                        ++it;
                } else {
...
Frame::Ptr FrameBridge::readFrame(Error *err) {
        ...
        while(_d->client.bytesAvailable() >=
              static_cast<int64_t>(TickPayloadBytes + 8)) {
                ...
                _d->lastTickSeq = seq;
                _d->lastTickFrame = fn;
                _d->lastTickTs = TimeStamp(TimeStamp::Value(
                        std::chrono::nanoseconds(tsNs)));
                _d->inputTicked = true;
