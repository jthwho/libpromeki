...
List<int> AudioDecoder::supportedOutputsFor(AudioCodec::ID codecId, AudioCodec::Backend pinned) {
        auto                     &reg = registry();
        ReadWriteLock::ReadLocker lock(reg.mutex);
        auto                      it = reg.entries.find(codecId);
...
Result<AudioDecoder *> AudioDecoder::create(AudioCodec::ID codecId, AudioCodec::Backend pinned,
