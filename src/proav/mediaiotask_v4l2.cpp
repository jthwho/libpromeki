...
static void applyV4l2Controls(int fd, const MediaIO::Config &cfg) {
        ...
        for (const auto &rc : controls) {
                ...
                if (xioctl(fd, VIDIOC_S_CTRL, &ctrl) < 0) {
                        promekiWarn("MediaIOTask_V4L2: failed to set %s=%d: %s", rc.v4l2Name.cstr(), val,
                                    std::strerror(errno));
                }
        }
}
...
static String findPairedAlsaDevice(const String &v4l2DevPath) {
        ...
        if (ec) {
                promekiDebug("MediaIOTask_V4L2: auto-detect: cannot resolve %s: %s", v4l2SysfsLink.c_str(),
                             ec.message().c_str());
                return String();
        }
...
static List<MediaDesc> v4l2QueryDevice(const MediaIO::Config &config) {
        ...
        while (xioctl(fd, VIDIOC_ENUM_FMT, &fmtdesc) == 0) {
                PixelFormat::ID pdId = MediaIOTask_V4L2::v4l2ToPixelFormat(fmtdesc.pixelformat);
...
MediaIO::FormatDesc MediaIOTask_V4L2::formatDesc() {
        return {"V4L2",
                ...
                []() -> MediaIOTask * { return new MediaIOTask_V4L2(); },
                []() -> MediaIO::Config::SpecMap {
                        ...
                        for (int i = 0; i < ControlMapCount; i++) {
                                ...
                                if (gs != nullptr && gs->acceptsType(Variant::TypeEnum) &&
...
Error MediaIOTask_V4L2::openVideo(const MediaIO::Config &cfg) {
        ...
        if (_fd < 0) {
                promekiErr("MediaIOTask_V4L2: cannot open %s: %s", devPath.cstr(), std::strerror(errno));
        ...
        if (xioctl(_fd, VIDIOC_QUERYCAP, &cap) < 0) {
                promekiErr("MediaIOTask_V4L2: VIDIOC_QUERYCAP failed on %s", devPath.cstr());
                return Error::DeviceError;
        }
        if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)) {
                promekiErr("MediaIOTask_V4L2: %s does not support video capture", devPath.cstr());
                return Error::NotSupported;
        }
        if (!(cap.capabilities & V4L2_CAP_STREAMING)) {
                promekiErr("MediaIOTask_V4L2: %s does not support streaming I/O", devPath.cstr());
                return Error::NotSupported;
        }

        promekiDebug("MediaIOTask_V4L2: device %s  driver=%s  card=%s  bus=%s  caps=0x%08x", devPath.cstr(),
        ...
        if (xioctl(_fd, VIDIOC_S_PARM, &parm) < 0) {
                promekiWarn("MediaIOTask_V4L2: VIDIOC_S_PARM failed: %s (using device default)", std::strerror(errno));
        }
        ...
        if (xioctl(_fd, VIDIOC_REQBUFS, &req) < 0) {
                promekiErr("MediaIOTask_V4L2: VIDIOC_REQBUFS failed: %s", std::strerror(errno));
                return Error::DeviceError;
        }
        if (req.count < 2) {
                promekiErr("MediaIOTask_V4L2: insufficient buffer memory (got %u)", req.count);
                return Error::NoMem;
        }
        ...
        for (uint32_t i = 0; i < req.count; i++) {
                ...
                if (_buffers[static_cast<int>(i)].start == MAP_FAILED) {
                        ...
                        promekiErr("MediaIOTask_V4L2: mmap failed for buffer %u: %s", i, std::strerror(errno));
                        return Error::NoMem;
                }
        }

        promekiDebug("MediaIOTask_V4L2: mapped %zu MMAP buffers (%zu bytes each)", _buffers.size(),
...
Error MediaIOTask_V4L2::startStreaming() {
        ...
        if (xioctl(_fd, VIDIOC_STREAMON, &type) < 0) {
                promekiErr("MediaIOTask_V4L2: VIDIOC_STREAMON failed: %s", std::strerror(errno));
                return Error::DeviceError;
        }
        _streaming = true;
        promekiDebug("MediaIOTask_V4L2: STREAMON — capture started with %zu buffers", _buffers.size());
        return Error::Ok;
}
...
Error MediaIOTask_V4L2::openAudio(const MediaIO::Config &cfg) {
        ...
        if (err < 0) {
                promekiErr("MediaIOTask_V4L2: snd_pcm_open(%s) failed: %s", alsaDev.cstr(), snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        if (err < 0) {
                promekiErr("MediaIOTask_V4L2: cannot set interleaved access: %s", snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        if (err < 0) {
                ...
                if (err < 0) {
                        promekiErr("MediaIOTask_V4L2: cannot set PCM format: %s", snd_strerror(err));
                        return Error::DeviceError;
                }
        ...
        if (err < 0) {
                promekiErr("MediaIOTask_V4L2: cannot set sample rate %u: %s", rate, snd_strerror(err));
                return Error::DeviceError;
        }

        err = snd_pcm_hw_params_set_channels(_pcm, hwparams, static_cast<unsigned int>(channels));
        if (err < 0) {
                promekiErr("MediaIOTask_V4L2: cannot set %d channels: %s", channels, snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        if (err < 0) {
                promekiErr("MediaIOTask_V4L2: snd_pcm_hw_params failed: %s", snd_strerror(err));
                return Error::DeviceError;
        }
        ...
        if (err < 0) {
                promekiErr("MediaIOTask_V4L2: snd_pcm_prepare failed: %s", snd_strerror(err));
                return Error::DeviceError;
        }
...
void MediaIOTask_V4L2::videoCaptureLoop() {
        ...
        while (!_stopFlag.load(std::memory_order_acquire)) {
                ...
                if (pr < 0) {
                        ...
                        promekiErr("MediaIOTask_V4L2: poll failed: %s", std::strerror(errno));
                ...
                if (xioctl(_fd, VIDIOC_DQBUF, &vbuf) < 0) {
                        ...
                        int e = errno;
                        promekiErr("MediaIOTask_V4L2: VIDIOC_DQBUF failed: %s", std::strerror(e));
                ...
                if (xioctl(_fd, VIDIOC_QBUF, &vbuf) < 0) {
                        int e = errno;
                        promekiErr("MediaIOTask_V4L2: VIDIOC_QBUF re-queue failed: %s", std::strerror(e));
                        if (e == ENODEV) {
                                _deviceError.store(e, std::memory_order_release);
                                break;
                        }
...
void MediaIOTask_V4L2::audioCaptureLoop() {
        ...
        while (!_stopFlag.load(std::memory_order_acquire)) {
                ...
                if (rd < 0) {
                        ...
                        promekiWarn("MediaIOTask_V4L2: snd_pcm_readi error: %s", snd_strerror(static_cast<int>(rd)));
                        snd_pcm_prepare(_pcm);
                        continue;
                }
                if (rd > 0) {
...
Error MediaIOTask_V4L2::executeCmd(MediaIOCommandRead &cmd) {
        ...
        if (devErr != 0) {
                promekiErr("MediaIOTask_V4L2: device error: %s", std::strerror(devErr));
                return Error::DeviceError;
        }
        ...
        if (_audioEnabled) {
                ...
                if (_frameCount == 0) {
                        ...
                        if (serr < 0) {
                                promekiErr("MediaIOTask_V4L2: snd_pcm_start failed: %s", snd_strerror(serr));
                                return Error::DeviceError;
                        }
                        _audioThread = std::thread(&MediaIOTask_V4L2::audioCaptureLoop, this);
