...
static void extractCineonMetadata(Metadata &meta, const CineonHeader *hdr) {
        ...
        if (hdr->file.date[0] != 0 && static_cast<uint8_t>(hdr->file.date[0]) != 0xFF) {
                char datetime[32];
                std::snprintf(datetime, sizeof(datetime), "%.12s %.12s", hdr->file.date, hdr->file.time);
                meta.set(Metadata::Date, String(datetime));
        }

        // Film info
        if (hdr->film.id != 0xFF) meta.set(Metadata::FilmMfgID, String(1, static_cast<char>('0' + hdr->film.id)));
        if (hdr->film.type != 0xFF) meta.set(Metadata::FilmType, String(1, static_cast<char>('0' + hdr->film.type)));
        if (hdr->film.perf != 0xFF) meta.set(Metadata::FilmOffset, String(1, static_cast<char>('0' + hdr->film.perf)));
        ...
        if (hdr->film.position != 0xFFFFFFFF) meta.set(Metadata::FilmSeqPos, static_cast<int>(hdr->film.position));
        ...
        if (gammaBits != 0xFFFFFFFF && !std::isinf(gamma)) meta.set(Metadata::Gamma, static_cast<double>(gamma));
}
...
Error ImageFileIO_Cineon::load(ImageFile &imageFile, const MediaConfig &config) const {
        ...
        if (hdr.img.nchan != 3 || hdr.img.chan[0].bpp != 10 || hdr.fmt.packing != 5) {
                promekiErr("Cineon load '%s': unsupported format (nchan=%d, bpp=%d, packing=%d)", filename.cstr(),
                           hdr.img.nchan, hdr.img.chan[0].bpp, hdr.fmt.packing);
                file.close();
        ...
        file.close();
        if (err.isError()) {
                promekiErr("Cineon load '%s': read image data failed: %s", filename.cstr(), err.name().cstr());
                return err;
        }
        ...
        std::memcpy(payload.modify()->data()[0].data(), imgBuf.data(), imageBytes);
