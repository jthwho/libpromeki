                ...
                for (int i = 0; i < stages.size(); ++i) {
                        ...
                        if (sj.valueIsObject("config")) {
                                ...
                                cj.forEach([&s, &good](const String &key, const Variant &val) {
                                        ...
                                        if (serr.isError()) {
                                                promekiWarn("MediaPipelineConfig::fromJson: stage '%s' config key '%s' "
                                                            "rejected: %s",
...
Error MediaPipelineConfig::saveToFile(const FilePath &path, unsigned int indent) const {
        ...
        if (err.isError()) {
                promekiErr("MediaPipelineConfig::saveToFile: open '%s' failed: %s", path.toString().cstr(),
                           err.desc().cstr());
                return err;
        }
        ...
        if (n != expected) {
                promekiErr("MediaPipelineConfig::saveToFile: short write to '%s' (%lld of %zu bytes)",
                           path.toString().cstr(), static_cast<long long>(n), text.size());
...
MediaPipelineConfig MediaPipelineConfig::loadFromFile(const FilePath &path, Error *err) {
        ...
        if (oerr.isError()) {
                promekiErr("MediaPipelineConfig::loadFromFile: open '%s' failed: %s", path.toString().cstr(),
                           oerr.desc().cstr());
                if (err) *err = oerr;
        ...
        f.close();
        if (nread <= 0) {
                promekiErr("MediaPipelineConfig::loadFromFile: '%s' is empty or unreadable", path.toString().cstr());
        ...
        if (parseErr.isError()) {
                promekiErr("MediaPipelineConfig::loadFromFile: '%s' is not valid JSON", path.toString().cstr());
                if (err) *err = parseErr;
                return MediaPipelineConfig();
        }
...
namespace {
        ...
        Error validateStageConfig(const MediaPipelineConfig::Stage &stage) {
                ...
                stage.config.forEach([&](MediaConfig::ID id, const Variant &val) {
                        ...
                        if (it != backendSpecs.end())
                                spec = &it->second;
                        else
                                spec = MediaConfig::spec(id);
                        ...
                        if (!spec->validate(val, &specErr)) {
                                promekiWarn("MediaPipelineConfig: stage '%s' key '%s' fails spec (%s).",
                                            stage.name.cstr(), id.name().cstr(), specErr.name().cstr());
...
MediaPipelineConfig MediaPipelineConfig::resolved(Error *err, String *diagnostic) const {
...
Error MediaPipelineConfig::validate() const {
        ...
        for (size_t i = 0; i < _stages.size(); ++i) {
                ...
                if (seenNames.contains(s.name)) {
                        promekiErr("MediaPipelineConfig::validate: duplicate stage name '%s'.", s.name.cstr());
                        return Error::InvalidArgument;
                }
        ...
        for (size_t i = 0; i < _routes.size(); ++i) {
                ...
                if (!seenNames.contains(r.from)) {
                        promekiErr("MediaPipelineConfig::validate: route '%s→%s' from-stage not found.", r.from.cstr(),
                                   r.to.cstr());
                        return Error::InvalidArgument;
                }
                if (!seenNames.contains(r.to)) {
                        promekiErr("MediaPipelineConfig::validate: route '%s→%s' to-stage not found.", r.from.cstr(),
                                   r.to.cstr());
                        return Error::InvalidArgument;
                }
                if (r.from == r.to) {
                        promekiErr("MediaPipelineConfig::validate: route '%s→%s' is a self-loop.", r.from.cstr(),
                                   r.to.cstr());
                        return Error::InvalidArgument;
                }
...
DataStream &operator<<(DataStream &stream, const MediaPipelineConfig::Stage &s) {
        stream.beginFrame(DataTypeMediaPipelineStage, 1);
        stream << s.name;
        stream << s.type;
        stream << s.path;
...
DataStream &operator>>(DataStream &stream, MediaPipelineConfig::Stage &s) {
        if (!stream.readFrame(DataTypeMediaPipelineStage)) {
...
DataStream &operator<<(DataStream &stream, const MediaPipelineConfig::Route &r) {
        stream.beginFrame(DataTypeMediaPipelineRoute, 1);
        stream << r.from;
        stream << r.to;
        stream << r.fromTrack;
        stream << r.toTrack;
...
DataStream &operator>>(DataStream &stream, MediaPipelineConfig::Route &r) {
        if (!stream.readFrame(DataTypeMediaPipelineRoute)) {
...
DataStream &operator<<(DataStream &stream, const MediaPipelineConfig &c) {
        stream.beginFrame(DataTypeMediaPipelineConfig, 1);
...
DataStream &operator>>(DataStream &stream, MediaPipelineConfig &c) {
        if (!stream.readFrame(DataTypeMediaPipelineConfig)) {
