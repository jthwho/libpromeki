...
ImgSeq ImgSeq::load(const FilePath &path, Error *err) {
        ...
        if (openErr.isError()) {
                promekiErr("ImgSeq::load: failed to open '%s': %s", path.toString().cstr(), openErr.name().cstr());
                if (err) *err = openErr;
                return ret;
        }
        ...
        if (parseErr.isError()) {
                promekiErr("ImgSeq::load: '%s' is not a valid JSON object", path.toString().cstr());
...
ImgSeq ImgSeq::fromJson(const JsonObject &json, Error *err) {
        ...
        if (json.getString(kFieldType) != String(TypeTag)) {
                promekiErr("ImgSeq::fromJson: '%s' field is not '%s'", kFieldType, TypeTag);
                if (err) *err = Error::Invalid;
                return ret;
        }
        ...
        if (!nn.isValid()) {
                promekiErr("ImgSeq::fromJson: '%s' is not a valid pattern", nameStr.cstr());
                if (err) *err = Error::Invalid;
                return ret;
        }
        ret.setName(nn);
        ...
        if (json.contains(kFieldFrameRate)) {
                ...
                if (r.second().isOk()) {
                        ...
                        promekiWarn("ImgSeq::fromJson: invalid '%s' value '%s'", kFieldFrameRate, frStr.cstr());
                }
        }
        ...
        if (json.contains(kFieldVideoSize)) {
                ...
                if (r.second().isOk()) {
                        ...
                        promekiWarn("ImgSeq::fromJson: invalid '%s' value '%s'", kFieldVideoSize, szStr.cstr());
                }
        }
        ...
        if (json.contains(kFieldPixelFormat)) {
                ...
                if (pd.isValid()) {
                        ...
                        promekiWarn("ImgSeq::fromJson: invalid '%s' value '%s'", kFieldPixelFormat, pdStr.cstr());
                }
        }
...
Error ImgSeq::save(const FilePath &path) const {
        ...
        if (err.isError()) {
                promekiErr("ImgSeq::save: failed to open '%s': %s", path.toString().cstr(), err.name().cstr());
                return err;
        }
        int64_t n = f.write(text.cstr(), static_cast<int64_t>(text.size()));
        f.close();
        if (n != static_cast<int64_t>(text.size())) {
                promekiErr("ImgSeq::save: short write to '%s'", path.toString().cstr());
