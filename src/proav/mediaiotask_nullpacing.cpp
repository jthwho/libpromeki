...
MediaIO::FormatDesc MediaIOTask_NullPacing::formatDesc() {
        return {"NullPacing",
                ...
                []() -> MediaIOTask * { return new MediaIOTask_NullPacing(); },
...
Error MediaIOTask_NullPacing::executeCmd(MediaIOCommandWrite &cmd) {
        ...
        if (consume) {
                if (_burnTimings && _hasLastConsumed) {
                        const Duration sincePrev =
                                Duration::fromNanoseconds(std::chrono::duration_cast<std::chrono::nanoseconds>(
                                                                  arrival.value() - _lastConsumed.value())
                                                                  .count());
                        ...
                        promekiDebug("MediaIOTask_NullPacing: consume "
                ...
                noteFrameDropped();
                if (_burnTimings) {
                        promekiDebug("MediaIOTask_NullPacing: drop "
                                     "heldUs=%lld",
