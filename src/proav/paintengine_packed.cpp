...
template <bool MethodB>
class PaintEngine_DPX : public PaintEngine::Impl {
        ...
        public:
                ...
                size_t drawRect(const PaintEngine::Pixel &pixel,
                        ...
                        auto hline = [&](int y, int x0, int x1) {
                                if(y < 0 || y >= h) return;
                                x0 = std::max(x0, 0); x1 = std::min(x1, w);
...
class PaintEngine_v210 : public PaintEngine::Impl {
        ...
        public:
                ...
                static constexpr V210Addr YAddr[6] = {
                        {0, 10}, {1,  0}, {1, 20},
                        {2, 10}, {3,  0}, {3, 20}
                };
                static constexpr V210Addr CbAddr[3] = {
                        {0,  0}, {1, 10}, {2, 20}
                };
                static constexpr V210Addr CrAddr[3] = {
                        {0, 20}, {2,  0}, {3, 10}
                };
                ...
                PaintEngine::Pixel readPixelFrom(const uint8_t *buf, size_t stride,
                                                 int x, int y) const {
                        PaintEngine::Pixel ret;
                        uint16_t *p = reinterpret_cast<uint16_t *>(ret.data());
                        const uint32_t *group = reinterpret_cast<const uint32_t *>(
                                buf + y * stride + (x / 6) * 16);
                        int pos = x % 6;
                        int pair = pos / 2;
                        ...
                        for(int y = 0; y < sh; y++)
                                for(int x = 0; x < sw; x++)
                                        writePixelAt(dx + x, dy + y,
                                                     readPixelFrom(sbuf, sStride, sx + x, sy + y));
                        return true;
                }
