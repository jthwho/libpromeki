...
PROMEKI_REGISTER_MEDIAIO_FACTORY(CscFactory)
...
namespace {
        ...
        // MediaIOFactory::bridge; smaller = higher quality.
        //
        // Two effects combine:
        //
        ...
        int cscBridgeCost(const PixelFormat &from, const PixelFormat &to) {
                ...
                const int fromBits =
                        from.memLayout().compCount() > 0 ? static_cast<int>(from.memLayout().compDesc(0).bits) : 0;
                const int toBits =
                        to.memLayout().compCount() > 0 ? static_cast<int>(to.memLayout().compDesc(0).bits) : 0;
        ...
        bool cscBridgeImpl(const MediaDesc &from, const MediaDesc &to, MediaIO::Config *outConfig, int *outCost) {
                ...
                if (outConfig != nullptr) {
                        *outConfig = MediaIOFactory::defaultConfig("CSC");
...
MediaIOFactory::Config::SpecMap CscFactory::configSpecs() const {
...
bool CscFactory::bridge(const MediaDesc &from, const MediaDesc &to, Config *outConfig, int *outCost) const {
        return cscBridgeImpl(from, to, outConfig, outCost);
}

MediaIO *CscFactory::create(const Config &config, ObjectBase *parent) const {
        auto *io = new CscMediaIO(parent);
        io->setConfig(config);
        return io;
}

CscMediaIO::CscMediaIO(ObjectBase *parent) : SharedThreadMediaIO(parent) {}

CscMediaIO::~CscMediaIO() {
        if (isOpen()) (void)close().wait();
}

Error CscMediaIO::executeCmd(MediaIOCommandOpen &cmd) {
...
Error CscMediaIO::executeCmd(MediaIOCommandClose &cmd) {
        (void)cmd;
        _outputQueue.clear();
...
Error CscMediaIO::convertPayload(const UncompressedVideoPayload &input,
        ...
        if (srcPd.isCompressed() || _outputPixelFormat.isCompressed()) {
                promekiErr("CscMediaIO: %s -> %s is a compression hop; "
                           "use VideoEncoderMediaIO / VideoDecoderMediaIO",
                           srcPd.name().cstr(), _outputPixelFormat.name().cstr());
                return Error::NotSupported;
        }

        MediaConfig convertConfig;
        output = input.convert(_outputPixelFormat, input.desc().metadata(), convertConfig);
        if (!output.isValid()) {
                promekiErr("CscMediaIO: convert %s -> %s failed", srcPd.name().cstr(),
...
Error CscMediaIO::executeCmd(MediaIOCommandWrite &cmd) {
        if (!cmd.frame.isValid()) {
                promekiErr("CscMediaIO: write with null frame");
                return Error::InvalidArgument;
        }

        if (static_cast<int>(_outputQueue.size()) >= _capacity && !_capacityWarned) {
                promekiWarn("CscMediaIO: output queue exceeded capacity (%d >= %d)",
...
Error CscMediaIO::executeCmd(MediaIOCommandRead &cmd) {
...
Error CscMediaIO::executeCmd(MediaIOCommandStats &cmd) {
...
int CscMediaIO::pendingInternalWrites() const {
        return static_cast<int>(_outputQueue.size());
}

// ---- Phase 1 introspection / negotiation overrides ----

Error CscMediaIO::describe(MediaIODescription *out) const {
        if (out == nullptr) return Error::Invalid;
        // Let the base populate identity / role flags / port snapshots
        // first; we add backend-specific @c preferredFormat /
        // @c producibleFormats below when @c OutputPixelFormat is set.
        ...
        if (_outputPixelFormatSet) {
                MediaDesc preferred;
                preferred.imageList().pushToBack(ImageDesc(Size2Du32(0, 0), _outputPixelFormat));
...
Error CscMediaIO::proposeInput(const MediaDesc &offered, MediaDesc *preferred) const {
