...
static bool guessDimensions(size_t fileSize, const PixelFormat &pd, size_t &outWidth, size_t &outHeight) {
...
class ImageFileIO_RawYUV : public ImageFileIO {
        public:
                ImageFileIO_RawYUV() {
                        ...
                        _extensions = {"uyvy", "yuyv",    "yuy2", "v210",    "i420",
                                       "nv12", "yuv420p", "i422", "yuv422p", "yuv"};
                }
...
Error ImageFileIO_RawYUV::load(ImageFile &imageFile, const MediaConfig &config) const {
        ...
        if (pdId == PixelFormat::Invalid && !isGenericYuv) {
                promekiErr("RawYUV load '%s': unrecognized extension '%s'", filename.cstr(), ext.cstr());
        ...
        const VideoPayload   *hint = (!hintVps.isEmpty() && hintVps[0].isValid()) ? hintVps[0].ptr() : nullptr;
        if (hint != nullptr && hint->desc().isValid()) {
                ...
                if (fileSizeErr != Error::Ok) {
                        promekiErr("RawYUV load '%s': cannot determine file size: %s", filename.cstr(),
                                   fileSizeErr.name().cstr());
                        return fileSizeErr;
                }
                ...
                promekiDebug("RawYUV load '%s': guessed %zux%zu %s from file size %zu", filename.cstr(), width, height,
        ...
        if (!payload.isValid()) {
                promekiErr("RawYUV load '%s': failed to allocate %zux%zu payload", filename.cstr(), width, height);
                return Error::NoMem;
        }
