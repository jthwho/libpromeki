...
MediaIO::FormatDesc MediaIOTask_TPG::formatDesc() {
        return {"TPG",
                ...
                []() -> MediaIOTask * { return new MediaIOTask_TPG(); },
...
Error MediaIOTask_TPG::executeCmd(MediaIOCommandOpen &cmd) {
        ...
        if (_videoEnabled) {
                ...
                if (!size.isValid()) {
                        promekiErr("MediaIOTask_TPG: invalid raster dimensions %s", size.toString().cstr());
        ...
        if (_videoEnabled && _burnEnabled && !_burnTextTemplate.isEmpty()) {
                ...
                if (!burnText.isEmpty()) {
                        if (MediaPayload::Ptr *slot = lastVideoPayloadSlot(frame.modify())) {
                                ...
                                if (burnErr.isError()) {
                                        promekiWarn("MediaIOTask_TPG: applyBurn failed: %s", burnErr.name().cstr());
                                }
                        }
                }
        }
...
Error MediaIOTask_TPG::proposeOutput(const MediaDesc &requested, MediaDesc *achievable) const {
