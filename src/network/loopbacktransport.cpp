...
ssize_t LoopbackTransport::sendPacket(const void *data, size_t size, const SocketAddress &dest) {
        if (!_open) return -1;
...
void LoopbackTransport::deliver(const void *data, size_t size, const SocketAddress &sender) {
        QueueEntry e;
        e.data = Buffer(size);
        e.data.setSize(size);
