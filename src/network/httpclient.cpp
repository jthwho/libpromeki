...
Buffer serializeRequest(const HttpRequest &req) {
        ...
        if(bodyLen > 0) {
                std::memcpy(static_cast<char *>(out.data()) + headLen,
                            body.data(), bodyLen);
        }
...
int HttpClient::Pending::cbHeadersComplete(llhttp_t *p) {
        ...
        self->response.setHttpVersion(String::sprintf("HTTP/%u.%u",
                self->parser.http_major, self->parser.http_minor));
        return 0;
}
...
int HttpClient::Pending::cbBody(llhttp_t *p, const char *at, size_t len) {
        ...
        if(self->maxBodyBytes >= 0 &&
           self->bodyBytesSoFar > self->maxBodyBytes) {
                return -1;
        }
...
void HttpClient::Pending::start() {
        ...
        if(err.isError()) { finish(err); return; }
        ...
        err = socket->connectToHost(SocketAddress(Ipv4Address(ipv4),
        ...
        if(client != nullptr && client->_timeoutMs > 0 && loop != nullptr) {
                ...
                timerId = loop->startTimer(client->_timeoutMs,
                        [pin]() {
                                Pending *self =
                                        const_cast<Pending *>(pin.ptr());
...
void HttpClient::Pending::reregister(uint32_t mask) {
        ...
        if(fd < 0) return;
        SharedPtr<Pending, false> pin = selfPin;
        ioHandle = loop->addIoSource(fd, mask,
                [pin](int /*f*/, uint32_t events) {
...
void HttpClient::Pending::onIoReady(uint32_t events) {
        ...
        if(firstWriteCheckPending && socket != nullptr &&
           (events & (EventLoop::IoError | EventLoop::IoWrite))) {
                firstWriteCheckPending = false;
                int soerr = 0;
                socklen_t soerrLen = sizeof(soerr);
                if(::getsockopt(socket->socketDescriptor(),
                        SOL_SOCKET, SO_ERROR, &soerr, &soerrLen) == 0
                   && soerr != 0) {
...
void HttpClient::Pending::pumpWrite() {
        ...
        const char *src = static_cast<const char *>(writeBuf.data())
                          + writeOffset;
        const int64_t want = static_cast<int64_t>(writeBuf.size())
                             - static_cast<int64_t>(writeOffset);
...
void HttpClient::Pending::readSome() {
        ...
        const llhttp_errno_t rc = llhttp_execute(&parser, buf,
                static_cast<size_t>(n));
        if(rc != HPE_OK) { finish(Error::Invalid); return; }
...
void HttpClient::Pending::finish(Error err) {
        ...
        if(loop != nullptr) {
                if(ioHandle >= 0) { loop->removeIoSource(ioHandle); ioHandle = -1; }
                if(timerId  >= 0) { loop->stopTimer(timerId);       timerId  = -1; }
        }
        if(socket != nullptr) {
                socket->close();
                delete socket;
...
HttpClient::~HttpClient() {
        ...
        while(!_active.isEmpty()) {
                ...
                if(!_active.isEmpty() && _active[0].ptr() == p.ptr()) {
                        ...
                        // was already cleared.
                        _active.removeIf([&](const PendingPtr &x) {
                                return x.ptr() == p.ptr();
                        });
                }
        }
...
Future<HttpResponse> HttpClient::post(const String &url, const Buffer &body,
...
Future<HttpResponse> HttpClient::put(const String &url, const Buffer &body,
                                     const String &contentType) {
        HttpRequest r;
...
Future<HttpResponse> HttpClient::dispatch(HttpRequest request) {
        ...
        if(!u.isValid() || u.host().isEmpty()) { reject(Error::Invalid);        return fut; }
#if PROMEKI_ENABLE_TLS
        if(u.scheme() == "https") {
                p->useTls      = true;
                ...
                reject(Error::Invalid); return fut;
        }
#else
        if(u.scheme() == "https") { reject(Error::NotImplemented); return fut; }
        if(u.scheme() != "http")  { reject(Error::Invalid);        return fut; }
#endif
        if(_loop == nullptr)                    { reject(Error::Invalid);        return fut; }
