...
class RtpSession::ReceiveThread : public Thread {
        public:
                ReceiveThread(RtpSession *session, const String &name)
                        : _session(session) {
        ...
        protected:
                void run() override {
                        ...
                        if(auto *udpTransport =
                                   dynamic_cast<UdpSocketTransport *>(
                                           _session->_transport)) {
                                if(UdpSocket *sock = udpTransport->socket()) {
                                        (void)sock->setReceiveTimeout(
                                                _session->_receivePollMs);
                                }
                        }

                        while(!_stopRequested.value()) {
                                Buffer::Ptr buf =
                                        Buffer::Ptr::create(kMaxPacketSize);
                                SocketAddress sender;
                                ssize_t n = _session->_transport->receivePacket(
                                        buf->data(), kMaxPacketSize, &sender);
                                if(n <= 0) {
                                ...
                                _session->packetReceivedSignal.emit(
                                        Buffer(*buf), pkt.timestamp(),
...
Error RtpSession::sendPacketsPaced(RtpPacket::List &packets, uint32_t timestamp,
        ...
        for(int64_t i = 0; i < n; i++) {
                ...
                if(i > 0) {
                        const TimeStamp pktDeadline =
                                startTime + (spreadInterval * i) / n;
