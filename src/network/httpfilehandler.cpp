...
HttpFileHandler::HttpFileHandler(const String &rootPath) : _root(FilePath(rootPath)) {}
...
String HttpFileHandler::etagFor(int64_t size, int64_t mtimeEpoch) {
        ...
        return String::sprintf("W/\"%llx-%llx\"", static_cast<unsigned long long>(size),
...
namespace {
        ...
        static bool parseSingleRange(const String &header, int64_t fileSize, int64_t &startOut, int64_t &endOut) {
...
bool HttpFileHandler::serveRange(const HttpRequest &request, HttpResponse &response, const String &fullPath,
        ...
        if (!parseSingleRange(range, fileSize, start, end)) {
                ...
                response.setHeader("Content-Range", String::sprintf("bytes */%lld", static_cast<long long>(fileSize)));
                return true;
        }

        IODevice::Shared dev = IODevice::Shared::takeOwnership(new File(fullPath));
        ...
        response.setHeader("Content-Range",
                           String::sprintf("bytes %lld-%lld/%lld", static_cast<long long>(start),
                                           static_cast<long long>(end), static_cast<long long>(fileSize)));
...
void HttpFileHandler::serve(const HttpRequest &request, HttpResponse &response) {
