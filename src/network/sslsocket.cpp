...
Error SslSocket::startEncryption(const String &hostname) {
        ...
        mbedtls_ssl_config *conf =
                static_cast<mbedtls_ssl_config *>(_ctx->nativeConfig());
        ...
        if(rc != 0) {
                promekiWarn("SslSocket: ssl_setup failed: %s",
        ...
        if(!hostname.isEmpty()) {
                ...
                if(rc != 0) {
                        promekiWarn("SslSocket: set_hostname failed: %s",
                                mbedtlsErrText(rc).cstr());
                        return Error::LibraryFailure;
                }
        }
        ...
        if(rc == 0) {
                ...
                if(flags != 0) {
                        ...
                        if(flags & MBEDTLS_X509_BADCERT_EXPIRED)
                                errs.pushToBack("certificate expired");
                        if(flags & MBEDTLS_X509_BADCERT_REVOKED)
                                errs.pushToBack("certificate revoked");
                        if(flags & MBEDTLS_X509_BADCERT_CN_MISMATCH)
                                errs.pushToBack("hostname mismatch");
                        if(flags & MBEDTLS_X509_BADCERT_NOT_TRUSTED)
                                errs.pushToBack("not trusted");
                        if(errs.isEmpty())
                                errs.pushToBack(String::sprintf("verify result 0x%08x", flags));
...
int64_t SslSocket::read(void *data, int64_t maxSize) {
        ...
        const int rc = mbedtls_ssl_read(&_d->ssl,
...
int64_t SslSocket::write(const void *data, int64_t maxSize) {
        ...
        const int rc = mbedtls_ssl_write(&_d->ssl,
