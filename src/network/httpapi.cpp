                        ...
                        case Variant::TypeColor: {
                                if (componentsOut != nullptr) {
                                        ...
                                        if (!componentsOut->contains("Color")) {
                                                JsonObject sch;
                                                sch.set("type", "object");
                                                sch.set("description", "Promeki Color: 4 floats plus a "
                        ...
                        case Variant::TypeMasteringDisplay: {
                                if (componentsOut != nullptr) {
                                        ...
                                        if (!componentsOut->contains("MasteringDisplay")) {
                                                JsonObject sch;
                                                sch.set("type", "object");
                                                sch.set("description", "SMPTE ST 2086 mastering display "
                        ...
                        case Variant::TypeContentLightLevel: {
                                if (componentsOut != nullptr) {
                                        ...
                                        if (!componentsOut->contains("ContentLightLevel")) {
                                                JsonObject sch;
                                                sch.set("type", "object");
                                                sch.set("description", "CTA-861.3 content light level "
                                                                       "(MaxCLL/MaxFALL).");
                                                componentsOut->set("ContentLightLevel", sch);
                                        }
                                        return out;
                                }
                                out.set("type", "object");
                        ...
                        case Variant::TypeSdpSession: {
                                if (componentsOut != nullptr) {
                                        ...
                                        if (!componentsOut->contains("SdpSession")) {
                                                JsonObject sch;
                                                sch.set("type", "object");
                                                sch.set("description", "SDP session description (RFC 8866).");
...
HttpApi::HttpApi(HttpServer &server, const String &prefix, ObjectBase *parent)
    : ObjectBase(parent), _server(server), _prefix(prefix), _title(DefaultTitle), _version(DefaultVersion) {
...
Error HttpApi::registerEndpoint(Endpoint ep, HttpHandler::Ptr handler) {
        ...
        for (size_t i = 0; i < _endpoints.size(); ++i) {
                ...
                if (existing.path == ep.path && existing.method == ep.method) {
                        promekiWarn("HttpApi: endpoint %s %s already registered", ep.method.valueName().cstr(),
                                    ep.path.cstr());
                        return Error::Exists;
                }
        }
...
Error HttpApi::route(Endpoint ep, HttpHandlerFunc handler) {
        auto wrapped = HttpHandler::Ptr::takeOwnership(new HttpFunctionHandler(std::move(handler)));
...
Error HttpApi::rpc(Endpoint ep, RpcCall call) {
        ...
        for (size_t i = 0; i < ep.params.size(); ++i) {
                if (ep.params[i].in == ParamIn::Body) {
                        hasBody = true;
                        break;
                }
        }
        ...
        auto fn = [params, errors, call = std::move(call)](const HttpRequest &req, HttpResponse &res) {
                VariantMap args;
                ...
                for (size_t i = 0; i < params.size(); ++i) {
                        ...
                        switch (p.in) {
                                ...
                                case HttpApi::ParamIn::Body: {
                                        ensureBody();
                                        if (bodyParseError) {
                                                res = HttpResponse::badRequest("Body must be a JSON "
                                                                               "object");
                                                return;
                                        }
                                        if (bodyJson.contains(p.name)) {
                                                bodyJson.forEach([&](const String &k, const Variant &val) {
                                                        if (k == p.name) v = val;
                                                });
                                                present = v.isValid();
                                        }
                        ...
                        if (!present) {
                                if (p.required) {
                                        res = HttpResponse::badRequest(String("Missing required "
                                                                              "parameter: ") +
                                                                       p.name);
                                        return;
                                }
                        ...
                        if (present) {
                                ...
                                if (!p.spec.validate(v, &verr)) {
                                        res = HttpResponse::badRequest(String("Invalid value for ") + p.name + ": " +
                                                                       verr.desc());
                                        return;
                                }
                ...
                if (resultErr.isError()) {
                        ...
                        if (code == Error::Invalid || code == Error::InvalidArgument || code == Error::ParseFailed) {
                                status = 400;
                        } else if (code == Error::NotExist || code == Error::IdNotFound) {
                                status = 404;
                        } else if (code == Error::PermissionDenied || code == Error::NoPermission) {
                                status = 403;
                        } else {
...
Error HttpApi::mount() {
        ...
        _server.route(resolve("/_catalog"), HttpMethod::Get, makeCatalogHandler());
        _server.route(resolve("/_openapi"), HttpMethod::Get, makeOpenApiHandler());
        ...
        _server.route(_prefix, HttpMethod::Get, [slashed](const HttpRequest &, HttpResponse &res) {
                res.setStatus(HttpStatus::Found);
                res.setHeader("Location", slashed);
                res.setText("");
        });
        _server.route(resolve("/{path:*}"), HttpMethod::Get, makeExplorerHandler());
        return Error::Ok;
}
...
HttpHandler::Ptr HttpApi::makeExplorerHandler() const {
        ...
        return HttpHandler::Ptr::takeOwnership(new HttpFileHandler(Dir(":/.PROMEKI/explorer")));
}
        ...
        for (size_t i = 0; i < _endpoints.size(); ++i) {
                ...
                for (size_t j = 0; j < erefs.size(); ++j) {
                        ...
                        if (erefs[j].body.isValid()) {
                                ej.set("schema", variantSpecToJsonSchema(erefs[j].body));
                        }
                        errs.add(ej);
                }
                obj.set("errors", errs);
        ...
        for (size_t i = 0; i < _endpoints.size(); ++i) {
                ...
                for (size_t j = 0; j < ep.params.size(); ++j) {
                        ...
                        if (p.in == ParamIn::Body) {
                                hasBody = true;
                                bodyProps.set(p.name, variantSpecToJsonSchema(p.spec, &schemas));
                        ...
                        pj.set("required", p.in == ParamIn::Path ? true : p.required);
                        if (!p.spec.description().isEmpty()) {
                                pj.set("description", p.spec.description());
                        }
                        pj.set("schema", variantSpecToJsonSchema(p.spec, &schemas));
                        params.add(pj);
                }
                ...
                JsonObject responses;
                {
                        ...
                        if (ep.response.isValid() || !ep.response.description().isEmpty()) {
                                JsonObject content;
                                JsonObject media;
                                media.set("schema", variantSpecToJsonSchema(ep.response, &schemas));
                ...
                for (size_t j = 0; j < erefs.size(); ++j) {
                        ...
                        if (erefs[j].body.isValid()) {
                                JsonObject content;
                                JsonObject media;
                                media.set("schema", variantSpecToJsonSchema(erefs[j].body, &schemas));
                                content.set("application/json", media);
...
JsonObject HttpApi::variantSpecToJsonSchema(const VariantSpec &spec, JsonObject *componentsOut) {
        ...
        auto applyConstraints = [&](JsonObject &target) {
                ...
                if (spec.hasEnumType()) {
                        ...
                        JsonArray             values;
                        const Enum::ValueList valuePairs = Enum::values(spec.enumType());
                        for (size_t i = 0; i < valuePairs.size(); ++i) {
                                values.add(valuePairs[i].first());
                        }
                        if (values.size() > 0) target.set("enum", values);
                        target.set("x-promeki-enum-type", spec.enumType().name());
                }
        };
