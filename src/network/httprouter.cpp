...
void HttpRouter::route(const String &pattern, const HttpMethod &method, HttpHandlerFunc handler) {
        route(pattern, method, HttpHandler::Ptr::takeOwnership(new HttpFunctionHandler(std::move(handler))));
}

void HttpRouter::route(const String &pattern, const HttpMethod &method, HttpHandler::Ptr handler) {
        Route r;
...
void HttpRouter::any(const String &pattern, HttpHandlerFunc handler) {
        any(pattern, HttpHandler::Ptr::takeOwnership(new HttpFunctionHandler(std::move(handler))));
}
...
HttpRouter::Pattern HttpRouter::compilePattern(const String &source) {
        ...
        out.trailingSlash = (s.byteCount() > 1) && (s.cstr()[s.byteCount() - 1] == '/');
...
bool HttpRouter::matchPattern(const Pattern &pattern, const String &path, HashMap<String, String> &paramsOut) {
        ...
        const bool   requestTrailing = (p.byteCount() > 1) && (p.cstr()[p.byteCount() - 1] == '/');
...
void HttpRouter::runChain(HttpRequest &request, HttpResponse &response, HttpHandlerFunc terminal) const {
...
void HttpRouter::dispatch(HttpRequest &request, HttpResponse &response) const {
        ...
        runChain(request, response, [&](const HttpRequest &, HttpResponse &res) {
                ...
                for (size_t i = 0; i < _routes.size(); ++i) {
                        ...
                        if (r.methodValue >= 0 && r.methodValue != request.method().value()) {
                                continue;
                        }
                ...
                if (best != nullptr) {
                        ...
                        if (best->handler.isValid()) {
                                ...
                                const_cast<HttpHandler *>(best->handler.ptr())->serve(request, res);
                        }
                        return;
                }
                ...
                // 404
                if (_notFound)
                        _notFound(request, res);
                else
                        defaultNotFound(request, res);
        });
}
...
void HttpRouter::defaultNotFound(const HttpRequest &request, HttpResponse &response) {
        response = HttpResponse::notFound(String::sprintf("404 Not Found: %s", request.path().cstr()));
}

void HttpRouter::defaultMethodNotAllowed(const HttpRequest &, HttpResponse &response) {
