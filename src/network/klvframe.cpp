...
Error readExact(IODevice *dev, void *buf, size_t n) {
        ...
        while(remaining > 0) {
                ...
                if(got == 0) {
                        return anyRead ? Error(Error::IOError)
...
Error KlvReader::readHeader(FourCC &key, uint32_t &valueSize) {
        ...
        key = FourCC(static_cast<char>(hdr[0]), static_cast<char>(hdr[1]),
                     static_cast<char>(hdr[2]), static_cast<char>(hdr[3]));
...
Error KlvWriter::writeFrame(FourCC key, const Buffer &value) {
        return writeFrame(key, value.data(),
