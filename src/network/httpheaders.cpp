...
void HttpHeaders::forEach(std::function<void(const String &name,
...
bool HttpHeaders::operator==(const HttpHeaders &other) const {
        ...
        Entry::List a, b;
        forEach([&](const String &n, const String &v) {
                a.pushToBack(Entry{n, v});
        });
        other.forEach([&](const String &n, const String &v) {
                b.pushToBack(Entry{n, v});
        });
