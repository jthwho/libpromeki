...
int64_t LocalSocket::write(const void *data, int64_t maxSize) {
        ...
        ssize_t ret = ::send(_fd, data, static_cast<size_t>(maxSize),
                             MSG_NOSIGNAL);
        if(ret < 0) {
                int e = errno;
...
Error LocalSocket::open(OpenMode)                           { return Error::NotSupported; }
Error LocalSocket::close()                                  { return Error::NotSupported; }
Error LocalSocket::connectTo(const String &, unsigned int)  { return Error::NotSupported; }
int64_t LocalSocket::read(void *, int64_t)                  { return -1; }
int64_t LocalSocket::write(const void *, int64_t)           { return -1; }
int64_t LocalSocket::bytesAvailable() const                 { return 0; }
void LocalSocket::setSocketDescriptor(int, const String &)  { }
Error LocalSocket::setReceiveTimeout(unsigned int)          { return Error::NotSupported; }
Error LocalSocket::setSendTimeout(unsigned int)             { return Error::NotSupported; }
