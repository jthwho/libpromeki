        ...
        _socket(socket)
{
        ...
        _impl->settings.on_message_begin = reinterpret_cast<llhttp_cb>(
                &HttpConnection::cbMessageBegin);
        _impl->settings.on_url = reinterpret_cast<llhttp_data_cb>(
                &HttpConnection::cbUrl);
        _impl->settings.on_header_field = reinterpret_cast<llhttp_data_cb>(
                &HttpConnection::cbHeaderField);
        _impl->settings.on_header_value = reinterpret_cast<llhttp_data_cb>(
                &HttpConnection::cbHeaderValue);
        _impl->settings.on_headers_complete = reinterpret_cast<llhttp_cb>(
                &HttpConnection::cbHeadersComplete);
        _impl->settings.on_body = reinterpret_cast<llhttp_data_cb>(
                &HttpConnection::cbBody);
        _impl->settings.on_message_complete = reinterpret_cast<llhttp_cb>(
                &HttpConnection::cbMessageComplete);
...
Error HttpConnection::start() {
        ...
        const uint32_t mask = (_state == State::Handshaking)
                ? (EventLoop::IoRead | EventLoop::IoWrite)
                : EventLoop::IoRead;
        _ioHandle = _loop->addIoSource(fd, mask,
                [this](int fd, uint32_t events) { onIoReady(fd, events); });
        ...
        // ghosts.
        if(_idleTimeoutMs > 0) {
                _timerId = _loop->startTimer(_idleTimeoutMs,
                        [this]() { onIdleTimeout(); }, /*singleShot=*/true);
        }
...
void HttpConnection::onIoReady(int fd, uint32_t events) {
        ...
        if(_state == State::Handshaking) {
                ...
                if(_loop != nullptr && _ioHandle >= 0) {
                        _loop->removeIoSource(_ioHandle);
                        _ioHandle = _loop->addIoSource(_socket->socketDescriptor(),
                                EventLoop::IoRead,
...
int HttpConnection::cbHeadersComplete(void *parser) {
        ...
        // Method.
        const llhttp_method_t mt = static_cast<llhttp_method_t>(
                self->_impl->parser.method);
        ...
        // HTTP version.
        self->_pendingRequest.setHttpVersion(String::sprintf("HTTP/%u.%u",
                self->_impl->parser.http_major,
                self->_impl->parser.http_minor));
...
int HttpConnection::cbBody(void *parser, const char *at, size_t len) {
        ...
        if(self->_maxBodyBytes >= 0 &&
           self->_bodyBytesSoFar > self->_maxBodyBytes) {
...
void HttpConnection::enqueueResponse(HttpResponse response) {
        ...
        if(!_keepAlive) response.headers().set("Connection", "close");
        else if(connHdr.isEmpty())       response.headers().set("Connection", "keep-alive");
        ...
        if(!stream && response.body().isValid() && response.body().size() > 0) {
                appendBytes(_writeQueue,
                        static_cast<const char *>(response.body().data()),
                        response.body().size());
        }
...
void HttpConnection::pumpWrite() {
        ...
        if(_loop != nullptr && _ioHandle >= 0) {
                _loop->removeIoSource(_ioHandle);
                const int fd = _socket->socketDescriptor();
                _ioHandle = _loop->addIoSource(fd, EventLoop::IoRead,
                        [this](int f, uint32_t e) { onIoReady(f, e); });
        }
        _writeQueue = Buffer(0);
        _writeOffset = 0;
