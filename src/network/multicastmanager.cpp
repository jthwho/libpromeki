...
MulticastManager::MulticastManager(ObjectBase *parent) : ObjectBase(parent) {}
...
Error MulticastManager::joinGroup(const SocketAddress &group, UdpSocket *socket, const String &iface) {
...
Error MulticastManager::leaveSourceGroup(const SocketAddress &group, const SocketAddress &source, UdpSocket *socket) {
        ...
        for (size_t i = 0; i < _memberships.size(); i++) {
                const auto &m = _memberships[i];
                if (m.group == group && m.socket == socket && m.isSSM && m.source == source) {
                        _memberships.remove(i);
                        break;
                }
        }
