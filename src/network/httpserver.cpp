...
HttpServer::HttpServer(ObjectBase *parent) :
        ObjectBase(parent),
        _tcpServer(UniquePtr<TcpServer>::create(this))
{
...
Error HttpServer::listen(const SocketAddress &address, int backlog) {
        ...
        _acceptHandle = _loop->addIoSource(fd, EventLoop::IoRead,
                [this](int, uint32_t) { onNewConnection(); });
        if(_acceptHandle < 0) {
                _tcpServer->close();
...
void HttpServer::route(const String &pattern,
...
void HttpServer::routeWebSocket(const String &pattern,
        ...
        _router.route(pattern, HttpMethod::Get,
                [userHandler](const HttpRequest &req, HttpResponse &res) {
                        ...
                        res.setHeader("Sec-WebSocket-Accept",
                                WebSocket::computeAcceptValue(key));
                        ...
                        if(!wantedProto.isEmpty()) {
                                ...
                                if(parts.size() > 0) {
                                        res.setHeader("Sec-WebSocket-Protocol",
                                                parts[0].trim());
                                }
                        }
...
void HttpServer::onNewConnection() {
        ...
        for(;;) {
                ...
                conn->requestReceivedSignal.connect(
                        [this](HttpRequest req) {
                                requestReceivedSignal.emit(req);
                        }, this);
                conn->responseSentSignal.connect(
                        [this](HttpRequest req, HttpResponse res) {
                                responseSentSignal.emit(req, res);
                        }, this);
                conn->errorOccurredSignal.connect(
                        [this](Error err) { errorOccurredSignal.emit(err); }, this);
                conn->closedSignal.connect(
                        [this, conn]() { reapClosedConnection(conn); }, this);

                Error err = conn->start();
                if(err.isError()) {
                        promekiWarn("HttpServer: failed to start connection: %s",
                                err.name().cstr());
                        delete conn;
                        continue;
                }
