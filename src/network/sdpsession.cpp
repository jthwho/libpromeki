...
void SdpSession::setOrigin(const String &username, uint64_t sessionId, uint64_t sessionVersion, const String &netType,
...
SdpMediaDescription::RtpMap SdpMediaDescription::rtpMap() const {
        ...
        if (slash2 == String::npos) {
                ...
                rate = static_cast<uint32_t>(rest.substr(0, slash2).trim().toInt(&rateErr));
...
Result<SdpSession> SdpSession::fromFile(const String &path) {
        ...
        if (err.isError()) {
                promekiErr("SdpSession::fromFile: open '%s' failed: %s", path.cstr(), err.desc().cstr());
        ...
        f.close();
        if (nread <= 0) {
                promekiErr("SdpSession::fromFile: '%s' is empty or unreadable", path.cstr());
...
Error SdpSession::toFile(const String &path) const {
        ...
        if (err.isError()) {
                promekiErr("SdpSession::toFile: open '%s' failed: %s", path.cstr(), err.desc().cstr());
                return err;
        }
        ...
        if (n != static_cast<int64_t>(text.size())) {
                promekiErr("SdpSession::toFile: short write to '%s' "
                           "(%lld of %zu bytes)",
                           path.cstr(), static_cast<long long>(n), text.size());
