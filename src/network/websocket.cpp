...
WebSocket::WebSocket(ObjectBase *parent) : ObjectBase(parent), _impl(UniquePtr<Impl>::create()) {
...
Error WebSocket::connectToUrl(const String &urlStr) {
        ...
        err = _socket->connectToHost(SocketAddress(Ipv4Address(ipv4), static_cast<uint16_t>(port)));
        if (err.isError()) {
                finalizeClose(err);
                return err;
        }
        ...
        for (size_t i = 0; i < _impl->extraHeaderNames.size(); ++i) {
                head += _impl->extraHeaderNames[i] + ": " + _impl->extraHeaderValues[i] + "\r\n";
        }
        head += "\r\n";
...
void WebSocket::registerIo(uint32_t mask) {
        ...
        if (fd < 0) return;
        _ioHandle = _loop->addIoSource(fd, mask, [this](int f, uint32_t e) { onIoReady(f, e); });
}
...
void WebSocket::pumpWrite() {
        ...
        if (_isClient && !_impl->handshakeComplete && _impl->requestBytes.isValid()) {
                ...
                while (_impl->requestOffset < total) {
                        const char *src = static_cast<const char *>(_impl->requestBytes.data()) + _impl->requestOffset;
        ...
        if (_impl->writeQueue.isValid()) {
                while (_impl->writeOffset < _impl->writeQueue.size()) {
                        const char   *src = static_cast<const char *>(_impl->writeQueue.data()) + _impl->writeOffset;
                        const int64_t want = static_cast<int64_t>(_impl->writeQueue.size() - _impl->writeOffset);
                        const int64_t n = _socket->write(src, want);
...
void WebSocket::parseClientHandshakeResponse() {
        ...
        const String code = (sp2 == String::npos) ? status.mid(sp1 + 1) : status.mid(sp1 + 1, sp2 - sp1 - 1);
        ...
        if (leftover > 0) {
                ...
                std::memcpy(kept.data(), static_cast<const uint8_t *>(_impl->inbound.data()) + consumed, leftover);
...
void WebSocket::processIncomingBytes() {
        ...
        while (_state != Disconnected) {
                ...
                if (_maxMessageBytes >= 0 && plen > static_cast<uint64_t>(_maxMessageBytes)) {
                ...
                if (leftover > 0) {
                        Buffer kept(leftover);
                        std::memcpy(kept.data(), static_cast<const uint8_t *>(_impl->inbound.data()) + total, leftover);
                        kept.setSize(leftover);
                        _impl->inbound = std::move(kept);
                        _impl->inboundSize = leftover;
                } else {
                ...
                if (payload.isValid() && payload.size() > 0) {
                        bufAppend(_impl->messageBuf, _impl->messageSize, payload.data(), payload.size());
                }
                if (_maxMessageBytes >= 0 && _impl->messageSize > static_cast<size_t>(_maxMessageBytes)) {
        ...
        switch (opcode) {
                case OpClose: {
                        ...
                        if (!_impl->sentClose) {
                                ...
                                sendFrame(OpClose, echo.isValid() ? echo.data() : nullptr,
                ...
                case OpPing: {
                        ...
                        sendFrame(OpPong, payload.isValid() ? payload.data() : nullptr,
                ...
                default: enqueueClose(CloseProtocolError, "unknown control opcode"); break;
        }
}
...
void WebSocket::handleDataMessage(uint8_t opcode, Buffer payload) {
        if (opcode == OpText) {
                String text;
                if (payload.isValid() && payload.size() > 0) {
                        text = String(static_cast<const char *>(payload.data()), payload.size());
                }
...
Error WebSocket::sendFrame(uint8_t opcode, const void *data, size_t len, bool fin) {
        ...
        if (len < 126) {
                ...
                for (int i = 7; i >= 0; --i) {
                        header[hLen++] = static_cast<uint8_t>((static_cast<uint64_t>(len) >> (i * 8)) & 0xFF);
                }
        }
