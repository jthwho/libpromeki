...
void SDLEventPump::handleKeyEvent(const SDL_Event &e) {
        ...
        Event::Type type = (e.type == SDL_EVENT_KEY_DOWN)
                ? KeyEvent::KeyPress : KeyEvent::KeyRelease;
...
void SDLEventPump::handleMouseEvent(const SDL_Event &e) {
        ...
        if(e.type == SDL_EVENT_MOUSE_MOTION) {
                ...
                if(window == nullptr) return;
                pos = Point2Di32(static_cast<int>(e.motion.x),
                ...
                if(window == nullptr) return;
                pos = Point2Di32(static_cast<int>(e.button.x),
                                 static_cast<int>(e.button.y));
                action = (e.type == SDL_EVENT_MOUSE_BUTTON_DOWN)
...
void SDLEventPump::handleMouseWheelEvent(const SDL_Event &e) {
        ...
        MouseEvent::Action action = (e.wheel.y > 0)
                ? MouseEvent::ScrollUp : MouseEvent::ScrollDown;
        uint8_t mods = translateModifiers(SDL_GetModState());
