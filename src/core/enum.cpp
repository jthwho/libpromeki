...
void populateStringLiterals(Enum::Definition *def) {
        ...
        for(size_t i = 0; i < def->entryCount; ++i) {
                ...
                new (&nameLits[i])      StringLiteralData(qualifiedBuf + typeNameLen + 2,
                                                          entryNameLen);
        }
...
Enum::Type Enum::registerType(const String &typeName,
        ...
        size_t totalBytes = sizeof(DynamicStorage)
                          + sizeof(Enum::Entry) * entryCount
                          + nameBufSize;
...
Enum::ValueList Enum::values(Type type) {
        ...
        for(size_t i = 0; i < type._def->entryCount; ++i) {
                out.pushToBack(Value(
                        String::fromLiteralData(&type._def->entryNameLits[i]),
                        type._def->entries[i].value));
        }
        return out;
}
