...
MediaTimeStamp::MediaTimeStamp(const TimeStamp &ts, const ClockDomain &domain, const Duration &offset)
    : _timeStamp(ts), _domain(domain), _offset(offset) {}

String MediaTimeStamp::toString() const {
        if (!isValid()) return String();
        int64_t offsetNs = _offset.nanoseconds();
        return String::sprintf("%s %s %+lld", _domain.toString().cstr(), _timeStamp.toString().cstr(),
...
bool MediaTimeStamp::operator==(const MediaTimeStamp &other) const {
        return _domain == other._domain && _timeStamp == other._timeStamp && _offset == other._offset;
}
