...
EventLoopWakeFd::EventLoopWakeFd() {
        ...
        if(efd < 0) {
                promekiWarn("EventLoop: eventfd() failed (errno %d: %s)",
...
int EventLoopWakeFd::pollFd() const { return _eventFd; }
...
EventLoopWakeFd::~EventLoopWakeFd() = default;
int  EventLoopWakeFd::pollFd() const { return _pipe.readFd(); }
void EventLoopWakeFd::wake()         { _pipe.wake(); }
void EventLoopWakeFd::drain()        { _pipe.drain(); }

#else // !PROMEKI_PLATFORM_POSIX

EventLoopWakeFd::EventLoopWakeFd()  = default;
EventLoopWakeFd::~EventLoopWakeFd() = default;
int  EventLoopWakeFd::pollFd() const { return -1; }
void EventLoopWakeFd::wake()         { }
void EventLoopWakeFd::drain()        { }
...
void EventLoop::waitOnSources(unsigned int waitMs) {
        ...
        {
                Mutex::Locker lock(_ioMutex);
                _ioSources.removeIf([](const IoSource &s) {
                        return s.pendingRemove;
                });
        ...
        if(rc < 0) {
                ...
                promekiWarn("EventLoop::waitOnSources: poll() failed (errno %d: %s)",
                            errno, std::strerror(errno));
                return;
        }
