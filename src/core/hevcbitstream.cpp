...
Error HevcDecoderConfig::fromAnnexB(const BufferView &au, HevcDecoderConfig &out) {
        ...
        Error iterErr = H264Bitstream::forEachAnnexBNal(au,
                [&](const H264Bitstream::NalUnit &nal) -> Error {
                        ...
                        switch(t) {
                                ...
                                case HevcNalTypeSps: {
                                        if(!haveProfile) {
                                                Error pe = extractHevcSpsProfile(nal.view.data(),
                                                                                 nal.view.size(), out);
...
Error HevcDecoderConfig::parse(const BufferView &payload, HevcDecoderConfig &out) {
        ...
        out.generalProfileCompatibilityFlags =
                static_cast<uint32_t>(readBE(data + 2, 4));
        out.generalConstraintIndicatorFlags = readBE(data + 6, 6);
        out.generalLevelIdc = data[12];
        out.minSpatialSegmentationIdc =
                static_cast<uint16_t>(((data[13] & 0x0f) << 8) | data[14]);
        ...
        for(uint8_t a = 0; a < numOfArrays; ++a) {
                ...
                if(nalType == HevcNalTypeVps) bucket = &out.vps;
                else if(nalType == HevcNalTypeSps) bucket = &out.sps;
                else if(nalType == HevcNalTypePps) bucket = &out.pps;
...
Error HevcDecoderConfig::serialize(Buffer::Ptr &outBuf) const {
        ...
        struct ArrayRef { uint8_t nalType; const List<Buffer::Ptr> *nals; };
        ArrayRef arrays[] = {
                { HevcNalTypeVps, &vps },
                { HevcNalTypeSps, &sps },
                { HevcNalTypePps, &pps },
        ...
        dst[1]  = static_cast<uint8_t>(((generalProfileSpace & 0x03) << 6) |
                                       ((generalTierFlag & 0x01) << 5) |
        ...
        dst[21] = static_cast<uint8_t>(((constantFrameRate & 0x03) << 6) |
