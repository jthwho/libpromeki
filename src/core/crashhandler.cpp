...
namespace {
        ...
        int readCurrentThreadName(char *buf, size_t bufLen) {
                buf[0] = '\0';
#if defined(PROMEKI_PLATFORM_LINUX) || defined(PROMEKI_PLATFORM_APPLE) || defined(PROMEKI_PLATFORM_BSD)
        ...
        void writeThreadList(int fd1, int fd2, uint64_t markedTid, const char *marker) {
                ...
                safeWrite2(fd1, fd2, "\n");
                safeWrite2(fd1, fd2,
                           "  (full thread enumeration requires Mach APIs, "
                           "not signal-safe)\n");
#endif
        }
        ...
        void writeDemangledFrame(int fd1, int fd2, char *sym) {
                ...
                if (lparen != nullptr) {
                        ...
                        if (plus != nullptr && rparen != nullptr && plus > lparen + 1 && plus < rparen) {
                                // Terminate at '+' so the mangled name is a
                                // standalone C string for __cxa_demangle.
                                *plus = '\0';
                                int   status = 0;
                                char *demangled = abi::__cxa_demangle(lparen + 1, nullptr, nullptr, &status);
                                *plus = '+';
        ...
        void writeMemSpaceStats(int fd1, int fd2) {
                ...
                for (size_t i = 0; i < g_memSpaceCount; ++i) {
                        ...
                        safeWrite2(fd1, fd2, "[");
                        safeWrite2(fd1, fd2, utoa(static_cast<uint64_t>(e.id), numBuf, sizeof(numBuf)));
                        safeWrite2(fd1, fd2, ":");
                        ...
                        auto writeLabel = [&](const char *label, uint64_t val) {
                                safeWrite2(fd1, fd2, label);
                                safeWrite2(fd1, fd2, utoa(val, numBuf, sizeof(numBuf)));
                        };
        ...
        void writeCpuRegisters(int fd1, int fd2, void *ucontext) {
                ...
                for (int i = 0; i <= 30; ++i) {
                        ...
                        writeReg(fd1, fd2, nameBuf, static_cast<uint64_t>(mc->regs[i]));
        ...
        void writeReportBody(int consoleFd, int logFd, bool isCrash) {
                ...
                safeWrite2(consoleFd, logFd, formatIsoUtc(now, timeBuf, sizeof(timeBuf)));
                safeWrite2(consoleFd, logFd, "  (epoch ");
                safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(now), numBuf, sizeof(numBuf)));
                ...
                if (g_startTime != 0 && now >= g_startTime) {
                        ...
                        safeWrite2(consoleFd, logFd, utoa(uptime, numBuf, sizeof(numBuf)));
                        safeWrite2(consoleFd, logFd, " seconds\n");
                }

                // PID / PPID.
                safeWrite2(consoleFd, logFd, "PID: ");
                safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(getpid()), numBuf, sizeof(numBuf)));
                safeWrite2(consoleFd, logFd, "   Parent PID: ");
                safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(getppid()), numBuf, sizeof(numBuf)));
                safeWrite2(consoleFd, logFd, "\n");

                // User / group (real & effective).
                safeWrite2(consoleFd, logFd, "User: ");
                safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(getuid()), numBuf, sizeof(numBuf)));
                safeWrite2(consoleFd, logFd, "/");
                safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(geteuid()), numBuf, sizeof(numBuf)));
                safeWrite2(consoleFd, logFd, " (real/effective)   Group: ");
                safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(getgid()), numBuf, sizeof(numBuf)));
                safeWrite2(consoleFd, logFd, "/");
                safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(getegid()), numBuf, sizeof(numBuf)));
                safeWrite2(consoleFd, logFd, "\n");
                ...
                safeWrite2(consoleFd, logFd, itoa(savedErrno, numBuf, sizeof(numBuf)));
                safeWrite2(consoleFd, logFd, "\n");
                ...
                if (nCpu > 0) {
                        ...
                        safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(nCpu), numBuf, sizeof(numBuf)));
                ...
                if (getrusage(RUSAGE_SELF, &ru) == 0) {
                        ...
                        safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(ru.ru_maxrss), numBuf, sizeof(numBuf)));
                        ...
                        safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(ru.ru_minflt), numBuf, sizeof(numBuf)));
                        safeWrite2(consoleFd, logFd, " minor / ");
                        safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(ru.ru_majflt), numBuf, sizeof(numBuf)));
                        safeWrite2(consoleFd, logFd, " major\n");

                        safeWrite2(consoleFd, logFd, "CPU Time (user):   ");
                        safeWrite2(consoleFd, logFd,
                                   utoa(static_cast<uint64_t>(ru.ru_utime.tv_sec), numBuf, sizeof(numBuf)));
                        safeWrite2(consoleFd, logFd, "s\n");
                        safeWrite2(consoleFd, logFd, "CPU Time (system): ");
                        safeWrite2(consoleFd, logFd,
                                   utoa(static_cast<uint64_t>(ru.ru_stime.tv_sec), numBuf, sizeof(numBuf)));
                        safeWrite2(consoleFd, logFd, "s\n");

                        safeWrite2(consoleFd, logFd, "Context Switches:  ");
                        safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(ru.ru_nvcsw), numBuf, sizeof(numBuf)));
                        safeWrite2(consoleFd, logFd, " voluntary / ");
                        safeWrite2(consoleFd, logFd, utoa(static_cast<uint64_t>(ru.ru_nivcsw), numBuf, sizeof(numBuf)));
                ...
                if (sysinfo(&si) == 0) {
                        ...
                        auto     writeMb = [&](const char *label, uint64_t bytes) {
                                ...
                                safeWrite2(consoleFd, logFd, utoa(bytes / (1024 * 1024), numBuf, sizeof(numBuf)));
                ...
                if (logFd >= 0 && g_envSnapshotLen > 0) {
                        ...
                        while (off < g_envSnapshotLen) {
                                ssize_t n = ::write(logFd, g_envSnapshot + off, g_envSnapshotLen - off);
                                if (n <= 0) break;
        ...
        void signalHandler(int signo, siginfo_t *info, void *ucontext) {
                ...
                if (info != nullptr) {
                        ...
                        if (signo != SIGABRT) {
                                ...
                                safeWrite2(STDERR_FILENO, logFd,
                                           utox(reinterpret_cast<uint64_t>(info->si_addr), numBuf, sizeof(numBuf)));
                                safeWrite2(STDERR_FILENO, logFd, "\n");
                        }
                }
...
void CrashHandler::install() {
        ...
        String logFile = String::sprintf("%s/promeki-crash-%s-%d.log", logDir.toString().cstr(), appName.cstr(),
        ...
        {
                String dump;
                LibraryOptions::instance().forEach([&dump](LibraryOptions::ID id, const Variant &val) {
                        dump += String::sprintf("%-20s = %s\n", id.name().cstr(), val.get<String>().cstr());
                });
...
void CrashHandler::writeTrace(const char *reason) {
        ...
        String tracePath = String::sprintf("%s/promeki-trace-%s-%d-%04u.log", logDir.toString().cstr(), appName.cstr(),
                                           static_cast<int>(getpid()), static_cast<unsigned int>(seq));

        int logFd = ::open(tracePath.cstr(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
