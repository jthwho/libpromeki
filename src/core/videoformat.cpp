...
#define X(ID, NAME, W, H) {.id = VideoFormat::ID, .name = NAME, .width = (W), .height = (H)},
        StructDatabase<VideoFormat::WellKnownRaster, RasterInfo> rasterDb = {PROMEKI_WELL_KNOWN_RASTERS};
#undef X
...
#define X(ID, RASTER, RATE, SCAN, FLAGS)                                                                               \
        {.id = VideoFormat::ID,                                                                                        \
         ...
         .flags = (FLAGS)},
        StructDatabase<VideoFormat::WellKnownFormat, FormatInfo> formatDb = {{.id = VideoFormat::Invalid,
                                                                              .name = "INV",
...
#undef X
        ...
        const RasterAlias kRasterAliases[] = {
                {"ntsc", VideoFormat::Raster_SD525}, {"pal", VideoFormat::Raster_SD625},
                {"576", VideoFormat::Raster_SD625},  {"720", VideoFormat::Raster_HD720},
                {"1080", VideoFormat::Raster_HD},    {"1440", VideoFormat::Raster_QHD},
                {"2160", VideoFormat::Raster_UHD},   {"4kuhd", VideoFormat::Raster_UHD},
                {"uhd4k", VideoFormat::Raster_UHD},  {"dci2k", VideoFormat::Raster_2K},
                {"dci4k", VideoFormat::Raster_4K},   {"dci8k", VideoFormat::Raster_8K},
                {"4320", VideoFormat::Raster_UHD8K}, {"8kuhd", VideoFormat::Raster_UHD8K},
        };
        ...
        const SmpteHeight kSmpteHeights[] = {
                {486, 720}, {576, 720}, {720, 1280}, {1080, 1920}, {2160, 3840}, {4320, 7680},
        };

        bool isScanInterlaced(VideoScanMode m) {
                return m == VideoScanMode::Interlaced || m == VideoScanMode::InterlacedEvenFirst ||
...
#define X(ID, NAME, NUM, DEN)                                                                                          \
        case FrameRate::ID: return String(NAME);
...
VideoFormat::VideoFormat(const Size2Du32 &raster, const FrameRate &rate, VideoScanMode scanMode)
    : _raster(raster), _rate(rate), _scanMode(scanMode) {}

VideoFormat::VideoFormat(WellKnownRaster id, const FrameRate &rate, VideoScanMode scanMode)
...
VideoFormat::WellKnownRaster VideoFormat::wellKnownRaster() const {
        ...
        for (const auto &pair : rasterDb.database()) {
                ...
                if (pair.second.width == _raster.width() && pair.second.height == _raster.height()) {
                        return pair.first;
                }
        }
...
VideoFormat::WellKnownFormat VideoFormat::wellKnownFormat() const {
        ...
        for (const auto &pair : formatDb.database()) {
                ...
                if (info.raster == wkRaster && info.rate == wkRate && scanMatchesWellKnown(_scanMode, info.scan)) {
                        return pair.first;
                }
        }
        return NotWellKnown;
...
VideoFormat::WellKnownFormatList VideoFormat::allWellKnownFormats(uint32_t requiredFlags) {
...
String VideoFormat::frameRateString() const {
        ...
        if (isScanInterlaced(_scanMode)) {
                ...
                FrameRate fieldRate(FrameRate::RationalType(_rate.numerator() * 2, _rate.denominator()));
...
Result<VideoFormat> VideoFormat::fromString(const String &str, const ParseOptions &opts) {
        ...
        if (tokens.size() == 1) {
                ...
                if (rtN >= 4 && rt[0] == 'p' && rt[1] == 's' && rt[2] == 'f' &&
                    (std::isdigit(static_cast<unsigned char>(rt[3])) || rt[3] == '.')) {
                        ...
                        recordScan(rt[0] == 'p' ? VideoScanMode::Progressive : VideoScanMode::Interlaced);
                        rateToken = rateToken.mid(1);
                } else if (rtN >= 4 && rt[rtN - 1] == 'f' && rt[rtN - 2] == 's' && rt[rtN - 3] == 'p' &&
                           (std::isdigit(static_cast<unsigned char>(rt[rtN - 4])) || rt[rtN - 4] == '.' ||
                            rt[rtN - 4] == '/')) {
                        recordScan(VideoScanMode::PsF);
                        rateToken = rateToken.left(rtN - 3);
                } else if (rtN >= 2 && (rt[rtN - 1] == 'p' || rt[rtN - 1] == 'i') &&
                           (std::isdigit(static_cast<unsigned char>(rt[rtN - 2])) || rt[rtN - 2] == '.' ||
                            rt[rtN - 2] == '/')) {
                        recordScan(rt[rtN - 1] == 'p' ? VideoScanMode::Progressive : VideoScanMode::Interlaced);
        ...
        if (rstN >= 3 && rst[rstN - 1] == 'f' && rst[rstN - 2] == 's' && rst[rstN - 3] == 'p') {
                recordScan(VideoScanMode::PsF);
        ...
        } else if (!parseExplicitRaster(rasterPart, raster) && !parseSmpteHeight(rasterPart, raster)) {
                return makeError<VideoFormat>(Error::Invalid);
        }
        ...
        if (mode == VideoScanMode::Interlaced) {
                ...
                FrameRate halved(FrameRate::RationalType(parsedRate.numerator(), parsedRate.denominator() * 2));
