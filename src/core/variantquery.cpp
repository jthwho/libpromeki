        ...
        class Lexer {
                public:
                        ...
                        Token next() {
                                ...
                                if (c == '(') {
                                        ++_pos;
                                        return make(Tok::LParen, startCol);
                                }
                                if (c == ')') {
                                        ++_pos;
                                        return make(Tok::RParen, startCol);
                                }
                                if (c == '!') {
                                        if (peek(1) == '=') {
                                                _pos += 2;
                                                return make(Tok::NotEq, startCol);
                                        }
                                        ++_pos;
                                        return make(Tok::Bang, startCol);
                                }
                                if (c == '&' && peek(1) == '&') {
                                        _pos += 2;
                                        return make(Tok::AndAnd, startCol);
                                }
                                if (c == '|' && peek(1) == '|') {
                                        _pos += 2;
                                        return make(Tok::OrOr, startCol);
                                }
                                if (c == '=' && peek(1) == '=') {
                                        _pos += 2;
                                        return make(Tok::Eq, startCol);
                                }
                                if (c == '<') {
                                        if (peek(1) == '=') {
                                                _pos += 2;
                                                return make(Tok::LtEq, startCol);
                                        }
                                        ++_pos;
                                        return make(Tok::Lt, startCol);
                                }
                                if (c == '>') {
                                        if (peek(1) == '=') {
                                                _pos += 2;
                                                return make(Tok::GtEq, startCol);
                                        }
                                        ++_pos;
                                        return make(Tok::Gt, startCol);
                                }
                                if (c == '~') {
                                        if (peek(1) == '~') {
                                                _pos += 2;
                                                return make(Tok::TildeTilde, startCol);
                                        }
                                        ++_pos;
                                        return make(Tok::Tilde, startCol);
                                }
                ...
                private:
                        ...
                        char        peek(size_t off) const { return _pos + off < _len ? _src[_pos + off] : '\0'; }
                        ...
                        static bool isIdBody(char c) {
                                ...
                                return isIdStart(c) || (c >= '0' && c <= '9') || c == '.' || c == '[' || c == ']';
                        }
                        ...
                        Token readString(int startCol) {
                                ...
                                if (_pos >= _len) {
                                        _err = String::sprintf("unterminated string literal starting at col %d",
                                                               startCol);
                                        _pos = _len;
                                        return make(Tok::End, startCol);
                                }
                        ...
                        Token readRegex(int startCol) {
                                ...
                                if (_pos >= _len) {
                                        _err = String::sprintf("unterminated regex literal starting at col %d",
                                                               startCol);
                                        _pos = _len;
                                        return make(Tok::End, startCol);
                                }
                        ...
                        Token readIdent(int startCol) {
                                ...
                                if (t.text == String("true")) {
                                        t.kind = Tok::BoolLit;
                                        t.bval = true;
                                        return t;
                                }
                                if (t.text == String("false")) {
                                        t.kind = Tok::BoolLit;
                                        t.bval = false;
                                        return t;
                                }
                                if (t.text == String("has")) {
                                        t.kind = Tok::Has;
                                        return t;
                                }
                                t.kind = Tok::Ident;
...
namespace {
        ...
        class OrNode : public detail::VariantQueryNode {
                public:
                        OrNode(detail::VariantQueryNodeUPtr l, detail::VariantQueryNodeUPtr r)
        ...
        class AndNode : public detail::VariantQueryNode {
                public:
                        AndNode(detail::VariantQueryNodeUPtr l, detail::VariantQueryNodeUPtr r)
                            : lhs(std::move(l)), rhs(std::move(r)) {}
                        bool eval(const detail::VariantQueryContext &ctx) const override {
                                return lhs->eval(ctx) && rhs->eval(ctx);
                        }
        ...
        class CmpNode : public detail::VariantQueryNode {
                public:
                        ...
                        CmpNode(Op o, Operand l, Operand r) : op(o), lhs(std::move(l)), rhs(std::move(r)) {}
        ...
        class Parser {
                public:
                        explicit Parser(const String &src) : _src(src), _lex(src.cstr(), src.byteCount()) { advance(); }
                ...
                private:
                        ...
                        void setErr(const String &msg) {
                                if (_err.isEmpty()) _err = msg;
                        }
                        ...
                        static bool isRelop(Tok k) {
                                return k == Tok::Eq || k == Tok::NotEq || k == Tok::Lt || k == Tok::LtEq ||
                                       k == Tok::Gt || k == Tok::GtEq || k == Tok::Tilde || k == Tok::TildeTilde;
                        }
        };
...
template <typename T> VariantQuery<T>::VariantQuery() = default;

template <typename T> VariantQuery<T>::~VariantQuery() = default;

template <typename T> VariantQuery<T>::VariantQuery(VariantQuery &&) noexcept = default;

template <typename T> VariantQuery<T> &VariantQuery<T>::operator=(VariantQuery &&) noexcept = default;
...
template <typename T> Result<VariantQuery<T>> VariantQuery<T>::parse(const String &expr) {
        String errDetail;
...
template <typename T> bool VariantQuery<T>::isValid() const {
...
template <typename T> const String &VariantQuery<T>::source() const {
        return _source;
}

template <typename T> const String &VariantQuery<T>::errorDetail() const {
        return _errorDetail;
}
