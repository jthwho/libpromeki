...
template <typename T>
T reflectBits(T value, int nbits) {
        T result = 0;
...
template <typename T>
CRC<T>::CRC(const Params &params) : _params(params), _crc(params.init) {
        buildTable();
}
...
template <typename T>
void CRC<T>::update(const void *data, size_t len) {
...
template <typename T>
T CRC<T>::compute(const Params &params, const void *data, size_t len) {
        CRC<T> crc(params);
        crc.update(data, len);
        return crc.value();
}
