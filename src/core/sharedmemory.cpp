...
Error SharedMemory::create(const String &name, size_t size,
                           uint32_t mode, const String &groupName) {
        ...
        int fd = ::shm_open(canonical.cstr(),
        ...
        if(!groupName.isEmpty()) {
                ...
                int rc = ::getgrnam_r(groupName.cstr(), &gr,
                                      buf.data(), buf.size(), &result);
                if(rc != 0 || result == nullptr) {
                        Error grErr = (rc == 0) ? Error(Error::NotExist)
                                                : Error::syserr(rc);
                        ::close(fd);
                        ::shm_unlink(canonical.cstr());
                        return grErr;
...
void SharedMemory::close() {
        ...
        if(_owner && !_name.isEmpty()) {
                if(::shm_unlink(_name.cstr()) != 0 && errno != ENOENT) {
                        promekiWarn("SharedMemory: shm_unlink('%s') failed: %s",
                                    _name.cstr(), ::strerror(errno));
                }
        }
