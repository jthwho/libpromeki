 ...
 */
bool findNextStartCode(const uint8_t *data, size_t len, size_t from,
                       size_t &outCodeStart, size_t &outCodeLen) {
...
Error H264Bitstream::forEachAnnexBNal(const BufferView &in, const Visitor &visit) {
        ...
        while(true) {
                ...
                bool haveNext = findNextStartCode(data, len, nalStart,
                                                  nextCodeStart, nextCodeLen);
...
Error H264Bitstream::annexBToAvcc(const BufferView &in, uint8_t lenSize,
                                  Buffer::Ptr &outBuf) {
        if(!isValidLenSize(lenSize)) return Error::InvalidArgument;
        ...
        // tightly sized.
        size_t totalSize = 0;
        const uint64_t maxPayload = (lenSize == 4) ? 0xffffffffull
                                  : (lenSize == 2) ? 0xffffull
                                                   : 0xffull;
...
Error H264Bitstream::avccToAnnexB(const BufferView &in, uint8_t lenSize,
                                  Buffer::Ptr &outBuf) {
        if(!isValidLenSize(lenSize)) return Error::InvalidArgument;
