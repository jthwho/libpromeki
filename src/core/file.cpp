...
File::File(ObjectBase *parent) : BufferedIODevice(parent) {}

File::File(const String &fn, ObjectBase *parent) : BufferedIODevice(parent), _filename(fn) {}

File::File(const char *fn, ObjectBase *parent) : BufferedIODevice(parent), _filename(fn) {}
...
int64_t File::readFromDevice(void *data, int64_t maxSize) {
        if (_resourceFile != nullptr) {
                int64_t remaining = static_cast<int64_t>(_resourceFile->size) - _resourcePos;
                if (remaining <= 0) return 0;
                int64_t n = std::min<int64_t>(maxSize, remaining);
                ::memcpy(data, _resourceFile->data + _resourcePos, static_cast<size_t>(n));
                _resourcePos += n;
                return n;
        }
...
Result<int64_t> File::seekFromCurrent(int64_t offset) const {
        if (_resourceFile != nullptr) {
                ...
                if (target < 0 || target > static_cast<int64_t>(_resourceFile->size)) {
...
int64_t File::writeBulk(const void *data, int64_t size) {
        if (!isOpen() || !isWritable()) {
                setError(Error(Error::NotOpen));
                return -1;
        }
        if (size <= 0) {
                setError(Error(Error::InvalidArgument));
                return -1;
        }
        if (data == nullptr) {
                setError(Error(Error::InvalidArgument));
