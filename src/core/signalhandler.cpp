...
void deliverQuit(int signo) {
        ...
        if(seen >= 2 &&
           LibraryOptions::instance().getAs<bool>(LibraryOptions::SignalDoubleTapExit)) {
...
constexpr int kNumTerminationSignals =
        sizeof(kTerminationSignals) / sizeof(kTerminationSignals[0]);
...
void SignalHandler::install() {
        ...
        for(int i = 0; i < kNumTerminationSignals; ++i) {
                ...
                sa.sa_flags = 0;
                if(sigaction(kTerminationSignals[i], &sa,
                             &g_savedActions[i]) == 0) {
                        g_savedValid[i] = true;
                } else {
                        g_savedValid[i] = false;
                        promekiWarn("SignalHandler: sigaction(%d) failed (errno %d)",
                                    kTerminationSignals[i], errno);
                }
        }
...
void SignalHandler::uninstall() {
        ...
        if(g_watcher != nullptr &&
           pthread_equal(pthread_self(), g_watcher->native_handle())) {
        ...
        if(g_wakePipe[0] >= 0) { ::close(g_wakePipe[0]); g_wakePipe[0] = -1; }
        if(g_wakePipe[1] >= 0) { ::close(g_wakePipe[1]); g_wakePipe[1] = -1; }
#elif defined(PROMEKI_PLATFORM_WINDOWS)
