        ...
        vars.forEach([&](const String &envName, const String &value) {
                ...
                if(!id.isValid()) {
                        promekiWarn("LibraryOptions: unknown option '%s' "
                                    "(from %s)", optName.cstr(), envName.cstr());
                        return;
                }
                const VariantSpec *s = spec(id);
                if(s == nullptr) {
                        promekiWarn("LibraryOptions: no spec for '%s'",
                                    optName.cstr());
                        return;
                }
                Error err;
                ...
                if(err.isError()) {
                        promekiWarn("LibraryOptions: failed to parse '%s=%s': %s",
                                    optName.cstr(), value.cstr(), err.desc().cstr());
                        return;
                }
                set(id, std::move(v));
        });
