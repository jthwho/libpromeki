...
Result<UUID> UUID::fromString(const String &string) {
        ...
        uint8_t   *d = data.data();
        Error      err;
        if (!hexStr(d++, str, &err)) goto parseFailed;
        str += 2;
        if (!hexStr(d++, str, &err)) goto parseFailed;
        str += 2;
        if (!hexStr(d++, str, &err)) goto parseFailed;
...
parseFailed:
        promekiWarn("UUID::fromString failed to parse '%s'", string.cstr());
        return makeError<UUID>(Error::ParseFailed);
}
