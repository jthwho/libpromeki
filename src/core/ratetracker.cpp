...
RateTracker::RateTracker(int64_t windowMs) : _windowMs(windowMs < 1 ? 1 : windowMs) {
...
double RateTracker::bytesPerSecond() const {
        ...
        if (elapsedNs < halfNs && _haveLastWindow) {
                ...
                return static_cast<double>(_lastWindowBytes) * kNsPerSecond / static_cast<double>(_lastWindowElapsedNs);
        }

        if (elapsedNs <= 0) return 0.0;
        int64_t b = _bytes.value();
        return static_cast<double>(b) * kNsPerSecond / static_cast<double>(elapsedNs);
}
...
double RateTracker::framesPerSecond() const {
        ...
        if (elapsedNs <= 0) return 0.0;
        int64_t f = _frames.value();
        return static_cast<double>(f) * kNsPerSecond / static_cast<double>(elapsedNs);
}
