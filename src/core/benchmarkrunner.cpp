...
String renderTable(const List<Column> &cols) {
        ...
        for(size_t r = 0; r < rowCount; r++) {
                for(int i = 0; i < static_cast<int>(cols.size()); i++) {
                        if(i > 0) out += "  ";
                        String cell = (r < cols[i].cells.size())
                                ? cols[i].cells[r]
                                : String();
                        out += format(i, cell);
                }
...
BenchmarkResult BenchmarkResult::fromJson(const JsonObject &obj, Error *err) {
        ...
        if(obj.valueIsObject("custom")) {
                ...
                c.forEach([&](const String &k, const Variant &v) {
                        r.custom.insert(k, v.get<double>());
                });
        }
...
int64_t BenchmarkRunner::estimatedDurationMs() const {
        ...
        int64_t perCase = static_cast<int64_t>(_warmupMs)
                        + static_cast<int64_t>(_repeats)
                                * static_cast<int64_t>(_minTimeMs);
...
void BenchmarkRunner::runOne(const BenchmarkCase &theCase,
                             uint64_t iterations,
...
BenchmarkResult BenchmarkRunner::measureCase(const BenchmarkCase &theCase) {
        ...
        for(unsigned int rep = 0; rep < _repeats; rep++) {
                ...
                double nsPerIter = static_cast<double>(state.effectiveNs())
                        / static_cast<double>(iter > 0 ? iter : 1);
...
String BenchmarkRunner::formatTable() const {
        ...
        Column suiteCol;  suiteCol.header  = "Suite";   suiteCol.align = Align::Left;
        Column nameCol;   nameCol.header   = "Case";    nameCol.align  = Align::Left;
        Column iterCol;   iterCol.header   = "Iter";    iterCol.align  = Align::Right;
        Column timeCol;   timeCol.header   = "Time";    timeCol.align  = Align::Right;
        Column stddevCol; stddevCol.header = "Stddev";  stddevCol.align= Align::Right;
        Column itemsCol;  itemsCol.header  = "Items/s"; itemsCol.align = Align::Right;
        Column rateCol;   rateCol.header   = "Throughput"; rateCol.align = Align::Right;
        Column statusCol; statusCol.header = "";        statusCol.align= Align::Left;
...
String BenchmarkRunner::formatComparison(const List<BenchmarkResult> &baseline) const {
        ...
        Column suiteCol;  suiteCol.header   = "Suite";    suiteCol.align = Align::Left;
        Column nameCol;   nameCol.header    = "Case";     nameCol.align  = Align::Left;
        Column currentCol;currentCol.header = "Current";  currentCol.align = Align::Right;
        Column baseCol;   baseCol.header    = "Baseline"; baseCol.align  = Align::Right;
        Column deltaCol;  deltaCol.header   = "Delta";    deltaCol.align = Align::Right;
...
String BenchmarkRunner::formatRegisteredCases() {
        ...
        Column suiteCol; suiteCol.header = "Suite";       suiteCol.align = Align::Left;
        Column nameCol;  nameCol.header  = "Case";        nameCol.align  = Align::Left;
        Column descCol;  descCol.header  = "Description"; descCol.align  = Align::Left;
