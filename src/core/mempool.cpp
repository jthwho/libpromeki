...
void MemPool::dump() const {
        ...
        for (const auto &block : map) {
                if (block.allocated) {
                        promekiInfo("A [0x%16llX] %d bytes, %d align", (unsigned long long)block.address,
                                    (int)block.size, (int)block.alignment);
                } else {
                        promekiInfo("F [0x%16llX] %d bytes", (unsigned long long)block.address, (int)block.size);
                }
        }
        return;
}
...
void *MemPool::allocate(size_t size, size_t alignment) {
        if (!isValidAlignment(alignment)) {
                promekiErr("MemPool '%s': allocate failed, alignment invalid. Size %d, Align %d", _name.cstr(),
...
void MemPool::free(void *ptr) {
        ...
        if (!insret.second()) {
                promekiErr("MemPool '%s': free %p (%p) failed to insert block", _name.cstr(), ptr,
                           (void *)block.address);
                return;
        }
        ...
        if (update) {
                ...
                if (!insret.second()) {
                        promekiErr("MemPool '%s': free %p (%p) failed to insert updated block", _name.cstr(), ptr,
                                   (void *)block.address);
                        return;
                }
        }
