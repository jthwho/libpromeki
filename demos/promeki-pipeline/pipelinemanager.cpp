...
namespace promekipipeline {
        ...
        PipelineManager::~PipelineManager() {
                ...
                for (auto &e : doomed) {
                        ...
                        if (e.pipeline.get() != nullptr) {
                                ...
                                if (s != MediaPipeline::State::Empty && s != MediaPipeline::State::Closed) {
        ...
        String PipelineManager::create(const String &name) {
                ...
                entry.pipelineSubId =
                        raw->subscribe([this, capturedId](const PipelineEvent &ev) { publish(capturedId, ev); });
        ...
        Error PipelineManager::replaceConfig(const String &id, const MediaPipelineConfig &userConfig) {
                promeki::Mutex::Locker guard(_mutex);
                auto                   it = _entries.find(id);
                if (it == _entries.end()) return Error::NotExist;
                Entry &e = it->second;
        ...
        Error PipelineManager::replaceSettings(const String &id, const PipelineSettings &settings) {
                ...
                promeki::Duration newInterval;
                {
                        ...
                        e.settings = settings;
                        if (e.pipeline.get() != nullptr && e.pipeline->state() == MediaPipeline::State::Running &&
        ...
        Error PipelineManager::build(const String &id) {
                ...
                promeki::Duration   statsInterval;
                {
                        ...
                        if (e.settings.autoplan()) {
                                ...
                                Error err = MediaPipelinePlanner::plan(e.userConfig, &resolved, policy, &diag);
                                if (err.isError()) {
                                        if (!diag.isEmpty()) {
                                                promekiWarn("Pipeline %s plan failed: %s", id.cstr(), diag.cstr());
                                        }
                                        return err;
                                }
        ...
        Error PipelineManager::run(const String &id) {
                ...
                if (state == MediaPipeline::State::Empty || state == MediaPipeline::State::Closed) {
                        Error err = build(id);
                        if (err.isError()) return err;
        ...
        bool PipelineManager::stateAllowsConfigRewrite(MediaPipeline::State s) {
                return s == MediaPipeline::State::Empty || s == MediaPipeline::State::Built ||
        ...
        JsonObject PipelineManager::describeEntry(const Entry &e) const {
                ...
                obj.set("state", e.pipeline.get() != nullptr ? stateToString(e.pipeline->state()) : String("Empty"));
