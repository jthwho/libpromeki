...
namespace promekipipeline {
        ...
        EventBroadcaster::EventBroadcaster(HttpServer &server, PipelineManager &mgr, ObjectBase *parent)
            : ObjectBase(parent), _server(server), _manager(mgr) {
                ...
                _server.routeWebSocket(EventsRoute, [this](WebSocket *socket, const HttpRequest &req) {
                        if (socket == nullptr) return;
                        const String filter = req.queryValue(PipelineQueryKey);
                        onUpgrade(socket, filter);
                });

                _subId = _manager.subscribe([this, wsLoop](const String &id, const PipelineEvent &ev) {
                        ...
                        List<WebSocket *> targets;
                        {
                                ...
                                for (auto it = _subs.cbegin(); it != _subs.cend(); ++it) {
                                        ...
                                        if (s.pipelineFilter.isEmpty() || s.pipelineFilter == id) {
                        ...
                        for (size_t i = 0; i < targets.size(); ++i) {
                                ...
                                wsLoop->postCallable([this, ws, wire]() {
                                        ...
                                        bool stillKnown = false;
                                        {
                                                ...
                                                stillKnown = _subs.find(ws) != _subs.end();
                                        }
