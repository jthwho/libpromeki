...
const Duration PipelineSettings::DefaultStatsInterval =
        Duration::fromSeconds(1);

PipelineSettings::PipelineSettings()
        : _name(DefaultName),
          _statsInterval(DefaultStatsInterval),
          _quality(DefaultQuality),
          _maxBridgeDepth(DefaultMaxBridgeDepth),
          _autoplan(DefaultAutoplan) {}
...
JsonObject PipelineSettings::toJson() const {
        JsonObject obj;
        obj.set("name", _name);
        obj.set("statsIntervalMs",
                static_cast<int64_t>(_statsInterval.milliseconds()));
...
PipelineSettings PipelineSettings::fromJson(const JsonObject &obj, Error *err) {
        ...
        if(obj.contains("maxBridgeDepth")) {
                out._maxBridgeDepth =
                        static_cast<int>(obj.getInt("maxBridgeDepth"));
        }
...
MediaPipelinePlanner::Quality PipelineSettings::qualityFromString(
        const String &s, bool *ok) {
        if(ok) *ok = true;
