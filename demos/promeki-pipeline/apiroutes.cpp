...
JsonObject errorBody(const String &what,
...
void sendError(HttpResponse &res, const HttpStatus &status,
               const String &what, const String &name = String()) {
...
JsonArray typePresets(Variant::Type primary) {
        ...
        if(primary == Variant::TypeFrameRate) {
                const List<FrameRate::WellKnown> rates =
                        FrameRate::wellKnownRates();
...
bool stateAllowsStart(MediaPipeline::State s) {
        return s == MediaPipeline::State::Open
            || s == MediaPipeline::State::Stopped;
}
...
void sendLifecycleResult(HttpResponse &res, const Error &err,
                         const String &id, PipelineManager &manager) {
        if(err.isOk()) {
...
ApiRoutes::ApiRoutes(HttpServer &server, PipelineManager &manager)
...
void ApiRoutes::registerTypeRoutes() {
        ...
        _server.route("/api/health", HttpMethod::Get,
        ...
        _server.route("/api/types", HttpMethod::Get,
                [](const HttpRequest &, HttpResponse &res) {
                        JsonArray arr;
                        const auto &all = MediaIO::registeredFormats();
                        for(size_t i = 0; i < all.size(); ++i) {
        ...
        _server.route("/api/types/{name}/schema", HttpMethod::Get,
                [](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(findFormat(name) == nullptr) {
                                sendError(res, HttpStatus::NotFound,
                                          "unknown type", name);
                                return;
                        }
...
void ApiRoutes::registerPipelineRoutes() {
        ...
        _server.route("/api/pipelines", HttpMethod::Get,
                [&mgr](const HttpRequest &, HttpResponse &res) {
        ...
        _server.route("/api/pipelines", HttpMethod::Post,
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(hasBody && body.contains("settings")) {
                                Error sErr;
                                const JsonObject settingsJson =
                                        body.getObject("settings", &sErr);
                                if(sErr.isOk()) {
                                        Error decodeErr;
                                        PipelineSettings s =
                        ...
                        if(hasBody && body.contains("userConfig")) {
                                ...
                                const JsonObject userJson =
                                        body.getObject("userConfig",
                                                           &cErr);
                                if(cErr.isOk()) {
                                        Error decodeErr;
                                        MediaPipelineConfig cfg =
                                                MediaPipelineConfig::fromJson(
                                                        userJson, &decodeErr);
                                        if(decodeErr.isOk()) {
        ...
        _server.route("/api/pipelines/{id}", HttpMethod::Put,
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(perr.isError()) {
                                sendError(res, HttpStatus::BadRequest,
                                          "invalid JSON body");
                                return;
                        }
                        Error decodeErr;
                        const MediaPipelineConfig cfg =
                                MediaPipelineConfig::fromJson(body,
                                                              &decodeErr);
        ...
        _server.route("/api/pipelines/{id}/settings", HttpMethod::Put,
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        const PipelineSettings s =
                                PipelineSettings::fromJson(body,
                                                           &decodeErr);
                        if(decodeErr.isError()) {
                                sendError(res, HttpStatus::BadRequest,
                                          decodeErr.name());
                ...
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(!autoplanQ.isEmpty()) {
                                if(autoplanQ == "1" || autoplanQ.toLower() == "true") {
                                       ...
                                       || autoplanQ.toLower() == "false") {
                                        overrideAutoplan = true;
                                        autoplanValue = false;
                                } else {
                                        sendError(res, HttpStatus::BadRequest,
                                                  "invalid autoplan query");
                                        return;
                                }
                        }
                        ...
                        if(peek->pipeline.get() != nullptr &&
                           !stateAllowsBuild(peek->pipeline->state())) {
                                sendError(res, HttpStatus::Conflict,
                                          "pipeline not in a buildable state");
                                return;
                        }
                        if(overrideAutoplan) {
                ...
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(peek->pipeline.get() != nullptr &&
                           !stateAllowsOpen(peek->pipeline->state())) {
                                sendError(res, HttpStatus::Conflict,
                                          "pipeline not in an openable state");
                                return;
                        }
                ...
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(peek->pipeline.get() != nullptr &&
                           !stateAllowsStart(peek->pipeline->state())) {
                                sendError(res, HttpStatus::Conflict,
                                          "pipeline not in a startable state");
                                return;
                        }
                ...
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(peek->pipeline.get() != nullptr &&
                           !stateAllowsStop(peek->pipeline->state())) {
                                sendError(res, HttpStatus::Conflict,
                                          "pipeline not in a stoppable state");
                                return;
                        }
                ...
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        if(peek->pipeline.get() != nullptr &&
                           !stateAllowsClose(peek->pipeline->state())) {
                                sendError(res, HttpStatus::Conflict,
                                          "pipeline not in a closable state");
                                return;
                        }
...
void ApiRoutes::registerPreviewRoute() {
        ...
        _server.route("/api/pipelines/{id}/preview/{stage}", HttpMethod::Get,
                [&mgr](const HttpRequest &req, HttpResponse &res) {
                        ...
                        const String type = stage->config().contains(
                                MediaConfig::Type)
                                ? stage->config().getAs<String>(
                                        MediaConfig::Type)
                                : String();
                        if(type != "MjpegStream") {
                                sendError(res, HttpStatus::BadRequest,
                                          "stage is not an MjpegStream sink",
                                          stageName);
                                return;
                        }
                        auto *mjpeg = static_cast<MediaIOTask_MjpegStream *>(
                                stage->task());
                        if(mjpeg == nullptr) {
                                sendError(res, HttpStatus::ServiceUnavailable,
                        ...
                        MediaIOTask_MjpegStream::buildMultipartHandler(mjpeg)
                                (req, res);
                });
}
