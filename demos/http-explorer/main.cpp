        ...
        Args parseArgs(int argc, char **argv) {
                ...
                for (int i = 1; i < argc; ++i) {
                        if (std::strcmp(argv[i], "--port") == 0 && i + 1 < argc) {
                                ...
                                std::fprintf(stderr, "usage: %s [--port N] [--bind HOST]\n", argv[0]);
                                std::exit(2);
                        }
                }
                return a;
...
int main(int argc, char **argv) {
        ...
        api.setDescription("Sample API surface that mounts a custom VariantDatabase, "
        ...
        if (Error err = api.exposeDatabase("/demo/config", "Demo config", DemoConfig::instance(),
                                           /*readOnly=*/false);
            err.isError()) {
        ...
        if (Error err = api.exposeLookup("/demo/state", "Demo state", state); err.isError()) {
        ...
        if (Error err = api.rpc(echoEp,
                                [specDefault](const VariantMap &args) -> Result<Variant> {
                                        ...
                                        const String defaultText =
                                                cfg.get(DemoConfig::Greeting, specDefault("Greeting")).get<String>();
                                        const bool defaultVerbose =
                                                cfg.get(DemoConfig::Verbose, specDefault("Verbose")).get<bool>();

                                        const String text =
                                                args.contains("text") ? args.value("text").get<String>() : defaultText;
                                        ...
                                        String out;
                                        if (verbose) {
                                                out = String("[") + String::number(state.counter) + "] " + text;
                                        } else {
                                                out = text;
                                        }
        ...
        // ----------------------------------------------------------
        Error        resErr;
        const Buffer indexBuf = Resource::data(":/demo/explorer/index.html", &resErr);
        if (resErr.isError()) {
                promekiErr("missing baked-in index.html: %s", resErr.name().cstr());
                return 1;
        }
        const String indexTemplate(static_cast<const char *>(indexBuf.data()), indexBuf.size());
        const String indexHtml = indexTemplate.replace("__API_BASE__", apiPrefix);

        auto       staticHandler = HttpHandler::Ptr::takeOwnership(new HttpFileHandler(Dir(":/demo/explorer")));
        const auto homePage = [indexHtml](const HttpRequest &, HttpResponse &res) {
        ...
        // Step 7 — listen.
        // ----------------------------------------------------------
        SocketAddress bindAddr = SocketAddress::fromString(args.bind + ":" + String::number(args.port)).first();
        if (Error lerr = server.listen(bindAddr); lerr.isError()) {
                promekiErr("listen failed on %s: %s", bindAddr.toString().cstr(), lerr.name().cstr());
                return 1;
        }
