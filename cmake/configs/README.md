...
PROMEKI_ENABLE_JPEGXS        PROMEKI_ENABLE_X264
PROMEKI_ENABLE_FREETYPE
...
> **GPL note:** `PROMEKI_ENABLE_X264` (default ON) is the vendored libx264
> H.264 software encoder, licensed GPL-2.0-or-later. Enabling it makes the
> resulting libpromeki build a GPL artifact — CMake configure prints a GPL
> COMPLIANCE NOTICE (see `THIRD-PARTY-LICENSES`). It requires
> `PROMEKI_ENABLE_PROAV`. Set it OFF to keep a non-GPL build; NVENC still
> provides H.264 encode on GPU hosts.
